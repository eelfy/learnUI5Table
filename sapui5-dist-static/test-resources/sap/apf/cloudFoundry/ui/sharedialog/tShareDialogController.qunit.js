sap.ui.define([
...
], function (ShareDialog, LaunchPageUtils, MessageBox, Library, Fragment, ValueState, sinon) {
	'use strict';
	...
	QUnit.test("Get UI", function (assert) {
		var env = this;
		env.bSaved = true;
		env.oShareDialog.onInit();
		assert.ok(env.spyIsSaved.calledOnce, "isSaved is called once");
	...
	QUnit.test("Get Configuration", function (assert) {
		var env = this;
		env.bSaved = true;
		env.oShareDialog.onInit();
	...
	QUnit.test("Set Model", function (assert) {
		var env = this;
		env.bSaved = true;
		env.oShareDialog.onInit();
	...
	QUnit.module("Actions", {
		beforeEach: function () {
			...
			//stub triggerEmail
			env.stubTriggerEmail = sinon.stub(Library.URLHelper, "triggerEmail");
	...
	QUnit.module("Get Dialog", {
		beforeEach: function () {
			...
			env.stubGetView.returns({
				byId: env.stubById,
				addDependent: env.spyAddDependent,
				getId: env.stubGetId
			});
			env.stubXmlFragment = sinon.stub(sap.ui, "xmlfragment");
		},
		afterEach: function () {
			this.stubGetView.restore();
			this.stubXmlFragment.restore();
			this.oShareDialog.destroy();
		}
	});
	QUnit.test("Available", function (assert) {
		...
		assert.ok(env.stubGetId.notCalled, "view.getId is not called");
		assert.ok(env.stubXmlFragment.notCalled, "xmlfragment is not called");
		assert.ok(env.spyAddDependent.notCalled, "view.addDependent is not called");
	...
	QUnit.test("Not Available", function (assert) {
		...
		env.stubXmlFragment.returns(oDialog);
		var result = env.oShareDialog.getDialog();
		assert.ok(env.stubGetView.calledOnce, "getView is called once");
		assert.ok(env.stubById.calledOnce, "byId is called once");
		assert.ok(env.stubById.calledWith("shareDialog"), "byId is called with 'shareDialog'");
		assert.ok(env.stubGetId.calledOnce, "view.getId is called once");
		assert.ok(env.stubXmlFragment.calledOnce, "xmlfragment is called once");
		assert.ok(env.stubXmlFragment.calledWith(env.sViewId, "sap.apf.cloudFoundry.ui.sharedialog.fragment.ShareDialog", env.oShareDialog), "xmlfragment is called with the fragment name");
		assert.ok(env.spyAddDependent.calledOnce, "view.addDependent is called once");
		assert.ok(env.spyAddDependent.calledWith(oDialog), "view.addDependent is called with the dialog");
		assert.strictEqual(result, oDialog, "dialog is returned");
	});
