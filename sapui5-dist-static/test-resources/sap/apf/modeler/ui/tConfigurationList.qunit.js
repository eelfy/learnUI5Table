...
(function() {
	...
	QUnit.test("navigateToNode Test - when selected link is a configuration link", function(assert) {
		...
		this.oConfigurationListController.oTreeInstance.getModel().getContext(sPath);
	...
	QUnit.test("Test press of execute button", function(assert) {
		...
		assert.strictEqual(navigateToGenericRuntimeStub.calledWith("appId", "configId", openWindow), true, "then navigate to generic runtime was called");
		//cleanup
	...
	QUnit.module("Publish Button Text and Visibility", {
		beforeEach : function() {
			...
			this.stubShowShareDialog = sinon.stub(sap.apf.cloudFoundry.uiHandler, "showShareDialog");
		},
		afterEach : function() {
			...
			this.stubShowShareDialog.restore();
		}
	});
	...
	QUnit.test("Set correct text", function(assert) {
		...
		this.byId = function(name) {
			return {
				setTitle : function() {},
				setText : name === "idPublishbutton" ? spySetText : function() {},
				setVisible : function() {}
			};
		};
		this.controller.onInit();
	...
	QUnit.test("Set publish visible (not on cf)", function(assert) {
		var spySetVisible = sinon.spy();
		this.byId = function(name) {
			return {
				setTitle : function() {},
				setText : function() {},
				setVisible : name === "idPublishbutton" ? spySetVisible : function() {}
			};
		};
	...
	QUnit.test("Set publish visible (on cf)", function(assert) {
		var spySetVisible = sinon.spy();
		this.byId = function(name) {
			return {
				setTitle : function() {},
	...
	QUnit.test("Handle publish press (not on cf)", function(assert) {
		this.isUsingCloudFoundryProxy = false;
		this.controller.onInit();
		...
		assert.ok(this.stubShowShareDialog.notCalled, "publishTileDialog is not called");
	});
	QUnit.test("Handle publish press (on cf)", function(assert) {
		this.isUsingCloudFoundryProxy = true;
		this.controller.onInit();
		this.controller.handlePublishPress();
		assert.ok(this.spyIsUsingCF.calledTwice, "isUsingCloudFoundryProxy is called twice (init and handlePublishPress)");
		assert.ok(this.stubShowShareDialog.calledOnce, "publishTileDialog is called once");
		assert.ok(this.stubShowShareDialog.calledWith(this.oCoreApi, this.controller), "publishTileDialog is called with the core api");
	});
}());
