sap.ui.define("sap.apf.modeler.ui.tImportDeliveredContent", [
	...
	function _readAllCachedConfigurationsFromVendorLayerStub() {
		// see sap.apf.cloudFoundry.modelerProxy#readAllConfigurationsFromVendorLayer
		var oDeferredCall = new jQuery.Deferred();
		oDeferredCall.resolve(aCachedConfigs);
		return oDeferredCall.promise();
	}
	...
	QUnit.module("import configuration from vendor layer when lrep is active and configurations are cached", {
		beforeEach: function(assert) {
			var done = assert.async();
			var that = this;
			this.oModelerInstance = null;
			modelerUIHelper.getModelerInstance(function(modelerInstance) {
				...
				createMessageObjectSpy = sinon.spy(that.oModelerInstance.modelerCore, "createMessageObject");
				putMessageStub = sinon.stub(that.oModelerInstance.modelerCore, "putMessage", function(){});
				readAllConfigurationsFromVendorLayerSpy = sinon.stub(that.oModelerInstance.modelerCore, "readAllConfigurationsFromVendorLayer", _readAllCachedConfigurationsFromVendorLayerStub);
				importConfigurationSpy = sinon.stub(that.oModelerInstance.modelerCore, "importConfigurationFromVendorLayer", _importConfigurationFromVendorLayerStub);
				callbackOverwrite = sinon.stub();
				callbackDoNotOverwrite = sinon.stub();
				done();
			});
	...
	QUnit.test("When import delivered content view is opened twice", function(assert) {
		for (var i = 0; i < 2; i++) {
			var oImportDeliveredContentController = new sap.ui.controller("sap.apf.modeler.ui.controller.importDeliveredContent");
			var oParentControl = new sap.m.Table();
			var oImportDeliveredContentView = new sap.ui.view({
				viewName : "sap.apf.modeler.ui.view.importDeliveredContent",
				type : sap.ui.core.mvc.ViewType.XML,
				controller : oImportDeliveredContentController,
				viewData : {
					oParentControl : oParentControl,
					oCoreApi : this.oModelerInstance.modelerCore
				}
			});
