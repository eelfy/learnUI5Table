...
(function() {
	...
	QUnit.test("WHEN registerApplicationCreatedOnServer is called without 'this' available", function(assert) {
		// see sap.apf.cloudFoundry.modelerProxy#importVendorConfiguration.registerApplicationAndSendCallback
		var register = this.applicationHandler.registerApplicationCreatedOnServer;
		register("createId1", "app1");
		var appsAfterRegistration = this.applicationHandler.getList();
		assert.strictEqual(appsAfterRegistration.length, 1, "THEN application is registered");
	});
}());
