...
sap.ui.define("sap/apf/ui/representations/tBaseVizFrameChartRepresentation.qunit", [
	...
	QUnit.test("When getMainContent is called", function(assert) {
		assert.strictEqual(this.messageHandlerSpy.callCount, 2, "Only two messages was raised");
		assert.strictEqual(this.messageHandlerSpy.getCalls()[0].args[0].getCode(), "3001", "Text is not available message was raised - No chart related message");
		assert.strictEqual(this.messageHandlerSpy.getCalls()[1].args[0].getCode(), 5074, "ushell navigation message was raised - No chart related message");
	});
