...
sap.ui.define([
	...
	QUnit.module("Availability Tests For Analysis Path and Popover Menu", {
		beforeEach : function(assert) {
			...
			createUiApiAsPromise("comp", undefined, inject).done(function(api){
				...
				this.layoutController = this.oGlobalApi.oUiApi.getLayoutView().getController();
	...
	QUnit.test("Path name and dirty state of initial path", function(assert) {
		assert.strictEqual(this.layoutController.oSavedPathName.getText(), "Unnamed Analysis Path", "The Analysis Path name has the correct title");
		assert.strictEqual(this.layoutController.oSavedPathName.getText().indexOf("*"), -1, "Name of a new Analysis Path does not have a *");
		var bStarInName = (this.layoutController.oSavedPathName.getText().indexOf("*") !== -1);
	...
	QUnit.test('Set path title for new clean path', function(assert) {
		this.layoutController.setPathTitle();
		assert.strictEqual(this.layoutController.oSavedPathName.getText(), this.oGlobalApi.oCoreApi.getTextNotHtmlEncoded('unsaved'), 'Language dependent default title without asterisk');
	});
	QUnit.test('Set path title for unsaved dirty path', function(assert) {
		this.oGlobalApi.oCoreApi.setDirtyState(true);
		this.layoutController.setPathTitle();
		assert.strictEqual(this.layoutController.oSavedPathName.getText(), '*' + this.oGlobalApi.oCoreApi.getTextNotHtmlEncoded('unsaved'), 'Asterisk indicates dirty path with language dependent default title');
	});
	QUnit.test('Set path title for a saved or opened path', function(assert) {
		this.oGlobalApi.oCoreApi.setPathName("User's Delight");
		this.layoutController.setPathTitle();
		assert.strictEqual(this.layoutController.oSavedPathName.getText(), "User's Delight", 'Previously set path name returned displayed as clean path');
	});
	QUnit.test('Set path title for a saved or opened dirty path', function(assert) {
		this.oGlobalApi.oCoreApi.setPathName("User's Delight");
		this.oGlobalApi.oCoreApi.setDirtyState(true);
		this.layoutController.setPathTitle();
		assert.strictEqual(this.layoutController.oSavedPathName.getText(), "*User's Delight", 'Previously set path name prefixed with asterisk as indicator for dirty path');
	});
