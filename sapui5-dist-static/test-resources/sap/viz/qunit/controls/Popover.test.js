...
], function(vizFrameData, waitForThemeApplied) {
...
$(function() {
  ...
  $('#content').css('width', (window.screen.width-16)+'px');
});
...
QUnit.test("Check small data point." , function( assert ) {
  ...
  var checkSmallDataPoint = function(e){
    var data = e.getParameters(),
        ...
        popPos = this.$()[0].getBoundingClientRect();
    ...
    assert.equal($('.viz-controls-chartPopover-measure-name').css('text-align'), 'left', "Measure label should align in the left side"
      + " (" + jQuery("head > link[id^='sap-ui-theme-']").get().map(function(link) { return link.id.slice(13);}).join(", ") + ")");

    assert.equal($('.viz-controls-chartPopover-measure-labels.viz-controls-chartPopover-measure-value .sapMObjectNumberText').text(), "80", "Measure value should be correct.");
...
QUnit.test("check Negative DataPoint." , function( assert ) {
  ...
  var checkNegativeDataPoint = function(e){
    ...
    assert.ok(rectPos.right < popPos.left + popPos.width, 'Popover should be opend in the right side.');
    done();
...
QUnit.test("Check bullet data point." , function( assert ) {
  ...
  var checkSmallDataPoint = function(e){
    ...
    assert.equal($('.viz-controls-chartPopover-measure-labels.viz-controls-chartPopover-measure-value .sapMObjectNumberText')[0].textContent, "45.3", "Measure value should be correct.");
    assert.equal($('.viz-controls-chartPopover-measure-labels.viz-controls-chartPopover-measure-value .sapMObjectNumberText')[1].textContent, "73.2", "Measure value should be correct.");
...
QUnit.test("check samll data point." , function( assert ) {
  ...
  var checkSmallDataPoint = function(e){
    ...
    assert.ok(popPos.height + popPos.top < rectPos.top, 'Popover should be opend in the top side.');
...
QUnit.test("check Time format." , function( assert ) {
  ...
  var checkTimeFormat = function(e){
    ...
    assert.equal($('.viz-controls-chartPopover-measure-labels.viz-controls-chartPopover-measure-value .sapMObjectNumberText')[0].innerText, "Jan 2, 2015", "Follows chart's tooltip format");
...
QUnit.test("check Time format with Popover's formatString." , function( assert ) {
  ...
  var checkTimeFormat = function(e){
    assert.equal(this.getPlacement(), 'VerticalPreferredTop', 'Popover should be opened with VerticalPreferredTop.');
    assert.equal($('.viz-controls-chartPopover-measure-labels.viz-controls-chartPopover-measure-value .sapMObjectNumberText')[0].innerText, "2015-01-02", "Follows chart's tooltip format");
...
QUnit.test("check unitBinding" , function( assert ) {
  ...
  var checkUnitBinding = function(e){
    assert.equal($('.viz-controls-chartPopover-measure-labels.viz-controls-chartPopover-measure-value .sapMObjectNumberUnit')[0].textContent, "w", "unitBinding is correct displayed in Popover");
    done();
  };
...
QUnit.test("check unit format setting" , function( assert ) {
  ...
  var checkUnitBinding = function(e){
    assert.equal($('.viz-controls-chartPopover-measure-labels .sapMObjectNumberUnit')[0].innerText, "*^", "unit setting is correct and the priority of format setting is highter than unitBinding");
    done();
  };
...
QUnit.test("check bigNumber display" , function( assert ) {
  ...
  var checkUnitBinding = function(e){
    assert.equal($('.viz-controls-chartPopover-measure-labels .sapMObjectNumberText')[0].innerText, "1,337,087,666,787,876,655.53",
...
QUnit.done(function() {
	...
	document.getElementById("markers").style.height = "0";
});

  // start the tests only after the theme has been applied
