...
sap.ui.require([
    ...
    "use strict";
    /*global deepEqual, equal, module, ok, sinon, test */
    ...
    test("_applyJsonApplicationConfig: load config file and apply it without preparing sap-ushell-config", function () {
        // Arrange
        var oBackupConfig;
        if (window["sap-ushell-config"]) {
            oBackupConfig = JSON.parse(JSON.stringify(window["sap-ushell-config"]));
        }
        ...
        var oSjaxStub = sinon.stub(jQuery.sap, "sjax", function () {
            return {
                success: true,
                data: oConfig
            };
        ...
        // Cleanup
        oSjaxStub.restore();
        window["sap-ushell-config"] = oBackupConfig;
    });
