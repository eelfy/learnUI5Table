...
sap.ui.require([
    ...
    QUnit.test("If the step is byComponentCreate, resolving the promise finishes the step loading and emits stepdone", function (assert) {
        ...
        var oPromise = Promise.resolve("myDuperStep");
        assert.expect(7);
        var oProcessConfigStub = sinon.stub(ControlUnit, "_processStepConfiguration").returns(oStepConfig);
        ...
        assert.ok(this.oLoadByCreateStub.calledWith(oStepConfig), "FLP loader called with the correct object.");
        assert.ok(this.oSetForStep.calledWith("myDuperStep", state.id.loadingStep.InProgress, oStepConfig.oData), "Step updated to in progress.");
    ...
    QUnit.test("If the step is byComponentCreate, rejecting the promise finishes the step loading and emits stepfailed", function (assert) {
        ...
        var oPromise = Promise.reject("myDuperStep");
        assert.expect(7);
        var oProcessConfigStub = sinon.stub(ControlUnit, "_processStepConfiguration").returns(oStepConfig);
        var oEventHubSpy = sinon.spy(EventHub, "emit");
        this.oSchedulerNextStep.onCall(0).returns(this.oStep);
    ...
    QUnit.test("If there are several steps of byComponentCreate, all promises are erased", function (assert) {
        // Arrange
        var done = assert.async();
        this.oStep.sStatus = "step";
        var oStepConfig1 = {
            sStepType: "byComponentCreate",
            sStepName: "myDuperStep1",
            oData: "sap.ui.my.duper.component1",
            bConfigOk: true // so true
        };
        var oStepConfig2 = {
            sStepType: "byComponentCreate",
            sStepName: "myDuperStep2",
            oData: "sap.ui.my.duper.component2",
            bConfigOk: true // so true
        };
        var oDoneStep = {
            sStatus: "done",
        ...
        var oPromise1 = Promise.resolve(oStepConfig1.sStepName),
            oPromise2 = Promise.resolve(oStepConfig2.sStepName);
        assert.expect(8);
        var oProcessConfigStub = sinon.stub(ControlUnit, "_processStepConfiguration");
        oProcessConfigStub.onCall(0).returns(oStepConfig1);
        oProcessConfigStub.onCall(1).returns(oStepConfig2);
        var oEventHubSpy = sinon.spy(EventHub, "emit");
        this.oSchedulerNextStep.onCall(0).returns(this.oStep);
        this.oSchedulerNextStep.onCall(1).returns(this.oStep);
        // we stop the loading after the first call
        this.oSchedulerNextStep.onCall(2).returns(oDoneStep);
        this.oLoadByCreateStub.onCall(0).returns(oPromise1);
        this.oLoadByCreateStub.onCall(1).returns(oPromise2);
        // need to break the Agent's loop!
        this.oSchedulerNextStep.onCall(2).returns({sStatus: "done"});

        // Act
        ControlUnit.eventReceived();

        // Assert
        assert.ok(this.oLoadByCreateStub.calledTwice, "The FLP loader was called.");
        assert.ok(this.oLoadByCreateStub.firstCall.calledWith(oStepConfig1), "FLP loader called with the correct object.");
        assert.ok(this.oLoadByCreateStub.secondCall.calledWith(oStepConfig2), "FLP loader called with the correct object.");
        assert.ok(ControlUnit.oComponentsLoading.myDuperStep1 === oPromise1, "Promise was stored.");
        assert.ok(ControlUnit.oComponentsLoading.myDuperStep2 === oPromise2, "Promise was stored.");

        Promise.all([oPromise1, oPromise2]).then(function () {
            assert.ok(oEventHubSpy.calledWith("StepDone"), "The step done event was triggred");
            // deleted
            assert.ok(ControlUnit.oComponentsLoading.myDuperStep1 === undefined, "Promise was erased.");
            assert.ok(ControlUnit.oComponentsLoading.myDuperStep2 === undefined, "Promise was erased.");
