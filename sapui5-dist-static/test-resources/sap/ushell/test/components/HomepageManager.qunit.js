...
sap.ui.require([
    ...
    /* global asyncTest, equal, module, ok, start, test, sinon, assert */
        ...
        if (expectedAdditionalProp && expectedAdditionalProp.length > 0) {
            for (i = 0; i < expectedAdditionalProp.length; i++) {
                if (expectedAdditionalProp[i] !== undefined) {
                    ok(oUsageAnalyticsLogStub.args[0][2][i] === expectedAdditionalProp[i],
                        eventName + " action should call logCustomEvent(UsageAnalytics) with event property: " + expectedAdditionalProp);
                }
            }
        }
    }
