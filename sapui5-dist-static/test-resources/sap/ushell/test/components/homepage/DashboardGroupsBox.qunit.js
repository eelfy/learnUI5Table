...
sap.ui.require([
    ...
    QUnit.module("Event delegates", {
        beforeEach: function () {
            sap.ushell.Container = {
                getService: sandbox.stub().returns({
            ...
            this.oHandleSkipStub = sandbox.stub(this.oDashboardGroupsBox, "_handleSkip");
            this.oHandleBeforeFastNavigationFocusStub = sandbox.stub(this.oDashboardGroupsBox, "_handleBeforeFastNavigationFocus");
        },
        afterEach: function () {
            sandbox.restore();
    ...
    QUnit.test("onsapskipback", function (assert) {
        ...
        var fnDelegate = this.oAddEventDelegateStub.getCall(0).args[0].onsapskipback;
        // Act
        fnDelegate();
        // Assert
        assert.strictEqual(this.oHandleSkipStub.callCount, 1, "_handleSkip was called once");
        assert.deepEqual(this.oHandleSkipStub.getCall(0).args, [false], "_handleSkip was called with correct parameters");
    });

    QUnit.test("onsapskipforward", function (assert) {
        // Arrange
        this.oDashboardGroupsBox.createGroupsBox(this.oControllerMock, this.oModelMock);
        var fnDelegate = this.oAddEventDelegateStub.getCall(0).args[0].onsapskipforward;
        // Act
        fnDelegate();
        // Assert
        assert.strictEqual(this.oHandleSkipStub.callCount, 1, "_handleSkip was called once");
        assert.deepEqual(this.oHandleSkipStub.getCall(0).args, [true], "_handleSkip was called with correct parameters");
    });
    ...
    QUnit.module("The method _handleSkip", {
        beforeEach: function () {
            sap.ushell.Container = {
                getService: sandbox.stub().returns({
                    isLinkPersonalizationSupported: sandbox.stub().returns(true)
            ...
            this.oGoToTileContainerStub = sandbox.stub();
            sap.ushell.components.ComponentKeysHandler = {
                goToTileContainer: this.oGoToTileContainerStub
            };
            ...
            var oFixture = document.getElementById("qunit-fixture");
            this.oButton = document.createElement("button");
            oFixture.appendChild(this.oButton);
            this.oButton.id = "sapUshellDashboardFooterDoneBtn";
        ...
        afterEach: function () {
            ...
            this.oDashboardGroupsBox.destroy();
            delete sap.ushell.Container;
            delete sap.ushell.components.ComponentKeysHandler;
        }
    });

    QUnit.test("Tilecontainer gets focused when done button is visible and focused and navigation is backwards", function (assert) {
        // Arrange
        this.oButton.focus();
        // Act
        this.oDashboardGroupsBox._handleSkip(false, this.oEventMock);
        // Assert
        assert.strictEqual(this.oPreventDefaultStub.callCount, 1, "preventDefault was called once");
        assert.strictEqual(this.oGoToTileContainerStub.callCount, 1, "goToTileContainerStub was called once");
    });

    QUnit.test("Done button gets focused when done button is visible and not focused and navigation is forwards", function (assert) {
        // Arrange
        // Act
        this.oDashboardGroupsBox._handleSkip(true, this.oEventMock);
        // Assert
        assert.strictEqual(this.oPreventDefaultStub.callCount, 1, "preventDefault was called once");
        assert.strictEqual(document.activeElement, this.oButton, "done button was focused");
    });

    QUnit.test("Event stays unhandled when done button is visible and focused and navigation is forwards", function (assert) {
        // Arrange
        this.oButton.focus();
        // Act
        this.oDashboardGroupsBox._handleSkip(true, this.oEventMock);
    ...
    QUnit.test("Event stays unhandled when done button is visible and not focused and navigation is backwards", function (assert) {
        // Arrange
        // Act
        this.oDashboardGroupsBox._handleSkip(false, this.oEventMock);
        // Assert
    ...
    QUnit.test("Event stays unhandled when done button is not visible", function (assert) {
        // Arrange
        this.oButton.remove();
        // Act
        this.oDashboardGroupsBox._handleSkip(true, this.oEventMock);
    ...
    QUnit.test("Done button gets focused when button is visible and navigation backwards", function (assert) {
        // Arrange
        // Act
        this.oDashboardGroupsBox._handleBeforeFastNavigationFocus(this.oEventMock);
        // Assert
        assert.strictEqual(this.oPreventDefaultStub.callCount, 1, "preventDefault was called once");
        assert.strictEqual(document.activeElement, this.oButton, "done button was focused");
    });

    QUnit.test("Tilecontainer gets focused when button is visible and navigation forwards", function (assert) {
        // Arrange
        this.oEventMock.forward = true;
        // Act
        this.oDashboardGroupsBox._handleBeforeFastNavigationFocus(this.oEventMock);
        // Assert
        assert.strictEqual(this.oPreventDefaultStub.callCount, 1, "preventDefault was called once");
        assert.strictEqual(this.oGoToTileContainerStub.callCount, 1, "goToTileContainer was called once");
    });
