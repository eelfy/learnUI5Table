...
sap.ui.require([
    ...
    "sap/ui/util/Mobile"
], function (AppLifeCycle, BlueBoxHandler, Application, Storage, utils, Container, Config, ApplicationContainerDummy, HeaderManager, Mobile) {
    "use strict";
    /* global equal deepEqual module ok test QUnit sinon asyncTest start assert */
    ...
    test("test _getDefaultFavIcon without a value in the parameter", function () {
        ...
        sResult = oAppMeta._getDefaultFavIcon(oParameters);
        equal(sResult, "../../../../../../resources/sap/ushell/themes/base/img/launchpad_favicon.ico");
        getParameterStub.restore();
    ...
    fnRequireAsyncToSync = function (aReq, fnCallBack) {
        var index = 0, sReq, aReqObjs = [], serIndex;
    ...
    test("test _getDefaultFavIcon with a value in the parameter", function () {
        ...
        sResult = AppLifeCycle.getAppMeta()._getDefaultFavIcon(oParameters);
        equal(sResult, "../../someurl.jpeg");
        getParameterStub.restore();
        sap.ui.require = oOriginalRequire;
    });
...
//            ok(stbAppDestroy.args[0][0] == "testid1", "evicted one application");
    ...
    test("check closeKeepAliveRestrictedApps", function () {
        Storage.init(10);
        Storage.set("A", {
            appId: "A",
            container: {getId : sinon.stub().returns("A")},
        ...
        Storage.set("D", {
            appId: "D",
            container: {getId : sinon.stub().returns("D")},
            keepAliveMode: "false"
        });
        Storage.set("E", {
            appId: "E",
            container: {getId : sinon.stub().returns("E")},
            keepAliveMode: "true"
        });
        Storage.set("F", {
            appId: "F",
        ...
        Storage.set("G", {
            appId: "G",
            container: {getId : sinon.stub().returns("G")},
            keepAliveMode: "restricted"
        });

        AppLifeCycle.destroy = sinon.stub(AppLifeCycle, "destroy",
            function (sAppId, oFrom, oDeferred, bForceDestroy) {
                Storage.remove(sAppId);
            });

        var oSpyStorareRemove = sinon.spy(Storage, "remove");

        AppLifeCycle.closeKeepAliveRestrictedApps();

        equal(oSpyStorareRemove.callCount, 4);
        equal(Storage.length(), 3);

        oSpyStorareRemove.restore();
        AppLifeCycle.destroy.restore();
    });
