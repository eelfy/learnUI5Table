...
sap.ui.require([
    ...
    /* global sinon, ok, strictEqual, module, QUnit, test, asyncTest, start,
        throws, deepEqual, equal, some */
     ...
     */
    function renderInternallyAndExpectIFrame(oContainer, sApplicationType, sUrl, oMockConfigOverrides, bIframeWithPost, bAllowPolicyRendered) {
        // Arrange
        ...
        // Assert
        testIFrameRendered(oTargetNode, oPrepareEnv, oContainer, sApplicationType, sUrl, bIframeWithPost, bAllowPolicyRendered);
     ...
     * @param {boolean} bIframeWithPost the ifarme was rendered with post method and not get
     */
    function testIFrameRendered(oTargetNode, oPrepareEnv, oContainer, sApplicationType, sUrl, bIframeWithPost, bAllowPolicyRendered) {
        var oIframe;
        ...
        if (bAllowPolicyRendered === true) {
            strictEqual(oIframe.getAttribute("allow"), sIFRAME_FEATURE_POLICY, "iframe 'allow' is set correctly");
        } else {
            strictEqual(oIframe.getAttribute("allow"), null, "iframe 'allow' should not exist");
        }
    ...
    [{
        sDesc: "ApplicationContainer#setNewTRApplicationContext - simple URL",
        input: {
            sInputUrl: "scheme://host:port/resource",
            bPost: false
        },
        output: {
            body: {
                url: "scheme://host:port/resource"
            },
            bGetAppStateDataCalled: false
        }
    }, {
        sDesc: "ApplicationContainer#setNewTRApplicationContext - url with app states",
        input: {
            sInputUrl: "scheme://host:port/resource?p1=1&sap-iapp-state=100&p2=2&sap-xapp-state=101&sap-intent-param=102",
            bPost: true,
        ...
        output: {
            body: {
                ...
                url: "scheme://host:port/resource?p1=1&sap-iapp-state=100&p2=2&sap-xapp-state=101&sap-intent-param=102"
            },
            bGetAppStateDataCalled: true,
            aAppStatesKeys: [[["102"], ["101"], ["100"]]]
        }
    }, {
        sDesc: "ApplicationContainer#setNewTRApplicationContext - post, url without app states",
        input: {
            sInputUrl: "scheme://host:port/resource?p1=1&p2=2",
            bPost: true
        },
        output: {
            body: {
                ...
                url: "scheme://host:port/resource?p1=1&p2=2"
            },
            bGetAppStateDataCalled: false
        }
    }].forEach(function (oFixture) {
        QUnit.asyncTest(oFixture.sDesc, function (assert) {
            sap.ushell.bootstrap("local").done(function () {
                // Arrange
                sinon.stub(sap.ushell.Container, "getService").returns({
                    getAppStateData: sinon.stub().returns(new jQuery.Deferred().resolve(oFixture.input.aAppStatesData).promise())
                });
                sinon.stub(sap.ushell.Container, "getFLPUrl").returns("http://www.test.com#A-B");
                ...
                var oAppContainer = {
                    ...
                    getDomRef: sinon.stub().returns({
                        ...
                        contentWindow: oContentWindow,
                        src: new URI(),
                        getAttribute: sinon.stub().returns("id-dummy")
                    }),
                    _getIFrame: sap.ushell.components.container.ApplicationContainer.prototype._getIFrame,
                    _getIFrameUrl: sap.ushell.components.container.ApplicationContainer.prototype._getIFrameUrl,
                    getIframeWithPost: sinon.stub().returns(oFixture.input.bPost),
                    postMessageToIframe: sap.ushell.components.container.ApplicationContainer.prototype.postMessageToIframe,
                    createPostMessageRequest: sap.ushell.components.container.ApplicationContainer.prototype.createPostMessageRequest,
                    getSystemAlias: sinon.stub().returns(oFixture.input.systemAlias)
                };
                ...
                // Act
                setNewTRApplicationContext(oFixture.input.sInputUrl)
                    .then(function () {
                        var oMessage = JSON.parse(oContentWindow.postMessage.args[0][0]);
                        // Assert
                        assert.equal(
                            oAppContainer.getDomRef.calledOnce,
                            true,
                        ...
                        if (oFixture.output.bGetAppStateDataCalled === true) {
                            assert.ok(sap.ushell.Container.getService().getAppStateData.calledOnce);
                            assert.deepEqual(
                                sap.ushell.Container.getService().getAppStateData.args[0],
                                oFixture.output.aAppStatesKeys,
                                "proper arguments");
                        } else {
                            assert.ok(sap.ushell.Container.getService().getAppStateData.notCalled);
                        }
    ...
    [{
        ...
        inputUrl: "http://example.org/sap/bc/ui2/nwbc/~canvas", // NOTE: not the browser location URL!
        ...
        inputUrl: "http://example.org/sap/bc/ui2/nwbc/~canvas",
        ...
        inputUrl: "http://example.org/sap/bc/ui2/nwbc/~canvas",
        expectedUrlAddition: "sap-ie=edge&sap-ushell-timeout=0&sap-shell=FLP1.32.5",
        version: "1.32.5.270343434",
        nwbcTheme: undefined,
        accessibility: undefined,
        applicationType: "TR"
        ...
        inputUrl: "http://example.org/sap/bc/ui2/nwbc/~canvas",
        expectedUrlAddition: "sap-ie=edge&sap-ushell-timeout=0&sap-shell=FLP1.39.0",
        ...
        inputUrl: "http://example.org/sap/bc/ui2/nwbc/~canvas?sap-statistics=something",
        sapStatisticsUI5ConfigBoolean: true,
        expectedUrlAddition: "sap-ie=edge&sap-ushell-timeout=0",
        nwbcTheme: undefined,
        accessibility: undefined,
        ...
        inputUrl: "http://example.org/sap/bc/ui2/nwbc/~canvas?sap-statistics=false",
        sapStatisticsUI5ConfigBoolean: true,
        expectedUrlAddition: "sap-ie=edge&sap-ushell-timeout=0",
        nwbcTheme: undefined,
        accessibility: undefined,
        ...
        sessionTimeoutFunc: function () {
            return sinon.stub(Config, "last").returns(-1);
        }
    }, {
        ...
        applicationType: "NWBC",
        sessionTimeoutFunc: function () {
            return sinon.stub(Config, "last").returns(null);
        }
    }, {
        ...
        applicationType: "NWBC",
        sessionTimeoutFunc: function () {
            return sinon.stub(Config, "last").returns("");
        }
    }, {
        ...
        applicationType: "NWBC",
        sessionTimeoutFunc: function () {
            return sinon.stub(Config, "last").returns(undefined);
        }
    }, {
        ...
        applicationType: "NWBC",
        sessionTimeoutFunc: function () {
            return sinon.stub(Config, "last").returns(10);
        }
    }, {
        ...
        applicationType: "NWBC",
        sessionTimeoutFunc: function () {
            return sinon.stub(Config, "last").returns(0);
        }
    }].forEach(function (oFixture) {
    ...
    test("sap.ushell.components.container.render URL with 'allow' policy", function () {
        Application.destroy(oAppContainer);
        oAppContainer = new sap.ushell.components.container.ApplicationContainer();
        oAppContainer.setWidth("10%");
        oAppContainer.setHeight("20%");
        sap.ushell.Container = mockSapUshellContainer();
        var oStubLast = sinon.stub(Config, "last").returns(true);
        renderInternallyAndExpectIFrame(oAppContainer, ApplicationType.URL.type, sTESTURL, undefined, undefined, true);
        oStubLast.restore();
    });
    ...
    test("sap.ushell.components.container.render WDA", function () {
        ...
        renderInternallyAndExpectIFrame(oAppContainer, ApplicationType.WDA.type,
            "http://example.org/sap/bc/webdynpro/sap/test_navigation_parameter");
    });
        ...
        asyncTest("sap.ushell.components.container.render " + sLegacyApplicationType, function () {
            ...
            sap.ushell.bootstrap("local").done(function () {
                ...
                renderInternallyAndExpectIFrame(oAppContainer, ApplicationType.enum[sLegacyApplicationType],
                    "http://example.org/sap/bc/ui2/nwbc/~canvas");
            });
        });
    });
    ...
    asyncTest("sap.ushell.components.container.render UI5 (SAPUI5=)", function () {
        var oRenderManager = new sap.ui.core.RenderManager().getInterface(),
            oIcon = new sap.ui.core.Icon(),
            bRenderControlCalled1,
            bRenderControlCalled2,
            oDummyRenderer = {
                renderControl: sinon.stub(),
                flush: sinon.stub(),
                destroy: sinon.stub()
            };
        ...
        sinon.stub(sap.ushell.components.container.ApplicationContainer.prototype, "_createUi5View").returns(
        ...
        sinon.stub(sap.ui.getCore(), "createRenderManager").returns(oDummyRenderer);
        sinon.stub(oRenderManager, "renderControl");
        ...
        oAppContainer.getDeffedControlCreation().then(function () {
            start();
            ...
            ok(oAppContainer._renderControlInDiv.calledWith(oRenderManager, oAppContainer));
            bRenderControlCalled1 = sap.ui.getCore().createRenderManager().renderControl.calledWith(oIcon);
            bRenderControlCalled2 = oRenderManager.renderControl.calledWith(oIcon);
            ok((bRenderControlCalled1 && !bRenderControlCalled2) || (!bRenderControlCalled1 && bRenderControlCalled2));
            sap.ui.getCore().createRenderManager.restore();
        });
    });
    ...
    asyncTest("createUi5View", function () {
        Application.destroy(oAppContainer);
        oAppContainer = new sap.ushell.components.container.ApplicationContainer();
        ...
        var oIcon = new sap.ui.core.Icon(),
            ...
            // return a button instead of a view, so that we have a control with the necessary
            // properties, but don't have to supply another file for the view definition
            fnCreateView = sinon.stub(sap.ui, "view").returns({
                loaded: sinon.stub().returns(Promise.resolve(oIcon))
            }),
        ...
        oAppContainer._createUi5View(oAppContainer, oAppContainer.getUrl(), oAppContainer.getAdditionalInformation()).then(function (oView) {
            start();
            ...
            strictEqual(fnRegisterModulePath.firstCall.args[1],
                "http://example.org/sap/public/bc/ui2/staging/test/some/random/view/path");
    ...
    test("createUi5View view in subfolder", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/");
        ...
        var fnRegisterModulePath = sinon.spy(jQuery.sap, "registerModulePath"),
            fnCreateView = sinon.stub(sap.ui, "view").returns({
                loaded: sinon.stub().returns(Promise.resolve(new sap.ui.core.Icon()))
            }),
            fnAssert = sinon.spy(jQuery.sap, "assert");
        ...
        strictEqual(fnRegisterModulePath.firstCall.args[1], "http://example.org/some/random/path");
        ok(fnCreateView.calledOnce, "createView called");
    ...
    asyncTest("createUi5View with configuration data", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test");
        // explicitely use ".view." in the view's path to check that this is no problem
        oAppContainer.setAdditionalInformation("SAPUI5=some.random.view.path.Viewname.view.xml");
    ...
    asyncTest("createUi5View with view data", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test?foo=bar&foo=baz&bar=baz");
        // explicitely use ".view." in the view's path to check that this is no problem
        oAppContainer.setAdditionalInformation("SAPUI5=some.random.view.path.Viewname.view.xml");
    ...
    asyncTest("createUi5View: invalid view type", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test");
        ...
        oAppContainer._createUi5View(oAppContainer, oAppContainer.getUrl(), oAppContainer.getAdditionalInformation()).catch(function () {
            start();
    ...
    function createComponentViaSapui5(sQueryString, oExpectedComponentData) {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test" + sQueryString);
        ...
        oAppContainer._createUi5View(oAppContainer, oAppContainer.getUrl(), oAppContainer.getAdditionalInformation()).then(function (oControl) {
            start();
            ...
            ok(jQuery.sap.registerModulePath.calledWithExactly("some.random.path",
                "http://example.org/sap/public/bc/ui2/staging/test/some/random/path"),
    ...
    asyncTest("createUi5View: component w/o componentData", function () {
        createComponentViaSapui5("", { startupParameters: {} });
    });

    asyncTest("createUi5View: component w/ componentData", function () {
    ...
    asyncTest("createUi5View: with application config and without view name", function () {
        Application.destroy(oAppContainer);
        oAppContainer = new sap.ushell.components.container.ApplicationContainer();
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        ...
        ok(oResult.oControl === undefined);
        ok(oResult.oPromise !== undefined);
    ...
    asyncTest("sap.ushell.components.container.render UI5 (SAPUI5.component=)", function () {
        Application.destroy(oAppContainer);
        oAppContainer = new sap.ushell.components.container.ApplicationContainer();
        ...
        oAppContainer.getDeffedControlCreation().then(function () {
            ...
            ok(oRenderControlInDivStub.calledWith(oRenderManager, oAppContainer));
            start();
        });
    });

    asyncTest("rerender without property change does not recreate component", function () {
        Application.destroy(oAppContainer);
        oAppContainer = new sap.ushell.components.container.ApplicationContainer();

        var oControl = new sap.ui.core.Icon(), // any control with width and height is sufficient
            oRenderManager = new sap.ui.core.RenderManager(),
            oCreateUi5ComponentStub = sinon.stub(sap.ushell.components.container.ApplicationContainer.prototype,
    ...
    [{
        ...
        asyncTest("rerender with changed " + oFixture.sProperty + " does recreate component", function () {
            Application.destroy(oAppContainer);
            oAppContainer = new sap.ushell.components.container.ApplicationContainer();
    ...
    asyncTest("disable router : invoked stop if getRouter returns something present", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test?foo=bar&foo=baz&sap-xapp-state=1234242&bar=baz");
        oAppContainer.setAdditionalInformation("SAPUI5.Component=some.random.path");
        oAppContainer.setApplicationConfiguration({ "a": 1, "b": 2 });
    ...
    asyncTest("disable router : there is an evenhandler registered which effectively disables the router when the event is fired", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test?foo=bar&foo=baz&sap-xapp-state=1234242&bar=baz");
        oAppContainer.setAdditionalInformation("SAPUI5.Component=some.random.path");
        oAppContainer.setApplicationConfiguration({ "a": 1, "b": 2 });

        var spySubscribe = sinon.spy(sap.ui.getCore().getEventBus(), "subscribe");
    ...
    asyncTest("createUi5Component with configuration data", function () {
        oAppContainer.setApplicationType(ApplicationType.URL.type);
        oAppContainer.setUrl("http://example.org/sap/public/bc/ui2/staging/test?foo=bar&foo=baz&sap-xapp-state=1234242&bar=baz");
        oAppContainer.setAdditionalInformation("SAPUI5.Component=some.random.path");
        oAppContainer.setApplicationConfiguration({ "a": 1, "b": 2 });
    ...
    [
        ...
        asyncTest("application configuration: " + JSON.stringify(oFixture), function () {
            ...
            sinon.stub(sap.ui, "view").returns({
                loaded: sinon.stub().returns(Promise.resolve(oIcon))
            });
            ...
            oAppContainer._render(oRenderManager, oAppContainer, oFixture.type || ApplicationType.URL.type,
                "http://example.org/ushell/test-resources/sap/ushell/test/components/container", oFixture.additionalInfo);
        });
    });
    ...
    test("createSystemForUrl", function () {
        ...
        check("http://example.org/sap/bc/ui2/wda/~canvas?foo=bar", {
            alias: "http://example.org",
            baseUrl: "http://example.org"
        });
        check("http://example.org/sap/bc/ui2/wda/~canvas?foo=bar&sap-client=120", {
            alias: "http://example.org?sap-client=120",
            baseUrl: "http://example.org",
            client: "120"
        });
    });
    ...
    asyncTest("ApplicationContainer logout de-/registration", function () {
        sap.ushell.bootstrap("local").done(function () {
            ...
            renderInternallyAndExpectIFrame(oAppContainer, ApplicationType.WDA.type,
                "http://example.org/sap/bc/ui2/wda/~canvas", oMockOverrides);
            ...
            renderInternallyAndExpectIFrame(oAppContainer, ApplicationType.NWBC.type,
                "http://example.org/sap/bc/ui2/NWBC/~canvas?sap-client=120", oMockOverrides);
            ...
            ok(oAppContainer._createSystemForUrl
                .calledWith("http://example.org/sap/bc/ui2/NWBC/~canvas?sap-client=120"),
            ...
            ["NWBC", "TR"].forEach(function (sNwbcLikeApplicationType) {
                renderInternallyAndExpectIFrame(oAppContainer, ApplicationType.enum[sNwbcLikeApplicationType],
                    "http://example.org/sap/bc/ui2/" + sNwbcLikeApplicationType + "/~canvas", oMockOverrides);
                fnLogout = sap.ushell.Container.attachLogoutEvent.args[0][0];
        ...
        asyncTest("ApplicationContainer " + sNwbcLikeApplicationType + " Logoff fired", function () {
            sap.ushell.bootstrap("local").done(function () {
                ...
                var oEvent = new sap.ui.base.Event(),
                    fnGetAttribute = sinon.stub().returns("http://example.org/sap/bc/ui2/nwbc/~canvas"),
        ...
        asyncTest("ApplicationContainer " + sNwbcLikeApplicationType + " Logoff 2 Instances", function () {
            ...
            sap.ushell.bootstrap("local").done(function () {
                ...
                renderInternallyAndExpectIFrame(oContainer1, ApplicationType.enum[sNwbcLikeApplicationType],
                    "http://example.org/sap/bc/ui2/nwbc/~canvas", oMockOverrides);
    ...
    module("components/container/ApplicationContainer.js", {
        setup: function () {
            ...
            sap.ushell.components.container.ApplicationContainer.prototype._resetPluginsLoadIndications();
            sinon.stub(Storage.prototype, "removeItem");
            sinon.stub(Storage.prototype, "setItem");
            sinon.stub(sap.ui, "getVersionInfo").returns({ version: undefined });
    ...
    [{
        ...
        expectedMessageHandled: false,
        expectedLogCall: [
            "Skipping handling of postMessage 'message' event with data 'undefined' on inactive container 'SOME_CONTAINER_ID'",
    ...
    test("handleServiceMessageEvent service definition", function () {
        ...
        oLogMock.verify();
        ok(oMessageStatus.bApiRegistered, "Api was found");
        ...
        ok(!Log.warning.called, "No warning logged");
        ok(!oMessageStatus.bApiRegistered, "Api was not found");
    });
        ...
        test("ApplicationContainer localStorage eventing", function (assert) {
            ...
            sap.ushell.bootstrap("local").done(function () {
                ...
                renderInternallyAndExpectIFrame(oAppContainer, oAppContainer.getApplicationType(),
                    "http://example.org/sap/bc/ui2/" + sNwbcLikeApplicationType + "/~canvas?sap-client=120", oMockOverrides);
    ...
    [false, true].forEach(function (bIsPostMethod) {
        test("ApplicationContainer rendering on inactive container", function () {
            // Arrange
            var sUrl = "http://example.org/sap/public/bc/ui2/staging/test";
    ...
    [{
        ...
        appType: "NWBC",
        URL: "http://example.org/sap/public/bc/ui2/staging/test",
        expectedParameters: {
            totalNumber: 1,
            values: [
                {
        ...
        appType: "TR",
        sapPostParams: true,
        URL: "http://example.org/sap/public/bc/ui2/staging/test?sap-additional-param1=111&sap-additional-param2=222",
        expectedParameters: {
            totalNumber: 6,
            URL: "http://example.org/sap/public/bc/ui2/staging/test",
            values: [
        ...
        sapPostParams: true,
        isCFLP: true,
        URL: "http://example.org/sap/public/bc/ui2/staging/test?sap-iframe-hint=GUI&sap-additional-param1=111&sap-additional-param2=222",
        expectedParameters: {
            totalNumber: 6,
            URL: "http://example.org/sap/public/bc/ui2/staging/test?sap-iframe-hint=GUI",
            values: [
                {
                    name: "sap-flp-params",
                    value: "{\"sap-flp-url\":\"" + window.location.href + "\"}"
        ...
        appType: "NWBC",
        URL: "http://example.org/sap/public/bc/ui2/staging/test?sap-intent-param=V1",
        appStateData: [
        ...
        appType: "NWBC",
        URL: "http://example.org/sap/public/bc/ui2/staging/test?sap-intent-param=V1&sap-xapp-state=V2&sap-iapp-state=V3",
        appStateData: [
            ["sap-intent-param-data-1234"],
        ...
        expectedParameters: {
            ...
            values: [
                {
                    name: "sap-flp-params",
                    value: "{\"sap-intent-param-data\":\"sap-intent-param-data-1234\",\"sap-xapp-state-data\":\"sap-xapp-state-data-5678\",\"sap-iapp-state-data\":\"sap-iapp-state-data-9012\",\"sap-flp-url\":\"" + window.location.href + "\"}"
                }]
        }
    }, {
        id: "app-container-test-7",
        desc: "NWBC, URL contains sap-intent-param and another simple params that should be ignored",
        appType: "NWBC",
        URL: "http://example.org/sap/public/bc/ui2/staging/test?param1=1&sap-intent-param=V1&param2=2",
        appStateData: [
            ["sap-intent-param-data-1234"]
        ],
        expectedParameters: {
        ...
        asyncTest("ApplicationContainer rendering and IFrame with form post - " + oFixture.desc, function () {
            sap.ushell.bootstrap("local").done(function () {
                ...
                if (oFixture.isCFLP === true) {
                    oAppContainer.setIsPostCapability(true);
                }

                if (oFixture.sapPostParams === true) {
                    oAppContainer.setIframePostAllParams(true);
                }
    ...
    function checkPostFormNode(oTargetNode, oAppContainer, oFixture) {
        ...
        if (utils.isApplicationTypeEmbeddedInIframe(oFixture.appType)) {
            if (oFixture.appType === "TR" && oFixture.sapPostParams === true ) {
                strictEqual(oForm.action, oFixture.expectedParameters.URL);
            } else {
    ...
    [{
        ...
        asyncTest("ApplicationContainer - rendering cFLP application " + oFixture.description, function () {
            sap.ushell.bootstrap("local").done(function () {
                var sUrl = "http://example.org/sap/public/bc/ui2/staging/test";
                oAppContainer.setIframeWithPost(oFixture.setPostInURL);
                oAppContainer.setApplicationType(ApplicationType.URL.type);
                oAppContainer.setIsPostCapability(oFixture.setPOSTInURLTemplate);
                oAppContainer.setUrl(sUrl);

                var oRenderManager = new sap.ui.core.RenderManager();

                sinon.spy(oAppContainer.__proto__, "_filterURLParams");
                sinon.spy(oAppContainer.__proto__, "_getParamKeys");
                sinon.spy(oAppContainer.__proto__, "_buildHTMLElements");

                oRenderManager.render(oAppContainer, document.createElement("DIV"));

                ok(oAppContainer.__proto__._filterURLParams.callCount === oFixture.expected, "container _filterURLParams method was called as expected");
                ok(oAppContainer.__proto__._getParamKeys.callCount === oFixture.expected, "container _getParamKeys method was called as expected");
                ok(oAppContainer.__proto__._buildHTMLElements.callCount === oFixture.expected, "container _buildHTMLElements method was called as expected");

                oAppContainer.__proto__._filterURLParams.restore();
                oAppContainer.__proto__._getParamKeys.restore();
                oAppContainer.__proto__._buildHTMLElements.restore();
                start();
            });
        });
    });
    ...
    asyncTest("Post messages received after plugins initiated", function () {
        sap.ushell.bootstrap("local").done(function () {
            var oDeferred = new jQuery.Deferred(),
                oPromise = oDeferred.promise(),
                oPluginsStub = sinon.stub(sap.ushell.Container.getService("PluginManager"), "getPluginLoadingPromise").returns(oPromise),
                oHandleMessageStub = sinon.stub(sap.ushell.components.container.ApplicationContainer.prototype, "_handleMessageEvent");

            oDeferred.resolve();

            sap.ushell.components.container.ApplicationContainer.prototype._handlePostMessagesForPluginsPostLoading({}, {}, {
            ...
            oPromise.done(function() {
                start();
                ok(!oHandleMessageStub.called, "messages should not be processed");
                oPluginsStub.restore();
                oHandleMessageStub.restore();
            });
        });
    });

    asyncTest("Post messages received before plugins initiated", function () {
        sap.ushell.bootstrap("local").done(function () {
            var oDeferred = new jQuery.Deferred(),
                oPromise = oDeferred.promise(),
                oPluginsStub = sinon.stub(sap.ushell.Container.getService("PluginManager"), "getPluginLoadingPromise").returns(oPromise),
            ...
            oPromise.done(function() {
                start();
                ok(oHandleMessageStub.calledTwice, "messages should not be processed");
                oPluginsStub.restore();
                oHandleMessageStub.restore();
            });
        });
