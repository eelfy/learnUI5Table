...
sap.ui.require([
    ...
    [
        ...
        QUnit.test("update states: " + name, function (assert) {
            ...
            Object.keys(oTestCase.oExpectedBaseState).forEach(function (sStateName) {
                assert.deepEqual(HeaderManager._getBaseState(sStateName), oTestCase.oExpectedBaseState[sStateName],
                    "Base state: " + sStateName + " is correct");
            });
            oByIdStub.restore();
            assert.ok(Config.last("/core/shellHeader")[oTestCase.oEvent.propertyName].indexOf(oTestCase.oEvent.value[0]) > -1,
                "The current state was updated");
        });
    });
    ...
    QUnit.test("test_movingOfFLpActionsToShellHeaderInModel", function (assert) {
        ...
        ["home", "app", "minimal", "standalone", "embedded", "embedded-home", "lean"].forEach(function (sState) {
            ...
            assert.ok(aHeadEndItems.indexOf("ContactSupportBtn") !== -1 && aHeadEndItems.indexOf("EndUserFeedbackBtn") !== -1,
    ...
    QUnit.test("update centralAreaElement property for all states", function (assert) {
        ...
        setTimeout(function () {
            ["home", "app", "minimal", "standalone", "embedded", "embedded-home", "lean"].forEach(function (sState) {
                assert.equal(HeaderManager._getBaseStateMember(sState, "centralAreaElement"), oEventPayload.id,
                    "centralAreaElement set correctly for state: " + sState);
            });
    ...
    QUnit.test("update centralAreaElement property only for home state", function (assert) {
        ...
        EventHub.emit("setHeaderCentralAreaElement", oEventPayload);
        setTimeout(function () {
            assert.equal(HeaderManager._getBaseStateMember("home", "centralAreaElement"), oEventPayload.id,
                "centralAreaElement set correctly for \"home\" state");
            ["app", "minimal", "standalone", "embedded", "embedded-home", "lean"].forEach(function (sState) {
                assert.equal(HeaderManager._getBaseStateMember(sState, "centralAreaElement"), null,
                    "centralAreaElement should not be set for state: " + sState);
            });
            HeaderManager.destroy();
            fnDone();
        }, 10);
        ...
        [
            ...
            var aResult = oFixture.aEndItems,
                ...
                fnGetCoreStub = sinon.stub(sap.ui, "getCore").returns({
                    byId: function (sId) {
                        return sId === "endItemsOverflowBtn";
                    }
                }),
            ...
            // Assert
            assert.strictEqual(bEndItemsUpdated, oFixture.bExpectedEndItemsUpdated,
                "EndItems have (not) been updated when " + oFixture.sTestDesciption);
