...
sap.ui.require([
    ...
    QUnit.module("sap.ushell.renderers.fiori2.Renderer", {
        ...
        beforeEach: function () {
            ...
            EventHub.once("RendererLoaded").do(function () {
                this.oShellDummy = document.createElement("div");
                this.oShellLayout = sap.ushell.Container.getRenderer("fiori2").getShellController().getView().getOUnifiedShell();
                sinon.stub(this.oShellLayout, "getDomRef").returns(this.oShellDummy);
            }.bind(this));
        ...
        afterEach: function (assert) {
            ...
            sap.ushell.Container.getServiceAsync("ShellNavigation").then(function () {
                ...
                sinon.restore(this.oShellLayout, "getDomRef");
                delete this.oShellDummy;
   ...
   QUnit.test("enable search", function (assert) {
    ...
    QUnit.test("Check that appFinder redirect to home when appfinder is disabled", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            var oPromise = new Promise(function (resolve) {
                ...
                var oRouter = oRenderer.getRouter();
                fnNavToSpy = sinon.spy(oRouter, "navTo");
                var fnAttachRouteMatched = function (sRoute) {
                    var oRoute = oRouter.getRoute(sRoute);
                    oRoute.attachEventOnce("matched", resolve);
                };
            ...
            oPromise.then(function () {
                if (fnNavToSpy) {
                    assert.ok(fnNavToSpy.calledOnce, "navTo should be called");
    ...
    QUnit.test("Check that hash is changed to Shell-home when pages is disabled and user navigates to Launchpad-openFLPPage", function (assert) {
        var done = assert.async();
        var oReplaceHashWithoutNavigationStub;
        ...
        sap.ushell.bootstrap("local").done(function () {
            ...
            this.oRendererControl = sap.ushell.Container.createRenderer("fiori2");
            oReplaceHashWithoutNavigationStub = sinon.stub(sap.ushell.Container.getService("ShellNavigation"), "replaceHashWithoutNavigation");
        }.bind(this));
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            oPromise.then(function () {
                assert.strictEqual(oReplaceHashWithoutNavigationStub.callCount, 1, "replaceHashWithoutNavigation called once");
                assert.deepEqual(oReplaceHashWithoutNavigationStub.getCall(0).args, ["Shell-home"],
                    "replaceHashWithoutNavigation called with correct arguments");
                done();
            });
        });
    });
    ...
    QUnit.test("Check that state is set as \"app\", when root inten is not flp home", function (assert) {
        var done = assert.async(),
            sHash = "Shell-home",
            oSwitchViewStateSpy,
            fnErrorMessageSpy;
        ...
        var oConfig = _getContainerConfiguration.call(this);
        oConfig.renderers.fiori2.componentData.config.rootIntent = "Some-intent";
        window["sap-ushell-config"] = oConfig;
        ...
        sap.ushell.bootstrap("local").done(function () {
            ...
            this.oRendererControl = sap.ushell.Container.createRenderer("fiori2");
            var oShellView = sap.ui.getCore().byId("mainShell");
            oSwitchViewStateSpy = sinon.spy(AppLifeCycle, "switchViewState");
            fnErrorMessageSpy = sinon.spy(oShellView.getController(), "hashChangeFailure");
        }.bind(this));
    ...
    QUnit.test("Headerless application state & personalization enablement 3", function (assert) {
        ...
        window["sap-ushell-config"] = config;
        //Config instance is created once at the beggining of the module. Manual update the corresponding property
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            setTimeout(function () {
                assert.strictEqual(
                    Config.last("/core/shell/model/personalization"),
                    false,
                    "verify personalization is off due to non-headerless mode BUT configuration set personalization off"
                );
                done();
            }, 100);
        }.bind(this));
    ...
    QUnit.test("addHeader(End)Item API", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            sap.ushell.Container.getRenderer("fiori2").addHeaderItem(undefined, oHeaderItem, false, true);
            sap.ushell.Container.getRenderer("fiori2").addHeaderEndItem(undefined, oHeaderEndItem, false, true);
            checkItems();

            //check API without controlType
            sap.ushell.Container.getRenderer("fiori2").addHeaderItem(oHeaderItem, false, true);
            sap.ushell.Container.getRenderer("fiori2").addHeaderEndItem(oHeaderEndItem, false, true);
            checkItems();
    ...
    QUnit.test("Renderer API - setFooterControl", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            AppLifeCycle.switchViewState("home");
            this.oRenderer = sap.ushell.Container.getRenderer("fiori2");
            this.oRm.renderControl(this.oShellLayout);
            this.oRm.flush(this.oShellDummy);

            // Act
            oControl = this.oRenderer.setFooterControl("sap.m.Bar", {id: "testFooterId"});

            // Assert
            var oFooterContainer = this.oShellDummy.querySelector("#" + this.oShellLayout.getId() + "-footer");
            ...
            assert.ok(this.oShellDummy.classList.contains("sapUshellShellFooterVisible"), "The shell element had the correct class.");
            ...
            assert.notOk(this.oShellDummy.classList.contains("sapUshellShellFooterVisible"), "The sapUshellShellFooterVisible class was removed.");
            assert.strictEqual(oFooterContainer.childNodes.length, 0, "The footer control was removed.");
            assert.ok(oFooterContainer.classList.contains("sapUiHidden"), "The footer container was hidden.");
    ...
    QUnit.test("Renderer API - removeFooter", function (assert) {
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            assert.notOk(this.oShellDummy.classList.contains("sapUshellShellFooterVisible"), "The sapUshellShellFooterVisible class was removed.");
            assert.notOk(
                this.oRenderer.lastFooterId,
                "The footer control was destroyed before calling the removeFooter function, the function initialized the oRenderer.lastFooterId parameter without trying to destroy the footer control"
            );
    ...
    QUnit.test("_isBuiltInIntent - returns false if parameter is and object ", function (assert) {
        var done = assert.async();
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            // Act
            var bEmptyObject = this.oRenderer._isBuiltInIntent({});
            var bIntent = this.oRenderer._isBuiltInIntent("#bla-blup");

            // Assert
            assert.notOk(bEmptyObject, "If parameter is an empty object, return false");
    ...
    QUnit.test("_isBuiltInIntent - returns false if parameter is a string", function (assert) {
        var done = assert.async();
    ...
    QUnit.test("_isBuiltInIntent - constructs a concatenation of routes", function (assert) {
        var done = assert.async();
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            var oGetMetaDataStub = sinon.stub(Renderer, "getMetadata").returns({
                getRoutes: sinon.stub().returns(oFakeMetadata)
            });
            ...
            oGetMetaDataStub.restore();
            done();
        }.bind(this));
    });

    QUnit.test("_isBuiltInIntent - returns false if intent is not built-in", function (assert) {
        var done = assert.async();
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            // Act
            var bResponse = this.oRenderer._isBuiltInIntent({
                semanticObject: "Shell",
                action: "Home"
            });

            // Assert
            assert.notOk(bResponse, "Returns true for built-in intent");
    ...
    QUnit.test("_isBuiltInIntent - returns true if intent is built-in", function (assert) {
        var done = assert.async();
        var oFakeMetadata = [
            {
                name: "kartoffel",
        ...
        EventHub.once("RendererLoaded").do(function () {
            ...
            // Assert
            assert.ok(bResponse, "Returns true for built-in intent");
