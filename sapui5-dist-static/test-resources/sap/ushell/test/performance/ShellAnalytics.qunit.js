...
sap.ui.require([
    ...
    QUnit.test("Subscribe to EventHub when ShellAnalytics is enabled", function (assert) {
        ShellAnalytics.enable();
        assert.equal(this.oEventHubOnStub.callCount, 7, "ShellAnalytics listens 6 EventHub events");
    ...
    QUnit.test("All EventHub events should be off when disable the analytics", function (assert) {
        ShellAnalytics.enable();
        ShellAnalytics.disable();
        assert.equal(this.oDoableMock.off.callCount, 8, "All EventHub events should be off");
    });
    ...
    QUnit.test("Enable shall subscribe only once", function (assert) {
        ...
        assert.equal(this.oEventHubOnStub.callCount, 7, "ShellAnalytics should not subscribe to EventHub again");
    ...
    QUnit.test("Component id without comma is set as targetApplication, when getTechnicalParameter return no id for UI5 application", function (assert) {
        var fnDone = assert.async();
        ...
        fnEventHubInPromise("trackHashChange", "#Action-toApp")
            ...
            .then(function () {
                setTimeout(function () {
                    var aRecs = ShellAnalytics.getAllRecords();
                    assert.strictEqual(aRecs.length, 1, "shell analytics called for one application");
                    assert.strictEqual(aRecs[0].targetApplication, "a b c", "targetApplication was set correctly");
                    fnDone();
                }, 10);
            });
    });
    ...
    QUnit.test("When current application is not UI5 application, remove any comma signs from targetApplication", function (assert) {
        var fnDone = assert.async();
        ...
        var oMetadataStub = sinon.stub(AppConfiguration, "getMetadata").returns({
            technicalName: "SU01, GERMANY, (T-CODE)"
        });
        ...
        fnEventHubInPromise("trackHashChange", "#Action-toApp")
            ...
            .then(function () {
                setTimeout(function () {
                    var aRecs = ShellAnalytics.getAllRecords();
                    assert.strictEqual(aRecs.length, 1, "shell analytics called for one application");
                    assert.strictEqual(aRecs[0].targetApplication, "SU01 GERMANY (T-CODE)", "targetApplication was set correctly");
                    oMetadataStub.restore();
                    fnDone();
                }, 10);
            });
