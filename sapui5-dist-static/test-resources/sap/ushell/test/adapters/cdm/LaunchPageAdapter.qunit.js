...
sap.ui.require([
    ...
    function stubUsedServices (oSite, oServiceSpecifications) {
        ...
        // As sinon does not match sinon.match.falsy if the parameter does not exist in the arguments array, we have to
        // fake the function calls instead of simply using oGetServiceStub.withArgs("name", sinon.match.any, sinon.match.falsy) :(
        oGetServiceStub.withArgs("CommonDataModel").callsFake(function (serviceName, parameter, async) {
            if (async) {
        ...
        oGetServiceStub.withArgs("ClientSideTargetResolution").callsFake(function (serviceName, parameter, async) {
            if (async) {
                return Promise.resolve(oClientSideTargetResolutionService);
            }

            return oClientSideTargetResolutionService;
        });
    ...
    test("Confirm site with valid CDM version gets detected", function () {
        strictEqual(this.oAdapter.isSiteSupported({ _version: "3.1.0" }), true, "3.1.0 is supported");
    ...
    test("Confirm max and min CDM versions supported by the adapter", function () {
        ...
        strictEqual(this.oAdapter.getCdmVersionsSupported().max.toString(), "3.1.0", "3.1.0 is the maximum version supported by the adapter");
    });
    ...
    asyncTest("_ensureLoaded, success: bundle multiple parallel requests", function () {
        ...
        sinon.stub(sap.ushell.Container, "getService").callsFake(function (sServiceName) {
        ...
        sinon.stub(that.oAdapter, "_ensureGroupItemsResolved").callsFake(function () {
    ...
    [
        ...
        asyncTest("_ensureGroupItemsResolved: works as expected when " + oFixture.testDescription, function () {
            ...
            strictEqual(Array.isArray(aPromisesGot), true,
                "returned an array");
    ...
    [
        {
            ...
            oTile: { id: "tileId", vizId: "vizId", vizType: "sap.ushell.StaticAppLauncher", vizConfig: { "sap.flp": { target: { appId: "appId" } } } },
            oSite: {
                visualizations: {},
                vizTypes: { "sap.ushell.StaticAppLauncher": {} },
                applications: {
            ...
            oTile: { id: "tileId", vizId: "vizId", vizType: "sap.ushell.StaticAppLauncher", vizConfig: { "sap.flp": { target: { url: "www.sap.com", type: "URL" } } } },
            oSite: {
                visualizations: {},
                vizTypes: { "sap.ushell.StaticAppLauncher": {} },
                applications: {
    ...
    asyncTest("_getTileFromHash: Rejects as the resolution of a tile fails", function () {
        var that = this,
            sHash = "#Hash-irrelevant",
            oCstrStub = sinon.stub(sap.ushell.Container, "getService").callsFake(function () {
                return {
    ...
    asyncTest("getGroups: Resolves to an empty array as _ensureLoaded failed", function () {
        ...
        sinon.stub(that.oAdapter, "_ensureLoaded").callsFake(function () {
            var oDeferred = new jQuery.Deferred();
    ...
    asyncTest("_ensureLoaded: One of the loaded groups is defined as default group", function () {
        var that = this,
            fnGetService = sap.ushell.Container._getService;
    ...
    [
        ...
        asyncTest("getTileView for tiles: " + oFixture.testDescription, function () {
            ...
            sinon.stub(oEventHub, "once").callsFake(function () {
                return {
    ...
    asyncTest("getTileView catches for custom tile throwing an exception", function () {
        ...
        oAdapter.getTileView(oInputTile)
            ...
            .fail(function (sErrorMessage) {
                ...
                strictEqual(Log.error.callCount, 1, "Log.error was called");

                ok((new RegExp(sOriginalError)).test(Log.error.getCall(0).args[0]), "Rejects its promise when an error is thrown by sap.ui.component");
    ...
    [
        ...
        asyncTest("_getTileUiComponentContainer: " + oFixture.testDescription, function () {
            ...
            if (oFixture.fnModifyResolutionResult) {
                // clone to avoid affecting other tests
                var oTileResolutionResultCopy = jQuery.extend(true, {}, oResolvedTile.tileResolutionResult);
                oResolvedTile.tileResolutionResult = oFixture.fnModifyResolutionResult(oTileResolutionResultCopy);
            }
            // Act
    ...
    // _getTileUiComponentContainer error handling tests
    asyncTest("_getTileUiComponentContainer rejects promise when componentName is undefined in resolution result", function () {
        var oAdapter = this.oAdapter,
            oTile = {
                id: "#App1-viaStatic",
                target: {
        ...
        oAdapter._getTileUiComponentContainer(oTile, oResolvedTile, false)
            .fail(function (oError) {
                ...
                ok(oAdapter.getTileTitle.called, "getTileTitle called");
                deepEqual(oAdapter._createTileComponentData.firstCall.args, [oTile, false, oResolvedTile], "correct arguments applied to _createTileComponentData");
                strictEqual(oError, "Cannot find name of tile component for tile with id: '#App1-viaStatic'", "tileComponent attached to resolved tile");
                start();
            })
            .done(okFalseAndStart);
    });
    ...
    asyncTest("resetGroup: change title, then reset group", function () {
        ...
        sinon.stub(that.oAdapter, "isGroupRemovable").callsFake(function () {
            return false;
        });

        sinon.stub(that.oAdapter, "isGroupLocked").callsFake(function () {
            return false;
        });
    ...
    [{
        ...
        asyncTest("addGroup: " + oFixture.testDescription, function () {
            ...
            var that = this;
            stubUsedServices(getFilteredSite());
            sinon.stub(utils, "generateUniqueId").callsFake(function () {
    ...
    test("_onTileSettingsSave: save on CommonDataModel service rejects", function (assert) {
        // Arrange
        var that = this,
            oServiceSpecifications = {
                CommonDataModel: {
        ...
        oSavePromise
            ...
            .done(function () {
                ok(false, "The promise was resolved unexpectedly.");
                done();
            });
    });
    ...
    (function () {
        ...
        asyncTest("#visitBookmarks()", function () {
            ...
            aCasesConsideredWithVisitor = getFixtures().map(function (oFixture) {
                ...
                return oAdapter
                    ._visitBookmarks(oFixture.url, oFixture.visitor, oFixture.vizType)
                    .fail(function () {
            ...
            aCasesConsideredWithoutVisitor = getFixtures().map(function (oFixture) {
                return oAdapter
                    ._visitBookmarks(oFixture.url, undefined, oFixture.vizType)
                    .fail(function () {
        ...
        asyncTest("#updateBookmarks()", function () {
            ...
            assertionsCompleted = [
                ...
                // act
                return oAdapter.
                    updateBookmarks(oFixture.initialState.url, oFixture.finalState, oFixture.initialState.vizType).
        ...
        asyncTest("#deleteBookmarks()", function () {
            ...
            assertionsCompleted = [
                ...
                return oAdapter.deleteBookmarks(oFixture.url, oFixture.vizType)
