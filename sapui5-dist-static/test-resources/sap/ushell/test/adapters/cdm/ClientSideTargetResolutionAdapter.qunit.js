...
sap.ui.require([
    ...
    "use strict";
    /*global test deepEqual asyncTest strictEqual module start ok sinon */
    ...
    test("getContentProviderDataOriginsLookup: return null when contentProviderDataOrigins is not defined in the site", function (assert) {
        var fnDone = assert.async();

        var oCDMServiceStub = {
            getSite: sinon.stub().returns({})
        };
        sinon.stub(sap.ushell.Container, "getServiceAsync").returns(Promise.resolve(oCDMServiceStub));

        this.oAdapter.getContentProviderDataOriginsLookup()
            .then(function (oResult) {
                assert.ok(true, "The promise should be resolved");
                assert.strictEqual(oCDMServiceStub.getSite.callCount, 1, "getSite was called");
    ...
    test("getContentProviderDataOriginsLookup: return lookup object when contentProviderDataOrigins is defined in the site", function (assert) {
        var fnDone = assert.async();

        var oContentProviderDataOrigins = {
                "CP1": {
                    "CP1": true,
                    "SYS1": true,
                    "SYS3": true
            ...
            oCDMServiceStub = {
                getSite: sinon.stub().returns(Promise.resolve({
    ...
    test("getContentProviderDataOriginsLookup: the contentProviderDataOrigins is cached", function (assert) {
        var fnDone = assert.async();
    ...
    test("getContentProviderDataOriginsLookup: reject when getSite is rejected", function (assert) {
        var fnDone = assert.async();

        var oCDMServiceStub = {
                getSite: sinon.stub().returns(Promise.reject())
            };
        sinon.stub(sap.ushell.Container, "getServiceAsync").returns(Promise.resolve(oCDMServiceStub));
