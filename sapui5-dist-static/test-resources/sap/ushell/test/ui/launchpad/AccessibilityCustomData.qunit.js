    ...
    asyncTest("Append DIV element with navigationMode ID to tile, when focusing it", function () {
        ...
        oTile.addDelegate({
            onAfterRendering: function () {
                ...
                setTimeout(function () {
                    start();
                    var oNavigationModeDOM = jQuery("#" + oTile.getId() + "-navigationMode")[0];
                    ok(oNavigationModeDOM.tagName === "DIV", "Div element with ID that contains -navigationMode");
                    ok(oTile.getDomRef().contains(oNavigationModeDOM), "Tile have descendant with known ID");
                }, 0);
            }
        }, false);
