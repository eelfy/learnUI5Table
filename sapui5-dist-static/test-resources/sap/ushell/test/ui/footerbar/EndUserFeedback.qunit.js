...
/**
 * @fileOverview QUnit tests for sap.ushell.ui.footerbar.EndUserFeedback
 */
(function () {
    "use strict";
    ...
    module("Accessibility", {
        ...
        setup: function () {
            ...
            sap.ushell.bootstrap("local").then(function () {
                ...
                this.oServiceStub = sinon.stub(sap.ushell.Container, "getService").returns({
                    getLegalText: function () {
                        var oLegalText = new jQuery.Deferred();
                        oLegalText.resolve("Some fancy legalese");
                        return oLegalText.promise();
                    }
                });
                this.endUserFeedbackDialog = new sap.ushell.ui.footerbar.EndUserFeedback(this.oEndUserFeedbackConfiguration),
                this.endUserFeedbackDialogGetModelStub = sinon.stub(this.endUserFeedbackDialog, "getModel").returns(this.oModel);
                this.endUserFeedbackDialogUpdateModelStub = sinon.stub(this.endUserFeedbackDialog, "updateModelContext").returns(true);

                //Show the dialog
                this.endUserFeedbackDialog.ShowEndUserFeedbackDialog();
                stop();
    ...
    asyncTest("The legal agreement starts with the back button in focus", function () {
        sap.ui.getCore().byId('legalAgreementLink').firePress();
        setTimeout(function () {
            var oBackButton =  sap.ui.getCore().byId('endUserFeedbackBackBtn');
            start();
            assert.strictEqual(document.activeElement, oBackButton.getDomRef(), "Back button is focused");
        }, 1);
    });

    asyncTest("Coming back from the legal agreement, the link to it is in focus", function () {
        var oLegalAgreementLink = sap.ui.getCore().byId('legalAgreementLink');
        oLegalAgreementLink.firePress();

        setTimeout(function () {
            sap.ui.getCore().byId('endUserFeedbackBackBtn').firePress();
            sap.ui.getCore().applyChanges();
            start();
            assert.strictEqual(document.activeElement, oLegalAgreementLink.getDomRef(), "Legal agreement link is focused");
        }, 1);
    });
}());
