...
sap.ui.require([
...
], function (Device, utils, System, oUi5ServiceFactory, oUi5NativeServiceFactory, ServiceFactoryRegistry, testUtils, OData, URI, Ui5NativeServiceFactory, Log) {
    "use strict";
    ...
    test("the registered dirty state provider is called with the navigation context when isDirty is set to false", function (assert) {
        var done = assert.async();
        sap.ushell.bootstrap("local").then(function () {
            // Arrange
            var oShellNavigation = sap.ushell.Container.getService("ShellNavigation");
            var oGetNavigationContextStub = sinon.stub(oShellNavigation, "getNavigationContext");
            var oNavigationContext = {
                status: "Test"
            };
            oGetNavigationContextStub.returns(oNavigationContext);
            var oSetDirtyStub = sinon.stub().withArgs(oGetNavigationContextStub).returns(true);

            sap.ushell.Container.registerDirtyStateProvider(oSetDirtyStub);

            // Act
            sap.ushell.Container.setDirtyFlag(false);
            var bDirty = sap.ushell.Container.getDirtyFlag();

            // Assert
            ok(oSetDirtyStub.calledWith(oNavigationContext));
            strictEqual(bDirty, true, "dirty state provider returned the correct value");

            // Restore
            sap.ushell.Container.deregisterDirtyStateProvider(oSetDirtyStub);
            oGetNavigationContextStub.restore();

            done();
        });
    });

    test("the registered dirty state provider is not called when isDirty is set to true", function (assert) {
        var done = assert.async();
        sap.ushell.bootstrap("local").then(function () {
            // Arrange
            var oShellNavigation = sap.ushell.Container.getService("ShellNavigation");
            ...
            oGetNavigationContextStub.returns(oNavigationContext);
            var oSetDirtyStub = sinon.stub().withArgs(oGetNavigationContextStub).returns(false);
            ...
            // Assert
            strictEqual(oSetDirtyStub.callCount, 0, "the registered dirty state provider was not called.");
            strictEqual(bDirty, true, "dirty state provider returned the correct value");
    ...
    test("test registered external dirty methods are executed every time 'getDirtyState' is called", function (assert) {
        var done = assert.async();
        sap.ushell.bootstrap("local").then(function () {
            strictEqual(sap.ushell.Container.getDirtyFlag(), false, "isDirty default value is false");
            var fnDirty = sinon.stub().returns(true);

            sap.ushell.Container.registerDirtyStateProvider(fnDirty);
            strictEqual(sap.ushell.Container.getDirtyFlag(), true, "isDirty value was set to true");
            strictEqual(fnDirty.callCount, 1, "The dirty state provider is called once");
            strictEqual(sap.ushell.Container.getDirtyFlag(), true, "isDirty value was set to true");
            strictEqual(fnDirty.callCount, 2, "The dirty state provider is called twice");
            done();
        });
    });
    ...
    test("getServiceAsync", function (assert) {
        ...
        sap.ushell.bootstrap("local").fail(testUtils.onError).done(function () {
            ...
            oBarServicePromise = sap.ushell.Container.getServiceAsync("Bar", undefined);
            strictEqual(oLogSpy.callCount, 0, "No warning has been logged.");
            oLogSpy.restore();

            // we need to test a rapid succession of calls that could potentially generate
            // a race condition.
            oBarServiceDouble = sap.ushell.Container.getServiceAsync("Bar", undefined);

            ok(oBarServicePromise instanceof Promise, "Promise returned");

            // assertions
            Promise.all([oBarServicePromise, oBarServiceDouble]).then(function (aServices) {

                strictEqual(aServices[0], aServices[1], "getServiceAsync returns the same instance even if called several times in rapid succession");
                var oBarService = aServices[0];
                ok(oBarService instanceof oServiceConstructorStub, "getServiceAsync returned the created service");
                ok(oRequireStub.calledWith(["sap/ushell/services/Bar"]), "required the Bar service");
                ok(oRequireStub.calledWith(["sap/ushell/adapters/local/BarAdapter"]), "required the Bar adapter for local platform");

                strictEqual(oRequireStub.callCount, 2, "require was called twice");
                strictEqual(sap.ushell.Container.getService("Bar"), oBarService, "second call delivers the same instance");
                ...
                ok(oServiceAdapter instanceof sap.ushell.adapters.local.BarAdapter, "Service constructor was called with an instance of the adapter as first parameter");
    ...
    test("getService", function (assert) {
        ...
        sap.ushell.bootstrap("local").fail(testUtils.onError).done(function () {
            var oRequireStub,
                oBarService,
                oLogMock = testUtils.createLogMock().warning(
                    "Deprecated API call of 'sap.ushell.services.Container.getService'. Please use 'getServiceAsync' instead",
                    null,
                    "sap.ushell.services.Container");

            var oServiceConstructorStub = sinon.stub();
            oServiceConstructorStub.hasNoAdapter = false;
            oRequireStub = sinon.stub(sap.ui, "requireSync").returns(oServiceConstructorStub);

            // use virtual Bar service for testing service creation with adapter
            jQuery.sap.declare("sap.ushell.adapters.local.BarAdapter");
            sap.ushell.adapters.local.BarAdapter = function (oSystem, sParameter, oProperties) {
                // simple check if the given system is the logon system
                strictEqual(oSystem.getPlatform(), "local", "The given system has the correct platform");
                strictEqual(oSystem.getAlias(), "", "The given system has the correct alias");
                strictEqual(sParameter, undefined, "The correct parameter was given");
                deepEqual(oProperties, {config: {}}, "The correct config was given");
            };
            ...
            ok(oRequireStub.calledWithExactly("sap/ushell/services/Bar"), "required the Bar service");
            ok(oRequireStub.calledWithExactly("sap/ushell/adapters/local/BarAdapter"), "required the Bar adapter for local platform");
            strictEqual(oRequireStub.callCount, 2, "require was called twice");
            strictEqual(sap.ushell.Container.getService("Bar"), oBarService, "second call delivers the same instance");
            strictEqual(oRequireStub.callCount, 2, "require was not called again");
            ...
            ok(oServiceAdapter instanceof sap.ushell.adapters.local.BarAdapter, "Service constructor was called with an instance of the adapter as first parameter");
            strictEqual(typeof oContainerInterface.createAdapter, "function", "createAdapter is a function");
            strictEqual(sParameter, undefined, "Service constructor was called with undefined parameters");
            deepEqual(oProperties, {config: {}}, "Service constructor was called with  correct config");
            oLogMock.verify();
            done();
        });
    });

    test("calling getServiceAsync after getService returns the correct adapter", function (assert) {
        var done = assert.async();
        sap.ushell.bootstrap("local").fail(testUtils.onError).done(function () {
            var oBarService,
                oBarServicePromise;
            ...
            var oServiceConstructorStub = sinon.stub();
            oServiceConstructorStub.hasNoAdapter = false;
            sinon.stub(sap.ui, "requireSync").returns(oServiceConstructorStub);

            // use virtual Bar service for testing service creation with adapter
            jQuery.sap.declare("sap.ushell.adapters.local.BarAdapter");
            sap.ushell.adapters.local.BarAdapter = sinon.stub();
    ...
    test("getServiceAsync rejects promise if service couldn't be loaded", function (assert) {
        // Arrange
        var done = assert.async();
        sap.ushell.bootstrap("local")
            .fail(testUtils.onError)
            .done(function () {
                var oRequireStub = sinon.stub(sap.ui, "require", function (vModules, fnSuccessCallback, fnErrorCallback) {
                ...
                // Act
                sap.ushell.Container.getServiceAsync("NonExistingService")
                    .then(function () {
    ...
    test("getServiceAsync rejects promise if service adapter couldn't be loaded", function (assert) {
        // Arrange
        var done = assert.async();
        sap.ushell.bootstrap("local")
            .fail(testUtils.onError)
            .done(function () {
                var oRequireStub = sinon.stub(sap.ui, "require", function (vModules, fnSuccessCallback, fnErrorCallback) {
                    if (Array.isArray(vModules)) {
                        if (vModules.indexOf("sap/ushell/services/NonExistingService") >= 0) {
                            return fnSuccessCallback(sinon.stub());
                        }
...
function getServiceWConfig (assert, bAsync) {
    ...
    asyncTest("logout(): logout async event registration", function () {
        mockLocalStorage();
        sinon.spy(sap.ushell.adapters.local, "ContainerAdapter");
        sap.ushell.bootstrap("local").fail(testUtils.onError).done(function () {
            var iCounter = 0,
                fnAsyncCallback1 = sinon.spy(function () {
                ...
                fnAsyncCallback2 = sinon.spy(function () {
                    var d = new jQuery.Deferred();
                    setTimeout(function () {
                        iCounter++;
                        d.resolve();
            ...
            // code under test (deregistration)
            sap.ushell.Container.attachLogoutEvent(fnAsyncCallback1, true);
            sap.ushell.Container.detachLogoutEvent(fnAsyncCallback1);
            sap.ushell.Container.logout().done(function () {
                ok(fnAsyncCallback1.callCount === 0, "listener not called");
                sap.ushell.Container.attachLogoutEvent(fnAsyncCallback1, true);
                // code under test (registration)
                sap.ushell.Container.logout().done(function () {
                    ok(fnAsyncCallback1.callCount === 1, "listener called once");
                    sap.ushell.Container.detachLogoutEvent(fnAsyncCallback1);
                    sap.ushell.Container.attachLogoutEvent(fnAsyncCallback1, true);
                    sap.ushell.Container.attachLogoutEvent(fnAsyncCallback2, true);
                    iCounter = 0;
                    sap.ushell.Container.logout().done(function () {
                        ok(fnAsyncCallback1.callCount === 2, "listener called twice");
                        ok(fnAsyncCallback2.callCount === 1, "listener called once");
                        strictEqual(iCounter, 2, "counter is 2");
                        sap.ushell.Container.detachLogoutEvent(fnAsyncCallback1);
                        sap.ushell.Container.detachLogoutEvent(fnAsyncCallback2);
                        start();
                    });
                });
            });
    ...
    asyncTest("logout(): logout async event registration default timeout", function () {
        mockLocalStorage();
        sinon.spy(sap.ushell.adapters.local, "ContainerAdapter");
        sap.ushell.bootstrap("local").fail(testUtils.onError).done(function () {
            var fnAsyncCallback = sinon.spy(function () {
                    return new jQuery.Deferred().promise();
                });

            sap.ushell.Container.attachLogoutEvent(fnAsyncCallback, true);
            sap.ushell.Container.logout().done(function () {
                ok(fnAsyncCallback.callCount === 1, "listener not called");
                sap.ushell.Container.detachLogoutEvent(fnAsyncCallback);
                start();
            });
        });
    });
