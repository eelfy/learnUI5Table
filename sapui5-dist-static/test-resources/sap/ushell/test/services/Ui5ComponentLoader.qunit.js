...
sap.ui.require([
    ...
    (function () {
        ...
        [{
            ...
            aExpectedLogArgs: [ // expected arguments of Log.error
                ...
                sap.ushell.Container.getService("Ui5ComponentLoader").createComponent(
                    ...
                    .always(function () {
                        assert.strictEqual(Log.error.callCount, 1,
                            "Log.error was called one time");

                        // Assert
                        oFixture.aExpectedLogArgs.forEach(function (sExpectedArg, iIdx) {
                            var sGotArgument = Log.error.args[0][iIdx];
                            ...
                            assert.strictEqual(sGotArgument, sExpectedArg,
                                "Log.error was called with the correct argument #" + (iIdx + 1));
    ...
    [{
        ...
        QUnit.test("createComponent - ErrorLogging: " + oFixture.testDescription, function (assert) {
            ...
            if (Array.isArray(oApplicationDependencies.messages)) {
    ...
    [{
        ...
        QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement loads the correct resources when "
        + oFixture.testDescription + " and NO Bundle is provided in the configuration", function (assert) {
            // Arrange
    ...
    [{
        ...
        QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement loads the correct resources when "
        + oFixture.testDescription + " and a Bundle is provided in the configuration", function (assert) {
            // Arrange
            var oUi5ComponentLoader = sap.ushell.Container.getService("Ui5ComponentLoader"),
                oLoadBundleStub = sinon.stub(oUi5ComponentLoaderUtils, "loadBundle").returns(new jQuery.Deferred().resolve().promise()),
                oExpectedBundle = {
    ...
    QUnit.test("CoreResourcesComplement Loading - loadCoreResourcesComplement returns a Promise and, on success, "
    + "keeps a record of it for future calls so the logic doesn't get executed multiple times", function (assert) {
        // Arrange
        var oUi5ComponentLoader = sap.ushell.Container.getService("Ui5ComponentLoader"),
            oPromise,
            oSecondPromise,
