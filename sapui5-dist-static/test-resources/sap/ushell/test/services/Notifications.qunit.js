...
sap.ui.require([
    ...
    QUnit.test("Intent based consumption - read data from service configuration", function (assert) {
        ...
        testAsync(assert, function (oService) {
            ...
            assert.ok(oService._getConsumedIntents() === "&NavigationIntent%20eq%20%27object1-action1%27NavigationIntent%20eq%20%27object1-action2%27NavigationIntent%20eq%20%27object2-action1%27",
                 "Correct intents string");
    ...
    QUnit.test("Get request URLs including intent based consumption", function (assert) {
        testAsync(assert, function (oService) {
            ...
            assert.ok(oService._getRequestURI(0) === "NOTIFICATIONS_SRV/Notifications?$expand=Actions,NavigationTargetParams&$filter=IsGroupHeader%20eq%20false&intents%20eq%20a-b,a-c,c-b",
                "Intent based consumption - correct getNotifications URL");
            assert.ok(oService._getRequestURI(1) === "NOTIFICATIONS_SRV/Notifications?$expand=Actions,NavigationTargetParams&$filter=intents%20eq%20a-b,a-c,c-b",
                "Intent based consumption - correct getNotificationsByType URL");
            ...
            assert.ok(oService._getRequestURI(0) === "NOTIFICATIONS_SRV/Notifications?$expand=Actions,NavigationTargetParams&$filter=IsGroupHeader%20eq%20false",
                "Not intent based consumption - correct getNotifications URL");
    ...
    QUnit.test("Verify that the OData calls are sent correctly", function (assert) {
        ...
        testAsync(assert, function (oService) {
            ...
            assert.ok(OData.read.args[0][0].requestUri.endsWith("NOTIFICATIONS_SRV/GetBadgeNumber()") === true,
            ...
            var sExpectedRequestUri = "NOTIFICATIONS_SRV/Notifications?$expand=Actions,NavigationTargetParams&$orderby=CreatedAt%20desc&$filter=IsGroupHeader%20eq%20false&$skip=0&$top=10";
            assert.ok(OData.request.args[1][0].requestUri.endsWith(sExpectedRequestUri) === true, "_readNotificationsData 2nd call: Get Notifications");
            assert.strictEqual(OData.request.args[1][0].headers["Accept-Language"], sap.ui.getCore().getConfiguration().getLanguageTag(), "The user language was set to the 'Accept-Language' header");
    ...
    QUnit.test("Disable calling callback functions after notifications data read", function (assert) {
        ...
        testAsync(assert, function (oService) {
            ...
            assert.ok(oService._updateNotificationsConsumers.notCalled === true,
            ...
            assert.ok(oService._updateNotificationsCountConsumers.notCalled === true,
    ...
    QUnit.test("Intent based consumption full flow - verify URL correctness", function (assert) {
        ...
        testAsync(assert, function (oService) {
            ...
            var sExtectedRequestURL = "NOTIFICATIONS_SRV/Notifications?$expand=Actions,NavigationTargetParams&$orderby=CreatedAt%20desc&$filter=IsGroupHeader%20eq%20false&"
            ...
            assert.ok(OData.request.args[0][0].requestUri === sExtectedRequestURL, "The 2nd call to OData.read for badgenumber inclued the intents");
