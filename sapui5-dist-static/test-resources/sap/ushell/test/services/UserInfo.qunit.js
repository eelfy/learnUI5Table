...
sap.ui.require([
    ...
    test("delegation to Container - id", function () {
        var oUserInfoService = sap.ushell.Container.getService("UserInfo");
    ...
    test("delegation to Container - first name", function () {
        var oUserInfoService = sap.ushell.Container.getService("UserInfo");

        // prepare test
        sinon.stub(sap.ushell.Container, "getUser", function () {
            return new sap.ushell.User({firstName : "First_Name"});
        });

        // test
        strictEqual(oUserInfoService.getFirstName(), "First_Name");
        ok(sap.ushell.Container.getUser.calledOnce);
    });

    test("delegation to Container - last name", function () {
        var oUserInfoService = sap.ushell.Container.getService("UserInfo");

        // prepare test
        sinon.stub(sap.ushell.Container, "getUser", function () {
            return new sap.ushell.User({lastName : "Last_Name"});
        });

        // test
        strictEqual(oUserInfoService.getLastName(), "Last_Name");
        ok(sap.ushell.Container.getUser.calledOnce);
    });

    test("delegation to Container - full name", function () {
        var oUserInfoService = sap.ushell.Container.getService("UserInfo");

        // prepare test
        sinon.stub(sap.ushell.Container, "getUser", function () {
            return new sap.ushell.User({fullName : "Full_Name"});
        });

        // test
        strictEqual(oUserInfoService.getFullName(), "Full_Name");
        ok(sap.ushell.Container.getUser.calledOnce);
    });

    test("delegation to Container - email", function () {
        var oUserInfoService = sap.ushell.Container.getService("UserInfo");

        // prepare test
        sinon.stub(sap.ushell.Container, "getUser", function () {
            return new sap.ushell.User({email : "email"});
        });

        // test
        strictEqual(oUserInfoService.getEmail(), "email");
        ok(sap.ushell.Container.getUser.calledOnce);
    });
