...
sap.ui.require([
    ...
    "use strict";
    /*global hasher asyncTest deepEqual module sinon start  strictEqual test*/
    ...
    [
        ...
        test(oFixture.testDescription, function () {
            ...
            function getTypeAndDeleteAndAssert (sMethod) {
                var sType = typeof oCurrentApplication[sMethod];
                strictEqual(sType, "function", sMethod + "is a function");
    ...
    test("reloadCurrentApp", function (assert) {
        var done = assert.async();
        ...
        sap.ushell.bootstrap("local").then(function () {
            oService = sap.ushell.Container.getService("AppLifeCycle");
            var oStubGetHash = sinon.stub(window.hasher, "getHash").returns("#ABCD-123"),
                oStubTreatHashChanged = sinon.stub(sap.ushell.Container.getService("ShellNavigation").hashChanger, "treatHashChanged");

            oService.reloadCurrentApp();
