...
sap.ui.require([
    ...
    var O_COUNT_PARAMETERS = {
        ...
        I_DEBUG = Log.Level.DEBUG,
        I_TRACE = Log.Level.TRACE;
    ...
    function createService (oArgs) {
        ...
        var aSystemAlias = oArgs.systemAlias || [];
        ...
        if (!oAdapter) {
            ...
            oAdapter = {
                ...
                resolveSystemAlias: sinon.stub().returns(
    ...
    test("getServiceClientSideTargetResolution: returns something different than undefined", function (assert) {
        var done = assert.async();
        sap.ushell.Container.getServiceAsync("ClientSideTargetResolution").then(function (oClientSideTargetResolution) {
    ...
    [
        ...
        asyncTest("_getMatchingInbounds: works as expected when " + oFixture.testDescription, function () {
            ...
            var oUserDefaultParametersService = { // a fake UserDefaultParameters service
            ...
            oGetServiceStub.withArgs("UserDefaultParameters").callsFake(function (serviceName, parameter, async) {
                if (async) {
            ...
            oSrvc._getMatchingInbounds(oFixture.oParsedShellHash, oIndex, { bExcludeTileInbounds: oFixture.bExcludeTileInbounds })
                .done(function (aMatchingResults) {
                    ...
                    if (!jQuery.isEmptyObject(oFixture.mockedUserDefaultValues)) {
                        ok(oGetServiceStub.calledWith("UserDefaultParameters"), "the UserDefaultParameters service was invoked");
                    }
    ...
    [
        ...
        },
        {
            ...
            oIntent: mkInt("#Action-toappnavsample?id=aValue"),
            aFakeInbounds: [
                oTestHelper.createInbound("#Action-toappnavsample{id:@a@}", {}, { contentProviderId: "demoContentProviderId1" }), // id resolves to aValue (see test)
                oTestHelper.createInbound("#Action-toappnavsample{id:@b@}", {}, { contentProviderId: "demoContentProviderId2" }) // id resolves to bValue (see test)
            ],
            expectedLogHeader: [
                /\[REPORT #1\] Matching Intent 'Action-toappnavsample[?]id=aValue' to inbounds/,
                /form factor: <any>/
        ...
        asyncTest("_getMatchingInbounds: reports inbound search correctly when " + oFixture.testDescription, function (assert) {
            var done = assert.async();
            var oSrvc = createService();
            ...
            setTimeout(function () {
                ...
                oGetServiceStub.withArgs("ReferenceResolver").callsFake(function (serviceName, parameter, async) {
                    if (async) {
                        return Promise.resolve(oReferenceResolverService);
                    }

                    return oReferenceResolverService;
                });
                ...
                oSrvc._getMatchingInbounds(oFixture.oIntent, oIndex, oFixture.oConstraints)
                    .done(function (aMatchingResults) {
                        ...
                        strictEqual(Log.error.callCount, 0, "Log.error was called 0 times");
                        strictEqual(Log.warning.callCount, 0, "Log.warning was called 0 times");
                        strictEqual(Log.debug.callCount, 1, "Log.debug was called 1 time");

                        // check that each regexp matches the call argument of debug
                        oFixture.expectedLogHeader.forEach(function (rLog) {
                            var sLogHeader = Log.debug.getCall(0).args[0];
                        ...
                        oFixture.expectedLogBody.forEach(function (rLog) {
                            var sLogBody = Log.debug.getCall(0).args[1];
        ...
        setTimeout(function () {
            ...
            oSrvc._resolveHashFragment("#hash-fragment", function () { } /*fallback*/)
                ...
                .fail(function (sErrorMsg) {
                    ...
                    strictEqual(Log.warning.getCalls().length, 1, "Log.warning was called once");
                    strictEqual(Log.error.getCalls().length, 0, "Log.error was not called");
    ...
    asyncTest("_resolveHashFragment: promise is rejected when _getMatchingInbounds rejects", function () {
        ...
        setTimeout(function () {
            ...
            oSrvc._resolveHashFragment("#hash-fragment", function () { } /*fallback*/)
                ...
                .fail(function (sErrorMsg) {
                    ok(true, "Promise was rejected");
                    strictEqual(Log.warning.getCalls().length, 0, "Log.warning was not called");
                    strictEqual(Log.error.getCalls().length, 1, "Log.error was called once");
    ...
    [
        ...
        asyncTest("_resolveHashFragment: rejects promise when " + oFixture.testDescription, function () {
            ...
            oSrvc._resolveHashFragment(oFixture.sIntent)
                ...
                .fail(function () {
                    ok(true, "promise was rejected");
                    var bErrFound = Log.error.getCalls().some(function (oError) {
                        return oError.args[0].indexOf("Could not parse shell hash") === 0;
                    });
                    ok(bErrFound, "logged 'Could not parse shell hash ...'");
                })
                .always(function () {
                    start();
                });
    ...
    asyncTest("getUserDefaultParameterNames: filters inbounds according to the given systemContext", function () {
        ...
        var oExpectedParameters = { simple: { value3: {}, value4: {} }, extended: {} };
        var oSrvc = createService({
            inbounds: aInbounds
        });

        var oParameterNamesPromise = oSrvc.getUserDefaultParameterNames({ id: "contentProvider2" });
        ...
        oParameterNamesPromise.done(function (oGotParameterNames) {
            deepEqual(oGotParameterNames, oExpectedParameters, "obtained expected parameter names");
        }).always(function () {
            start();
        });
    });

    asyncTest("getUserDefaultParameterNames: defaults undefined contentProviderId to local", function () {
        ...
        var oExpectedParameters = { simple: { value1: {}, value2: {} }, extended: {} };
        var oSrvc = createService({
            inbounds: aInbounds
        });

        var oParameterNamesPromise = oSrvc.getUserDefaultParameterNames({ id: "" });
    ...
    [
        ...
        QUnit.test("getLinks: returns expected inbounds when " + oFixture.testDescription, function (assert) {
            var done = assert.async();
            var oSrvc = createService();
            setTimeout(function () {
                sinon.stub(Log, "warning");
                ...
                sinon.stub(oSrvc, "_getMatchingInbounds").returns(
                    jQuery.Deferred().resolve(oFixture.aMockedResolutionResults).promise()
                );
                ...
                if (oFixture.hasOwnProperty("sAction")) {
                    ...
                    .done(function (aResultSemanticObjectLinksOld) {
                        ...
                        testExcludeTileIntentArgument(oSrvc, true);
                        oSrvc._getMatchingInbounds.resetHistory(); // testExcludeTileIntentArgument called later again
    ...
    (function () {
        ...
        [
            ...
            QUnit.test("resolveTileIntent resolves as expected when " + oFixture.testDescription, function (assert) {
                var done = assert.async();
                ...
                sinon.stub(sap.ushell.Container, "getService").callsFake(function (sName) {
                ...
                oSrvc.resolveTileIntent(oFixture.sIntent)
                    ...
                    .fail(function (sError) {
                        if (oFixture.testType === "failure") {
                            ...
                            // warnings
                            var bFound;
                            if (oFixture.expectedWarningCallArgs !== undefined) {
                                bFound = Log.warning.getCalls().some(function (oLogCall) {
                                    for (var i = 0; i < oLogCall.args.length; i++) {
                                        if (oLogCall.args[i] !== oFixture.expectedWarningCallArgs[i]) {
                                            return false;
                                        }
                                    }
                                    return true;
                                });
                                ok(bFound, "Log.warning was called with the expected arguments");
                            }

                            // errors
                            if (oFixture.expectedErrorCallArgs !== undefined) {
                                bFound = Log.error.getCalls().some(function (oLogCall) {
                                    for (var i = 0; i < oLogCall.args.length; i++) {
                                        if (oLogCall.args[i] !== oFixture.expectedErrorCallArgs[i]) {
                                            return false;
                                        }
                                    }
                                    return true;
                                });
                                ok(bFound, "Log.error was called with the expected arguments");
                            }
    ...
    asyncTest("getDistinctSemanticObjects behave as expected when getInbounds fails", function () {
        ...
        oSrvc.getDistinctSemanticObjects()
            ...
            .fail(function (sErrorMessageGot) {
                ...
                strictEqual(
                    Log.error.getCalls().length,
                    0,
                    "Log.error was called 0 times"
                );

                strictEqual(
                    Log.warning.getCalls().length,
                    0,
                    "Log.warning was called 0 times"
                );
            })
            .always(function () {
                start();
        ...
        asyncTest("getLinks: logs an error and rejects promise when " + oFixture.testDescription, function () {
            ...
            fnGetSemanticObjectLinksBound()
                ...
                .fail(function (sErrorMessage) {
                    ...
                    strictEqual(Log.error.getCalls().length, 1, "Log.error was called once");
                    strictEqual(Log.error.getCall(0).args[0], "invalid input for _getLinks", "expected error title was logged");
                    ok(Log.error.getCall(0).args[1].indexOf(oFixture.sExpectedErrorDetailsPart) >= 0, oFixture.sExpectedErrorDetailsPart + " was found in logged error details");
                    strictEqual(Log.error.getCall(0).args[2], "sap.ushell.services.ClientSideTargetResolution", "error contains sap.ushell.services.ClientSideTargetResolution");
                })
                .always(function () {
                    start();
                });
    ...
    [
        ...
        asyncTest("getLinks: correctly logs resulting intents in log level " + oFixture.testLogLevel, function () {
            ...
            Log.getLevel = sinon.stub().returns(oFixture.testLogLevel);
            Log.Level = {
                DEBUG: I_DEBUG,
                TRACE: I_TRACE
            };
    ...
    asyncTest("isIntentSupported rejects promise with error message when one intent is not supported", function () {
        ...
        sinon.stub(oSrvc, "_isIntentSupportedOne").callsFake(function (sIntent) {
    ...
    asyncTest("_constructFallbackResolutionResult: logs an error when fallback function is passed as undefined", function () {
        ...
        oSrvc._constructFallbackResolutionResult(
            ...
            .then(function () {
                ...
                // test warnings
                strictEqual(Log.warning.getCalls().length, 0, "Log.warning was called 0 times");

                // test error message
                iTimesErrorCalled = Log.error.getCalls().length;
                strictEqual(iTimesErrorCalled, 1, "Log.warning was called 1 time");
                if (iTimesErrorCalled) {
                    deepEqual(Log.error.getCall(0).args, [
    ...
    [
        ...
        asyncTest("inbound filter on resolveHashFragment when " + oFixture.testDescription, function (assert) {
            var done = assert.async();
                        ...
                        if (oFixture.expectedWarningCalls && oFixture.expectedWarningCalls[sMenuType]) {
                            ...
                            if (aExpectedWarningCalls.length === Log.warning.callCount) {
                                ...
                                aExpectedWarningCalls.forEach(function (aCallArgs, iCall) {
                                    deepEqual(
                                        Log.warning.getCall(iCall).args,
                                        aCallArgs,
                                        "Log.warning was called with the expected arguments on call #" + (iCall + 1)
                                    );
                                });
                            }
                        } else {
                            strictEqual(Log.warning.callCount,
                                0, "Log.warning was not called");
                        }
                    });
            });
        });
    ...
    ["SAPUI5", "WDA"].forEach(function (sAppType) {
        test("transient app state does not persist for applicationType: " + sAppType, function (assert) {
            ...
            sap.ushell.Container.getServiceAsync("AppState").then(function (oAppStateService) {
                ...
                var oSrvc = createService();
                var oWindowAdapter = oAppStateService._oAdapter,
    ...
    QUnit.test("Has easy access resolver, but no url template 1", function (assert) {
        // Arrange
        var done = assert.async();
        sap.ushell.Container.getServiceAsync("ClientSideTargetResolution").then(function (oCSTR) {
            var oExpectedResolutionResult = {
                additionalInformation: "",
                applicationType: "NWBC",
                inboundPermanentKey: undefined,
                "sap-system": false,
                text: "XXX",
                url: "/sap/bc/ui2/nwbc/~canvas;window=app/wda/XXX/?sap-language=en-US"
            };

            // Act
            return oCSTR._resolveSingleMatchingTarget(this.oMatchTarget, null, this.sHash).done(function (resolutionResult) {
                // Assert
    ...
    QUnit.test("Has easy access resolver and a url template 2", function (assert) {
        ...
        sap.ushell.Container.getServiceAsync("ClientSideTargetResolution").then(function (oCSTR) {
            ...
            // Act
            return oCSTR._resolveSingleMatchingTarget(this.oMatchTarget, null, this.sHash)
                .done(function (resolutionResult) {
                    // Assert
                    assert.deepEqual(resolutionResult, oExpectedResolutionResult, "The Result is as expected!");
                    done();
                });
    ...
    QUnit.module("getSystemContext", {
        beforeEach: function () {
            ...
            this.oResolveSystemAliasStub = sinon.stub().returns(new jQuery.Deferred().resolve(this.oMockSystemAlias));
            ...
            this.oCreateSystemContextFromSystemAliasStub = sinon.stub(SystemContext, "createSystemContextFromSystemAlias").returns(this.oMockSystemContext);
        },
        afterEach: function () {
            this.oCreateSystemContextFromSystemAliasStub.restore();
        }
    });
    ...
    QUnit.test("Rejects if an invalid systemAlias was passed", function (assert) {
        ...
        this.oClientSideTargetResolution._oAdapter = {
            resolveSystemAlias: sinon.stub().returns(
                new jQuery.Deferred().reject({}))
        };

        // Act
        return this.oClientSideTargetResolution.getSystemContext("someInvalidSystemId").then(function () {
    ...
    QUnit.test("Returns the result of 'createSystemContextFromSystemAlias' with the local system context", function (assert) {
        ...
        // Act
        return this.oClientSideTargetResolution.getSystemContext().then(function (oSystemContext) {
    ...
    QUnit.test("Returns the result of 'createSystemContextFromSystemAlias'", function (assert) {
        ...
        // Act
        return this.oClientSideTargetResolution.getSystemContext("someSystemId").then(function (oSystemContext) {
            assert.strictEqual(this.oCreateSystemContextFromSystemAliasStub.callCount, 1, "'createSystemContextFromSystemAlias' was called exactly once");
