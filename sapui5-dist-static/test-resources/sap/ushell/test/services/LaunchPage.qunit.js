...
sap.ui.require([
...
], function (LaunchPage, testUtils, LaunchPageAdapter, Config, ContentExtensionAdapterFactory, AdapterFactoryConfig, FeaturedGroupMock) {
    "use strict";
    ...
    [
        ...
        test("addBookmark success", function () {
            ...
            sinon.stub(oLaunchPageService, "changeURLStatesToPersistent").returns(new jQuery.Deferred().resolve("MyUrl"));

            // code under test
            oLaunchPageService.addBookmark(oBookmarkConfig);
        ...
        test("test updateBookmarks", function (assert) {
            var oLaunchPageAdapter = {
                updateBookmarks: sinon.stub().returns(new jQuery.Deferred().resolve())
            };

            sinon.stub(ContentExtensionAdapterFactory, "getAdapters").returns(new jQuery.Deferred().resolve({}));
            ...
            sap.ushell.Container = {
                getService: sinon.stub().returns({
                    getSupportedPersistencyMethods: sinon.stub().returns([])
                })
            };

            return oLaunchPageService.updateBookmarks("###", { url: "foo" })
                .done(function () {
                    assert.strictEqual(oLaunchPageAdapter.updateBookmarks.callCount, 1,
                        "The adapter function \"updateBookmarks\" was called exactly once.");
                    assert.strictEqual(oLaunchPageAdapter.updateBookmarks.args[0][0], "###",
                        "1. Argument of adapter call is correct.");
                    assert.deepEqual(oLaunchPageAdapter.updateBookmarks.args[0][1], { url: "foo" },
                        "2. Argument of adapter call is correct.");

                    ContentExtensionAdapterFactory.getAdapters.restore();
