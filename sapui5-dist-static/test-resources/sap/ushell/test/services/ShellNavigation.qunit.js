...
sap.ui.require([
    ...
    test("getNavigationContext", function () {
        // Arrange
        var oNavigationService = sap.ushell.Container.getService("ShellNavigation");
        oHashChanger = oNavigationService.hashChanger;
        var oGetCurrentNavigationStateStub = sinon.stub(oHashChanger, "getCurrentNavigationState");
        oGetCurrentNavigationStateStub.returns({ status: "test"});
        var oIsInnerAppNavigationStub = sinon.stub(oHashChanger, "isInnerAppNavigation");
        oIsInnerAppNavigationStub.returns(true);

        //Act
        var oNavigationContext = oNavigationService.getNavigationContext();

        //assert
        equal(oGetCurrentNavigationStateStub.calledOnce, true, "getCurrentNavigationState was only called once.");
        equal(oIsInnerAppNavigationStub.calledOnce, true, "isInnerAppNavigation was only called once.");
        strictEqual(oNavigationContext.innerAppRoute, "");
        strictEqual(oNavigationContext.isCrossAppNavigation, false);
        strictEqual(oNavigationContext.status, "test");

        // Restore
        oGetCurrentNavigationStateStub.restore();
        oIsInnerAppNavigationStub.restore();
    });
    ...
    test("_enableHistoryEntryReplacedDetection: sets correct internal state when no hasher methods are called", function (assert) {
        // Arrange
        var oShellNavigationService = sap.ushell.Container.getService("ShellNavigation"),
        ...
        // Assert
        assert.strictEqual(oShellNavigationService._lastHashChangeMode, null);
        assert.strictEqual(oShellNavigationService._fnOriginalSetHash, oFakeHasher.setHash);
        assert.strictEqual(oShellNavigationService._fnOriginalReplaceHash, oFakeHasher.replaceHash);

        oFakeHasher.restore();
    });

    test("_enableHistoryEntryReplacedDetection: correct internal state when setHash is called", function (assert) {
        // Arrange
        var oShellNavigationService = sap.ushell.Container.getService("ShellNavigation"),
            oFakeHasher = stubHasherMethods(hasher);
        ...
        // Assert
        assert.strictEqual(oShellNavigationService._lastHashChangeMode, "setHash");
        assert.strictEqual(oShellNavigationService._fnOriginalSetHash.callCount, 1, "original method called once");

        oFakeHasher.restore();
    });

    test("_enableHistoryEntryReplacedDetection: correct internal state when hasher replaceHash is called", function (assert) {
        // Arrange
        var oShellNavigationService = sap.ushell.Container.getService("ShellNavigation"),
            oFakeHasher = stubHasherMethods(hasher);
        ...
        // Assert
        assert.strictEqual(oShellNavigationService._lastHashChangeMode, "replaceHash");
        assert.strictEqual(oShellNavigationService._fnOriginalReplaceHash.callCount, 1, "original method called once");
    ...
    test("replaceHashWithoutNavigation: rewite the hash without navigation", function (assert) {
        // Arrange
        var oShellNavigationService = sap.ushell.Container.getService("ShellNavigation"),
            oFakeHasher = stubHasherMethods(hasher);

        oShellNavigationService._fnOriginalSetHash = sinon.stub();
        // Act
        oShellNavigationService.replaceHashWithoutNavigation("#New-hash");
        hasher.replaceHash("#Some-hash");

        // Assert
        assert.strictEqual(oShellNavigationService._fnOriginalSetHash.callCount, 1, "original method called once");
        assert.deepEqual(oShellNavigationService._fnOriginalSetHash.getCall(0).args, ["#New-hash"], "the correct hash is set");
        assert.strictEqual(hasher.changed.active, true, "the hasher is acrive after setHash");

        oFakeHasher.restore();
        oShellNavigationService._fnOriginalSetHash = null;
    });
});
