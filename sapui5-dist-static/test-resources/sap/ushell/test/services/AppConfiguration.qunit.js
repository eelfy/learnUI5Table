...
sap.ui.require([
...
], function (AppType, AppConfiguration, testUtils, EventHub, ResourceBundle, ObjectPath, utils) {
    "use strict";
    ...
    */
    ...
    test("addMetadata: sets the technical name in case of Web Dynpro", function (assert) {
        ...
        var oSrvc = AppConfiguration,
            ...
            oStubGetApplicationName = sinon.stub(oSrvc, "getApplicationName").returns(""),
            oStubGetApplicationUrl = sinon.stub(oSrvc, "getApplicationUrl").returns(""),
            oStubIsApplicationTypeEmbeddedInIframe = sinon.stub(utils, "isApplicationTypeEmbeddedInIframe").returns(true);
            window.hasher = window.hasher || { getHash: function () { } };
            var oStubGetHash = sinon.stub(hasher, "getHash").returns("aaa-bb4");

        //act
        var oActualMetaData = oSrvc.getMetadata(oApplication);

        //assert
        assert.equal(oActualMetaData.technicalName, "myWdApp", "technical name as expected");

        //restore
        oStubGetApplicationName.restore();
        oStubGetApplicationUrl.restore();
        oStubIsApplicationTypeEmbeddedInIframe.restore();
        oStubGetHash.restore();
    });

    test("addMetadata: sets the technical name in case of Web Dynpro and host", function (assert) {
