...
sap.ui.require([
    ...
    QUnit.module("EventHub functionality", function (hooks) {
        ...
        QUnit.test("it calls the callback three times if multiple events with the same data are emitted but fireAlways flag is true", function (assert) {
            ...
            var fnEmit = EventHub.emit.bind(null, "Event", 1, true);
    ...
    QUnit.module("EventHub Channel functionality", function () {
        ...
        QUnit.test("no dispatching occurs immediately after #_reset is called", function (assert) {
            var fnDone = assert.async();

            var oCallback = sinon.stub();
            EventHub.once("Event").do(oCallback); // <-- executes first
            EventHub.once("Event").do(function () { // <-- executes next
                EventHub._reset();
            ...
            EventHub.once("Event").do(oCallback); // <-- executes last
