...
sap.ui.require([
...
], function (oApplicationType, URLParsing, oSystemAlias, ObjectPath, URLTemplateProcessor, Container) {
    "use strict";
    /* global QUnit sinon */
    ...
    QUnit.test("URLTemplateResolutionResult: empty application ID", function (assert) {
        ...
        var oStubURLTemplateProcessor = sinon.stub(URLTemplateProcessor, "expand");
        oStubURLTemplateProcessor.returns({});

        var oStubGetService = sinon.stub();
        oStubGetService.withArgs("UserInfo")
        .returns({
            getUser: function () {
                return {
        ...
        window.sap = {
            ushell: {
                Container: {
                    getService: oStubGetService
                }
            }
        };
        var oStubGetCore = sinon.stub();
        oStubGetCore.returns({
            getConfiguration: function () {
                return {};
            }
        });
        window.sap.ui = {
            getCore: oStubGetCore
        };
        ...
        oApplicationType.URL.generateResolutionResult(oMatchingTarget)
        .then(function () {
            ...
            oStubGetService.restore();
            oStubGetCore.restore();
        });
    });
