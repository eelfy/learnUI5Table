...
sap.ui.require([
    ...
    [
        ...
        test("utils." + sMethod + " logs an error when window.epcm.getNwbcFeatureBits throws", function () {
            ...
            ok(Log.error.calledOnce === true, "Log.error was called once");
    ...
    test("getParameterValueBoolean : ", function () {
        var val, stub;
        stub = sinon.stub(UriParameters, "fromQuery").returns({
            get: function () { return undefined; },
            getAll: function () { return undefined; }
        });
    ...
    */
    ...
    test("utils.calcVisibilityModes: empty groups on phone devices are not displayed on the homepage when not in edit mode", function () {
        // Arrange
    ...
    test("utils.calcVisibilityModes: empty groups on tablet devices are not displayed on the homepage when not in edit mode", function () {
        // Arrange
        var oGroupHasVisibleTilesStub = sinon.stub(utils, "groupHasVisibleTiles").returns(false),
            oOriginalSystem = Device.system;
        Device.system.desktop = false;
    ...
    test("utils.calcVisibilityModes: empty groups are displayed when tablet and desktop mode is enabled and the homepage is not in edit mode", function () {
        // Arrange
        var oGroupHasVisibleTilesStub = sinon.stub(utils, "groupHasVisibleTiles").returns(false),
            oOriginalSystem = Device.system;
        ...
        // Assert
        deepEqual(aResult, [true, true], "Group is shown as expected");
    ...
    QUnit.test("String Intent is the root intent", function () {
        strictEqual(utils.isRootIntent("#Custom-rootIntent"), true);
    });
    QUnit.test("String Intent with parameters", function () {
        strictEqual(utils.isRootIntent("#Custom-rootIntent?some=param1&param2=v2"), false);
    });
    QUnit.test("String Intent without '#' is the root intent", function () {
        strictEqual(utils.isRootIntent("Custom-rootIntent"), true);
    });
    QUnit.test("String Intent is not the root intent", function () {
        strictEqual(utils.isRootIntent("#Another-intent"), false);
    });
    QUnit.test("String Intent without '#' is not the root intent", function () {
        strictEqual(utils.isRootIntent("Another-intent"), false);
    });
    QUnit.test("Shell-home is not the root intent when not configured", function () {
        strictEqual(utils.isRootIntent("#Shell-home"), false);
    });
    QUnit.test("'#' intent is the root intent", function () {
        strictEqual(utils.isRootIntent("#"), true);
    });
    QUnit.test("empty intent is the root intent", function () {
        strictEqual(utils.isRootIntent(""), true);
    });
    QUnit.test("Invalid string root intent compared", function () {
        strictEqual(utils.isRootIntent("#1aunch-_me"), false);
    });
    QUnit.test("Array as input intent", function () {
        throws(utils.isRootIntent.bind(utils, ["#", "Some", "intent"]), Error("The given intent must be a string"));
    });
    QUnit.test("Undefined as input intent", function () {
        throws(utils.isRootIntent.bind(utils, undefined), Error("The given intent must be a string"));
    });
    QUnit.test("Number as input intent", function () {
        throws(utils.isRootIntent.bind(utils, 1234), Error("The given intent must be a string"));
    });
    QUnit.test("No arguments given", function () {
        throws(utils.isRootIntent.bind(utils), Error("The given intent must be a string"));
    });

    QUnit.module("isFlpHomeIntent");

    QUnit.test("Array as input intent", function () {
        throws(utils.isFlpHomeIntent.bind(utils, ["#", "Some", "intent"]), Error("The given intent must be a string"));
    });
    QUnit.test("Undefined as input intent", function () {
        throws(utils.isFlpHomeIntent.bind(utils, undefined), Error("The given intent must be a string"));
    });
    QUnit.test("Number as input intent", function () {
        throws(utils.isFlpHomeIntent.bind(utils, 1234), Error("The given intent must be a string"));
    });
    QUnit.test("No arguments given", function () {
        throws(utils.isFlpHomeIntent.bind(utils), Error("The given intent must be a string"));
    });
    QUnit.test("#Shell-home is the flp home intent", function () {
        strictEqual(utils.isFlpHomeIntent("#Shell-home"), true);
    });
    QUnit.test("#Launchpad-openFLPPage is the flp home intent", function () {
        strictEqual(utils.isFlpHomeIntent("#Launchpad-openFLPPage"), true);
    });
    QUnit.test("#Launchpad-openFLPPage with parameters is the flp home intent", function () {
        strictEqual(utils.isFlpHomeIntent("#Launchpad-openFLPPage?pageId=page&spaceId=space"), true);
    });
    QUnit.test("The flp home intent without hash", function () {
        strictEqual(utils.isFlpHomeIntent("Shell-home"), true);
    });
    QUnit.test("String Intent is not the flp home intent", function () {
        strictEqual(utils.isFlpHomeIntent("#Another-intent"), false);
    });
});
