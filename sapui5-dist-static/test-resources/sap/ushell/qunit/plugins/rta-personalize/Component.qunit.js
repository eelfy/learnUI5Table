...
function (
	...
	QUnit.module("init functionality", {
		beforeEach: function () {
			...
			return Renderer.createActionButton(this.oPlugin, this.oPlugin._onAdapt.bind(this.oPlugin), true)
				.then(this.oPlugin._checkForEnabledControls.bind(this.oPlugin));
		...
		function parameterizedRenderAndRemoveTest (bControlInstance, assert) {
			...
			sap.ui.getCore().getEventBus().publish(
				...
				bControlInstance ? oFakeObjectPageLayout0 : oFakeObjectPageLayout0.getId()
			);
			...
			sap.ui.getCore().getEventBus().publish(
				"sap.ui",
				"ControlForPersonalizationRendered",
				bControlInstance ? oFakeObjectPageLayout1 : oFakeObjectPageLayout1.getId()
			);
	...
	QUnit.module("init functionality with already rendered personalizable controls", {
		beforeEach: function () {
			...
			sandbox.stub(Trigger.prototype, "_startRta", function () {
				return new Promise(function (fnResolve) {
					this.sStatus = STATUS_STARTED;
					fnResolve();
				}.bind(this));
			});
			sandbox.stub(Trigger.prototype, "_stopRta", function () {
				return new Promise(function (fnResolve) {
			...
			return Renderer.createActionButton(this.oPlugin, this.oPlugin._onAdapt.bind(this.oPlugin), true)
				...
				.then(function () {
					this.oButton = new Button("PERSONALIZE_Plugin_ActionButton", {
						text: this.oPlugin.mConfig.i18n.getText(sPersTextKey)
		...
		QUnit.test("when the personalize button gets pressed with URL Parameter 'sap-ui-layer' !== 'USER'", function (assert) {
			var oEvent = {
				getSource: function () {
					return {
						getText: function () {
							return this.oPlugin.mConfig.i18n.getText("PERSONALIZE_BUTTON_TEXT");
						}.bind(this)
					};
				}.bind(this)
			};
			...
			var oStartRtaSpy = sandbox.spy(this.oPlugin.oTrigger, "triggerStartRta");
		...
		QUnit.test("when RTA gets started", function (assert) {
			...
			assert.equal(oButton.getText(), this.oPlugin.mConfig.i18n.getText(sEndPersTextKey), "the text of the button got updated");
			...
			assert.equal(oButton.getText(), this.oPlugin.mConfig.i18n.getText(sPersTextKey), "the text of the button got changed back");
		...
		QUnit.test("when RTA gets started without editable plugins", function (assert) {
			...
			this.oPlugin.oTrigger.triggerStartRta(this.oPlugin);
			...
			return this.oPlugin.oTrigger.oStoppingPromise.then(function () {
		...
		QUnit.test("_loadPlugins", function (assert) {
			...
			oRequireStub.withArgs([
				"sap/ui/rta/plugin/EasyAdd",
				"sap/ui/rta/plugin/EasyRemove"
			]).callsArgWithAsync(1, oEasyAdd, oEasyRemove);
