sap.ui.define([
	...
	function createTimelineWithODataModel(iGrowingThreshold) {
	...
	function testD(oTimeline, assert) {
		oTimeline._loadMore();
		oTimeline._sortClick();
		oTimeline.refreshContent();
		sap.ui.getCore().applyChanges();
		assert.equal(oTimeline._iItemCount, 0, "For oDataModel with operation mode default the item count need to be reset to initial on sort");
	}
	...
	QUnit.test("Show more after sorting - ODataModel", function (assert) {
		var oTimeline = createTimelineWithODataModel(4);
		testD(oTimeline, assert);
		oTimeline.destroy();
	});
