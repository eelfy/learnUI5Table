...
sap.ui.define([
...
], function (GraphTestUtils, Graph, TestLayout, Node, createAndAppendDiv, NoopLayout, SwimLaneChainLayout) {
	"use strict";
	...
	QUnit.test("Accessibility Testing", function(assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{
						key: 0
		...
		oGraph.attachGraphReady(function() {
			...
			assert.equal(oGraph._zoomIn.getAriaDescribedBy()[0], oGraph._zoomLabel.getId(), "Aria described by for Zoom in button");
			assert.equal(oGraph._zoomOut.getAriaDescribedBy()[0], oGraph._zoomLabel.getId(), "Aria described by for Zoom out button");

			var oActionButton = oGraph.$().find(".sapSuiteUiCommonsNetworkGraphDivActionButtonBackground")[0],
				sActionButtonTitle = oActionButton.title,
				sAriaLabelActionButton = oGraph.getNodes()[0]._getActionButtonTitle(oActionButton);
			var sExpectedLabel = "Action Button" + " " + sActionButtonTitle;
	...
	QUnit.test("Ensure Full screen close on HashChanged", function(assert) {
		var oGraph = GraphTestUtils.buildGraph({
			nodes: [
				{key: 0, title: "ABC"}
			]
		...
		oGraph.attachGraphReady(function () {
			...
			oGraph.toggleFullScreen();
			assert.equal(oGraph.isFullScreen(), true, "fullscreen is on");
			oGraph.oHashChanger.fireHashChanged();
			assert.equal(oGraph.isFullScreen(), false, "fullscreen is off");
			fnDone();
		});
		oGraph.placeAt("content");
	...
	QUnit.test("Exit destroys hashchanger instance", function (assert) {
		...
		fnDone = assert.async();
		oGraph.attachGraphReady(function () {
			sinon.stub(oGraph.oHashChanger, "destroy");
			oGraph.exit();
			assert.ok(oGraph.oHashChanger.destroy.calledOnce);
			fnDone();
		});
		oGraph.placeAt("content");
		sap.ui.getCore().applyChanges();
	...
	QUnit.test("Legend Click", function (assert) {
		...
		oGraph.attachGraphReady(function () {
			if (istate == 0){
				...
				sap.ui.getCore().byId(this.getId() + "-legendButton").firePress({ pressed: true });
				sap.ui.getCore().byId(this.getId() + "-legendButton").setPressed(true);
				istate++;
				oGraph.rerender();
			} else if (istate == 1){
				assert.equal(oGraph.getDomRef("legend").style.display,"block", "Legends Details is Visible");
				sap.ui.getCore().byId(this.getId() + "-legendButton").setPressed(false);
				istate++;
				oGraph.rerender();
			}else if (istate == 2){
				assert.equal(oGraph.getDomRef("legend").style.display,"none", "Legends Details is Hidden");
	...
	QUnit.module("Validate Node header color.", {
		beforeEach: function () {
			...
			this.applyTheme = function(sTheme, fnCallback) {
				...
				function fnThemeApplied(oEvent) {
					...
					if (sap.ui.getCore().getConfiguration().getTheme() === this.sRequiredTheme && sap.ui.getCore().isThemeApplied()) {
						...
						setTimeout(fnThemeApplied.bind(this), 1500, oEvent);
					}
				}
			};
		},
	...
	QUnit.test("Node Headers in HCB Theme", function (assert) {
		var done = assert.async();
		this.applyTheme("sap_fiori_3_hcb", function() {
			...
			oGraph.attachGraphReady(function () {
				for (var i = 0; i < oGraph.getNodes().length; i++) {
					assert.equal(window.getComputedStyle(oGraph.getNodes()[i].getDomRef("header")).backgroundColor,"rgb(0, 0, 0)","Node Headers are black for HCB Theme");
				}
				fnDone();
			});
			oGraph.placeAt("content");
	...
	QUnit.test("Node Headers in HCW Theme", function (assert) {
		var done = assert.async();
		this.applyTheme("sap_fiori_3_hcw", function() {
			var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, title: "Zero", shape: "Box", group: "A"},
					{key: 1, title: "One", shape: "Box", group: "A"},
			...
			oGraph.attachGraphReady(function () {
				for (var i = 0; i < oGraph.getNodes().length; i++) {
					assert.equal(window.getComputedStyle(oGraph.getNodes()[i].getDomRef("header")).backgroundColor,"rgb(255, 255, 255)","Node Headers are white for HCW Theme");
				}
				fnDone();
			});
			oGraph.placeAt("content");
	...
	QUnit.test("Test Accessibility Title for Group", function (assert) {
		var done = assert.async(),
			oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, title: "Zero", shape: "Box", group: "A"},
		...
		oGraph.attachGraphReady(function () {
			...
			assert.equal(oGraph.getDomRef("accessibility").innerHTML, "Action Button Group Detail", "Accessibility Title has been set correctly.");
