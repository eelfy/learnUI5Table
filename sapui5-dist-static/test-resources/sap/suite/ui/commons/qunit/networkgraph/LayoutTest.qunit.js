sap.ui.define([
	...
	// Skipped for integration to go on, version af1b7b9 of D3ForceWorker.js works everywhere,
	...
	QUnit.test("Swimlane algorithm is generated for groups which is collapsed and does not have any node.", function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: "A"}, {key: 1, group: "A"}, {key: 2, group: "A"},
					{key: 3, group: "C"}, {key: 4, group: "C"}, {key: 5, group: "C"}
				],
				lines: [
					{from: 0, to: 1}, {from: 0, to: 2}, {from: 1, to: 2}
				],
				groups: [
					{key: "A", collapsed: false}, {key: "B", collapsed: true}, {key: "C", collapsed: false}
				]
			}),
			fnAssert = function () {
				assert.equal(oGraph.getGroups()[0]._iHeight, oGraph.getGroups()[2]._iHeight, "Both groups should have the same height.");
			},
			fnDone = assert.async();
