sap.ui.define([
	...
	QUnit.test("Parsing string to items extended ", function (assert) {
		...
		this.oCalculationBuilder.setAllowStringLiterals(true);
	...
	QUnit.test("Empty custom Literal ", function (assert) {
		this.oCalculationBuilder.setAllowStringLiterals(true);

		this.oCalculationBuilder.setExpression("\"test Literal with spaces\"");

		render(this.oCalculationBuilder);

		assert.equal(this.oCalculationBuilder._oInput.$("input").find("span").length, 1, "Only one span is created");

		this.oCalculationBuilder.setAllowStringLiterals(false);
		this.oCalculationBuilder.setExpression("\"test Literal with spaces\"");
		sap.ui.getCore().applyChanges();
