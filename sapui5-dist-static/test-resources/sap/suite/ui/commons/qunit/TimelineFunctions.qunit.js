sap.ui.define([
	...
	QUnit.test("Test _setRangeFilter for Custom Range & Months", function (assert) {
		var oMinDate = new Date(2017, 5, 5),
			oMaxDate = new Date(2017, 5, 5),
			oTimeline = buildTimeline(oMinDate, oMinDate, TimelineGroupType.None);
		assert.ok(oTimeline._objects.getTimeRangeSlider().hasStyleClass("sapTimelineRangeFilterNoDiff"), "Class should be there");

		var oMinDate = new Date(2017, 5, 5),
			oMaxDate = new Date(2040, 5, 1),
			oTimeline = buildTimeline(oMinDate, oMaxDate, TimelineGroupType.Month);
		assert.ok(!oTimeline._objects.getTimeRangeSlider().hasStyleClass("sapTimelineRangeFilterNoDiff"), "Class should not be there");
	});
