sap.ui.define([
	...
	QUnit.test("init creates _oFullScreenButton correctly", function(assert) {
		...
		assert.equal(oFullScreenBtn.getText(), oFullScreenBtn.getTooltip(), "Full screen button text and tooltip are identical");
	...
	QUnit.test("_oShowLegendButton for different values of showLegend & showLegendButton", function(assert) {
		var oVizFrameContent = new ChartContainerContent({
			content: new VizFrame()
		});
		var oChartContainer = new ChartContainer({
			content: oVizFrameContent,
		...
		assert.notOk(oChartContainer._oToolBar.getContent().includes(oChartContainer._oShowLegendButton), "button is not added based on showLegendButton");
		...
		oChartContainer.setShowLegendButton(true);
		oChartContainer.setShowLegend(false);
		sap.ui.getCore().applyChanges();
		assert.equal(oChartContainer._oShowLegendButton.getPressed(), oChartContainer.getShowLegend(), "button is visually toggled based on showLegend");
	});
	...
	QUnit.test("exit destroys all controls", function(assert) {
		...
		var stubDeregister = sinon.stub(ResizeHandler, "deregister");
		//stubbing destroy method of hashchanger
		var stubHashChangerDestroy = sinon.stub(this.oChartContainer.oHashChanger, "destroy");
		...
		assert.ok(stubDeregister.called, "resizeHandler.deregister called");
		assert.ok(stubHashChangerDestroy.calledOnce, "oHashChanger destroyed on exit");
		assert.equal(this.oChartContainer.oHashChanger, undefined, "_oToolbarSpacer is undefined");
	});
