...
sap.ui.define([
	...
	QUnit.test("_getExifOrientation", function(assert) {
		var fnDone = assert.async(),
			oBlob = this.oImageEditor._oOriginalBlob;
		this.oImageEditor._getExifOrientation(oBlob).then(function(iOrientation) {
			assert.equal(iOrientation, -2, "correct exif orientation passed");
			fnDone();
		});
	});
});
