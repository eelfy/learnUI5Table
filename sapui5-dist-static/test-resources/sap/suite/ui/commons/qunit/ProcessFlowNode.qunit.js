sap.ui.define([
	...
	QUnit.test("Process flow node is rendered.", function(assert) {
		...
		var $node = jQuery(document.getElementById(nodeId));
	...
	QUnit.test("Process flow node title present.", function(assert) {
		var nodeId = this.processFlowNode.getId();
		var $node = jQuery(document.getElementById(nodeId + "-title"));
	...
	QUnit.test("Process flow node icon container present.", function(assert) {
		var nodeId = this.processFlowNode.getId();
		var $node = jQuery(document.getElementById(nodeId + "-icon-container"));
	...
	QUnit.test("Process flow node icon present.", function(assert) {
		var nodeId = this.processFlowNode.getId();
		var $node = jQuery(document.getElementById(nodeId + "-icon"));
	...
	QUnit.module("Process flow texts hyphenation", {
		beforeEach: function() {
			this.processFlowNode =
				new ProcessFlowNode({
					id: "processFlowNode1",
	...
	QUnit.test("Hyphenation exists for title, status and texts", function(assert) {
		/* Arrange */
		this.processFlowNode.setChildren(["1", "2"]);
		this.processFlowNode.setTitle("singlewordwithoutspacesbutwithhyphenationenabled");
		this.processFlowNode.setTexts(["singlewordwithoutspacesbutwithhyphenationenabled", "singlewordwithoutspacesbutwithhyphenationenabled"]);
		this.processFlowNode.setStateText("singlewordwithoutspacesbutwithhyphenationenabled");
		this.processFlowNode.setState(ProcessFlowNodeState.Planned);
		/* Act */
		var resultTitle = this.processFlowNode._getHeaderControl();
		var resultValueStateText = this.processFlowNode._getStateTextControl();
		var resultText1 = this.processFlowNode._createText1Control();
		var resultText2 = this.processFlowNode._createText2Control();
		/* Assert */
		assert.equal(resultTitle.getWrappingType(), sap.m.WrappingType.Hyphenated, "Hyphenation property is present for title");
		assert.equal(resultValueStateText.getWrappingType(), sap.m.WrappingType.Hyphenated, "Hyphenation property is present for state text");
		assert.equal(resultText1.getWrappingType(), sap.m.WrappingType.Hyphenated, "Hyphenation property is present for text 1");
		assert.equal(resultText2.getWrappingType(), sap.m.WrappingType.Hyphenated, "Hyphenation property is present for text 2");
	});
