sap.ui.define([
	...
	QUnit.test("Check status for all zeros value", function (assert) {
		this.processFlowLaneHeader1.setState([{state: "Positive", value: 0}, {state: "Negative", value: 0},
		{state: "Neutral", value: 0}, {state: "Planned", value: 0}, {state: "Critical", value: 0}]);

		var testText = this.processFlowLaneHeader1._getAriaText();
		assert.deepEqual(testText, this.processFlowLaneHeader1._oResBundle.getText(''));
	});
	...
	QUnit.test("Process flow lane header custom tooltip test", function(assert) {
		var sLaneTooltip = this.processFlowLaneHeader1.$().find(".suiteUiProcessFlowLaneHeaderBodyContainer")[0].title;
		assert.deepEqual(sLaneTooltip, "Custom Lane tooltip", "Custom tooltip for processflow lane header");
	});
