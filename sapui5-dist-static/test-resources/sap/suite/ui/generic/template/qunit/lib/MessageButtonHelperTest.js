...
sap.ui.define(["testUtils/sinonEnhanced",
			  ...
			  ], function(sinon, JSONModel, Filter, FilterOperator, MessageButtonHelper, Utils, testableHelper) {
	"use strict";
	...
	var sEntitySet = "entity" + Utils.getRandomNumber();
	var oTemplPriv;
	var oComponent = {
		...
		getModel: function(sName){
			...
			throw new Error("Only message, ui, and _templPriv model must be accessed");
		}
	};
	var oMessageButton = {};
	var oView = {
	...
	function fnGeneralSetup(){
		...
		oMessagePopover.getItems = function(){
		return aMessagesShownInPopover;
		};
		...
		oTemplPriv.setProperty = function(sProperty, oValue){
			if (sProperty !== "/generic/messageCount" && sProperty !== "/generic/errorMessageCount" && sProperty !== "/generic/messageButtonTooltip" && sProperty !== "/generic/messageBtnIcon" && sProperty !== "/generic/messageSeverity"){
	...
	function setMessagesForPopover(aMessages) {
		aMessagesShownInPopover.splice(0, aMessagesShownInPopover.length)
		for (var i in aMessages) {
			if (aMessages[i] === 'Error') {
				aMessagesShownInPopover.push({ getType: function() {
				return "Error" }})
			} else if (aMessages[i] === 'Warning') {
				aMessagesShownInPopover.push({ getType: function() {
				return "Warning" }})
			} else if (aMessages[i] === 'Information') {
				aMessagesShownInPopover.push({ getType: function() {
				return "Information" }})
			} else {
				aMessagesShownInPopover.push({ getType: function() {
	...
	function checkTemplPrivContent(assert, iLength, severity, iSeverityCount){
		oItemBinding.getLength.returns(iLength);
		fnChangeListener();
		if (iLength) {
			if (severity === "Negative") {
				assert.strictEqual(oTemplPriv.getProperty("/generic/errorMessageCount"), iSeverityCount, "number of severity messages must be correct");
			}
			assert.strictEqual(oTemplPriv.getProperty("/generic/messageSeverity"), severity, "severity of messages must be correct");
	...
	QUnit.test("Test that class can be instantiated", function(assert) {
		...
		//populate aMessagesShownInPopover with Information, Warning, Error
		setMessagesForPopover(["Information", "Warning", "Error", "Error"]);
		checkTemplPrivContent(assert, 4, "Negative", 2);
		// populate aMessagesShownInPopover with Success, Information
		setMessagesForPopover(["Success", "Information"]);
		checkTemplPrivContent(assert, 2, "Neutral", 1);
		// populate aMessagesShownInPopover with Information, Warning
		setMessagesForPopover(["Information", "Warning", "Information"]);
		checkTemplPrivContent(assert, 3, "Critical", 1);
		// populate aMessagesShownInPopover with Success
		setMessagesForPopover(["Success"]);
		checkTemplPrivContent(assert, 1, "Success", 1);
