...
sap.ui.define(["testUtils/sinonEnhanced", "sap/ui/core/mvc/XMLView", "sap/ui/model/odata/ODataMetaModel", "sap/ui/model/odata/v2/ODataModel", "sap/suite/ui/generic/template/lib/navigation/routingHelper",
	...
	module("lib.navigation.routingHelper.generateRoutingStructure", {
		setup: function() {
			...
			var oAppComponent = {
				...
				getManifestEntry: function(sName){
					myAssert.strictEqual(sName, "sap.app", "Only manifest entry 'sap.app' must be accessed");
					return {
						title: sTitle
					};
				},
	...
	test("generateRoutingStructure fails without configuration", function(assert) {
		fnStartTest(assert);
	...
	test("generateRoutingStructure fails without pages", function(assert) {
		fnStartTest(assert, []);
		oRoutingStructureGenerated.catch(function(oError){
	...
	var fnGeneralCheck = function(assert){
		assert.ok(oXMLViewCreated.calledOnce, "Only one view must have been generated");
	...
	test("generateRoutingStructure for app with only root page", function(assert) {
