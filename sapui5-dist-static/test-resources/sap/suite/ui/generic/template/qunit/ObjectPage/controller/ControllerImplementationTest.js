...
sap.ui.define([
	...
	var mInfoObjects = {
		...
		SmartTableId: {},
		lazyloadingAfterHeaderSubsection:{},
		activateAfterHeaderDataReceivedSubsection:{},
		activateWithBindingChangeSubsection:{}
	};
		...
		oComponentUtils: {
			...
			getTemplatePrivateGlobalModel: function() {
				var oTemplatePrivateGlobalModel = {
						getProperty: function() {
								var generic = {
									placeHolderValue : 'FE-VIEW-CREATED'
								};
								return generic.placeHolderValue;
								}
...
//					oRebindTableSpy.restore();
	...
	QUnit.test("The fnAdaptBindingParamsForInlineCreate function", function(assert) {
		...
		oSmartTable.data = function(key) {
				if (key === "disableInlineCreateSort" || key === "inlineCreate") {
					return "true";
				}
			};
	...
	QUnit.test("The fnHandleUITableExpand Function", function(assert) {
		...
		var oOPSubSection = {
			sLayoutType: "sap.uxap.ObjectPageSubSection",
			isA: function(sLayout) {
				return sLayout === this.sLayoutType ? true : false;
			},
		...
		var oGridLayout = {
			sLayoutType: "sap.ui.layout.Grid",
			getParent: function() {
...
/* Deactivated, since hiding functionality has been refactored. Needs to be reworked.
	...
	module("Different loading behaviour of subsections on Object Page", {
		setup: fnCommonSetup,
		teardown: fnCommonTeardown
	});
	...
	QUnit.test("Section having loadingBehavior waitForHeaderData and waitForViewportEnter true and entered into view port before header data", function(assert) {
		...
		var loadingStrategy = mInfoObjects["secondSectionIdSecondSubSection"].loadingStrategy;
		loadingStrategy.bWaitForViewportEnter = false;
		loadingStrategy.waitFor = {
			bLayoutFinished: true,
			bStateApplied: false,
			bRebindCompleted: true
		}
		var oInactiveSpy = oSandbox.spy(loadingStrategy, "inActiveHandler");
		var oActiveSpy = oSandbox.spy(loadingStrategy, "activeHandler");
		oSandbox.stub(oTemplateUtils.oCommonUtils, "executeForAllInformationObjects", function (sCategory, fnExecutionFunction){
			switch(sCategory) {
				case "subSectionNotWaitingForViewPort":
				case "subSection":
					return fnExecutionFunction({
		...
		oMethods.handlers.onTableInit({
			getSource: function() {
				return {
					...
					data: function (){},
					setEnableDataStateFiltering: function (){}
		...
		assert.ok(oInactiveSpy.calledOnce, "Binding for SubSection with SmartTable with VM is suspended");

		// subsection entered into viewport before receving header data
		onSubSectionEnteredViewPort({
			getParameter: function (){return oSubSection;}
		});
		assert.ok(!oActiveSpy.calledOnce, "Binding still suspended, subsection is entered into viewport but header data is not recived");
		...
		oStubForPrivate.getStateAppliedPromise().then(function(){
			...
			setTimeout(function(){
				assert.ok(oActiveSpy.calledOnce, "Header data has received and binding of subsection is activated");
				// ensure to do the check only after the execution itself
				setTimeout(function(){
					oSubSection.destroy();
					done();
	...
	QUnit.test("Section having loadingBehavior waitForHeaderData and waitForViewportEnter true and entered into view port after header data", function(assert) {
		...
		var done = assert.async();
		var oSubSection = new ObjectPageSubSection({id:"lazyloadingAfterHeaderSubsection", blocks:new sap.m.Text({text:"Demo"})}); // third subSection overall, therefore not initially visible
		var loadingStrategy = mInfoObjects["lazyloadingAfterHeaderSubsection"].loadingStrategy;
		var oInactiveSpy = oSandbox.spy(loadingStrategy, "inActiveHandler");
		var oActiveSpy = oSandbox.spy(loadingStrategy, "activeHandler");
		oSandbox.stub(oTemplateUtils.oCommonUtils, "executeForAllInformationObjects", function (sCategory, fnExecutionFunction){
			switch(sCategory) {
				case "subSectionNotWaitingForViewPort":
				case "subSection":
					if (sCategory === "subSection" || !loadingStrategy.bWaitForViewportEnter) {
						return fnExecutionFunction({
							loadingStrategy:loadingStrategy,
							refresh: Function.prototype
						}, oSubSection);
		...
		assert.ok(!oActiveSpy.calledOnce, "Binding still suspended, Neither header data received nor subsection entered into view port");
		...
		oStubForPrivate.getStateAppliedPromise().then(function(){
			// ensure to do the check only after the execution itself
			setTimeout(function(){
				assert.ok(!oActiveSpy.calledOnce, "Binding is still suspended,Header data has received but subsection did not enter into viewport");
				
				// subsection entered into viewport after receving header data
				onSubSectionEnteredViewPort({
					getParameter: function (){return oSubSection;}
				});
				assert.ok(oActiveSpy.calledOnce, "Binding is activated, header data has received and subsection is also entered into viewport");
				setTimeout(function(){
					oSubSection.destroy();
					done();
				}, 0);
	...
	QUnit.test("Section having loadingBehavior waitForHeaderData true and waitForViewportEnter false and entered into view port before header data", function(assert) {
		...
		var loadingStrategy = mInfoObjects["activateAfterHeaderDataReceivedSubsection"].loadingStrategy;
		var oInactiveSpy = oSandbox.spy(loadingStrategy, "inActiveHandler");
		var oActiveSpy = oSandbox.spy(loadingStrategy, "activeHandler");
		oSandbox.stub(oTemplateUtils.oCommonUtils, "executeForAllInformationObjects", function (sCategory, fnExecutionFunction){
			switch(sCategory) {
		...
		assert.ok(!oActiveSpy.calledOnce, "Binding still suspended, Although visible but waiting for header data");

		oStubForPrivate.getStateAppliedPromise().then(function(){
			// ensure to do the check only after the execution itself
			setTimeout(function(){			
				assert.ok(oActiveSpy.calledOnce, "Binding is activated, when header data is received");
				setTimeout(function(){
					oSubSection.destroy();
					done();
				}, 0);
	...
	QUnit.test("Section having loadingBehavior waitForHeaderData false and waitForViewportEnter false and does not enter into view port", function(assert) {
		...
		var loadingStrategy = mInfoObjects["activateWithBindingChangeSubsection"].loadingStrategy;
		var oInactiveSpy = oSandbox.spy(loadingStrategy, "inActiveHandler");
		var oActiveSpy = oSandbox.spy(loadingStrategy, "activeHandler");
		oSandbox.stub(oTemplateUtils.oCommonUtils, "executeForAllInformationObjects", function (sCategory, fnExecutionFunction){
			switch(sCategory) {
		...
		setTimeout(function(){
			...
			assert.ok(oActiveSpy.calledOnce, "Binding is activated, without header data received and without entering to viewport");
			setTimeout(function(){
				oSubSection.destroy();
				done();
			}, 0);
	...
	QUnit.test("Table with VM in Section, state applied before ViewPort entered", function(assert) {
		...
		// however, onTableInit still needs to be called
		var done = assert.async();
		var oSubSection = new ObjectPageSubSection({id:"secondSectionIdSecondSubSection", blocks:new sap.m.Text({text:"Demo"})}); // third subSection overall, therefore not initially visible
		var loadingStrategy = mInfoObjects["secondSectionIdSecondSubSection"].loadingStrategy;
		var oInactiveSpy = oSandbox.spy(loadingStrategy, "inActiveHandler");
		var oActiveSpy = oSandbox.spy(loadingStrategy, "activeHandler");
		oSandbox.stub(oTemplateUtils.oCommonUtils, "executeForAllInformationObjects", function (sCategory, fnExecutionFunction){
		...
		oStubForPrivate.getStateAppliedPromise().then(function(){
			// ensure to do the check only after the execution itself
			setTimeout(function(){
				assert.ok(!oActiveSpy.calledOnce, "Binding still suspended, when state applied, but viewport not yet entered");

				onSubSectionEnteredViewPort({
					getParameter: function (){return oSubSection;}
				});

				assert.ok(oActiveSpy.calledOnce, "Subsection enters into viewPort and activates binding of subsection");
				// ensure to do the check only after the execution itself
				setTimeout(function(){
					oSubSection.destroy();
					done();
	...
	QUnit.test("Table with VM in Section, viewport entered before state applied", function(assert) {
		...
		assert.ok(!oActiveSpy.calledOnce, "Waiting for flag bStateApplied to be set");

		// ensure to do the check only after the execution itself
		setTimeout(function(){
			assert.ok(!oActiveSpy.calledOnce, "Binding still suspended, when ViewPort entered, but state not yet applied");
			...
			oStubForPrivate.getStateAppliedPromise().then(function(){
				...
				setTimeout(function(){
					...
					assert.ok(oActiveSpy.calledOnce, "Binding for SubSection with SmartTable with VM is restored");
					oSubSection.destroy();
					done();
				}, 0);
			});
	...
	QUnit.module("Functions to set focus after new row is added to Grid table" , {
		setup: function () {
			fnCommonSetup();
			this.oSmartTable = {
				getTable: function () {
	...
	QUnit.test("getGridTableRowIndexFromContext function returns correct index 1", function(assert) {
		...
		var iExpected = 1;
		//call
		var iResult = oStubForPrivate.getGridTableRowIndexFromContext(oNewContext, this.oSmartTable.getTable());
		//assert
		assert.equal(iExpected, iResult, "correct index is calculated")
		//clean
	});
	QUnit.test("getGridTableRowIndexFromContext function returns -1", function(assert) {
		//prepare
		var oNewContext = {
			getPath: function () {
				return "path4";
	...
	QUnit.test("getGridTableRow function returns correct table row", function(assert) {
		...
		var oExpected = this.oSmartTable.getTable().getRows()[1];
		//call
		var oResult = oStubForPrivate.getGridTableRow(oNewContext, this.oSmartTable.getTable());
		//assert
		assert.deepEqual(oExpected, oResult, "correct table row is returned")
		//clean
	});
});
