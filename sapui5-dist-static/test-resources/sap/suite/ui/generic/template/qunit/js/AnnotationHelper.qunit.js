sap.ui.define(["sap/ui/core/UIComponent",
...
], function (UIComponent, Log, AnnotationHelper, MockServer, testableHelper) {
	...
	QUnit.test("Method to determine fetchValueListReadOnly value", function () {
		var oAnnotationHelper = this.oAnnotationHelper;
		var oDataFieldValue = {
		...
		var stubResolvePath = sinon.stub(sap.ui.model.odata.AnnotationHelper, "resolvePath", function () {
	...
	QUnit.test("check method formatWithExpandSimplePath with mock data", function () {
		var done = assert.async();
		var oModel = this.getMockModel();
		assert.ok(oModel, "oModel Initiated");
		if (oModel) {
	...
	QUnit.test("buildBreadCrumbExpression", function () {
		...
		sResultAct = this.oAnnotationHelper.buildBreadCrumbExpression(oContext, oTreeNode, oTitle, oTypeName);
		assert.equal(sResultAct, sResultExp1, "breadcrumb expression built");
		sResultAct = this.oAnnotationHelper.buildBreadCrumbExpression(oContext, oTreeNode, oTitle, oTypeNameWithQuote);
	...
	QUnit.test("Check Default Column Width for Responsive Table Columns", function () {
		var oModel = {
			getODataEntityType: function (sQualifiedName, bAsPath) {
				var oODataEntityType = null;
				if (sQualifiedName === "STTA_PROD_MAN.STTA_C_MP_ProductType") {
	...
	QUnit.test("Check getDataFieldValueSimplePath function", function () {
		var oInnerInterface = {};
		var sFormatString;
		var oModel = {};
		var oInterface = {
		...
		var oDataFieldValue;
		var fnSimplePath = sinon.stub(sap.ui.model.odata.AnnotationHelper, "simplePath", function(vPar1, vPar2){
			return sFormatString;
		});
		oDataFieldValue = {
			Path: ""
	...
	QUnit.test("Check setRowHighlight method", function () {
		...
		var sExpectedvalueReturned = "{parts: [{path: 'IsActiveEntity'}, {path: 'HasActiveEntity'}, {path: 'ui>/editable'}, {path: '_templPrivMessage>/'}, {path: ''}], formatter: 'sap.suite.ui.generic.template.js.AnnotationHelper.setInfoHighlight'}";
	...
	QUnit.test("Check function getSmartTableControl", function () {
		var getSmartTableControl = oTestStub.AnnotationHelper_getSmartTableControl; // the function to be tested
		//Arrange
		var oSmartTable = {
	...
	QUnit.test("Check function getLineItemQualifier", function () {
		//Arrange
		var getLineItemQualifier = oTestStub.AnnotationHelper_getLineItemQualifier; // the function to be tested
		...
		//Act
		var sQualifier = getLineItemQualifier(oControlCustomData);
		//Assert
	...
	QUnit.test("Check function getImportanceForTableColumns", function () {
		var oAnnotationHelper = this.oAnnotationHelper;
		var oVisible = [];
	...
	QUnit.test("Check function getIconSemanticColor", function () {
		var done = assert.async();
		var oModel = this.getMockModel();
		assert.ok(oModel, "oModel Initiated");
		if (oModel) {
	...
	QUnit.test("Get binding for sort and filter property for Record Type:DataFieldForAnnotation - Contact", function () {
		this.oModel = {
			getODataEntityType: function (sQualifiedName, bAsPath) {
				var oODataEntityType = null;
				if (sQualifiedName === "STTA_PROD_MAN.STTA_C_MP_ProductType") {
		...
		var sAnnotationPath = "to_Supplier/@com.sap.vocabularies.Communication.v1.Contact";
		var sActualValue = this.oAnnotationHelper.getSortProperty(this.oInterface, oDataField, oContextSet, oDataFieldTarget1, sAnnotationPath, true);
		var sExpectedValue = "to_Supplier/CompanyName";
		assert.equal(sActualValue, sExpectedValue, "Should return Expected Value:" + sExpectedValue);

		var sAnnotationPath = "to_Supplier/@com.sap.vocabularies.Communication.v1.Contact";
		var sActualValue = this.oAnnotationHelper.getFilterProperty(this.oInterface, oDataField, oContextSet, oDataFieldTarget2, sAnnotationPath, false);
		var sExpectedValue = "to_Supplier/FullName";
		assert.equal(sActualValue, sExpectedValue, "Should return Expected Value:" + sExpectedValue);
	});

	QUnit.test("Get binding for sort and filter property for Record Type:DataFieldForAnnotation - DataPoint", function () {
		this.oInterface = {
			getInterface: function (iPart, sPath) {
				var oResult = {}
				if (iPart === 0) {
		...
		var sActualValue = this.oAnnotationHelper.getSortProperty(this.oInterface, oDataField, oContextSet, oDataFieldTarget, oDataField.Target.AnnotationPath, true);
		var sExpectedValue = "Quantity";
		assert.equal(sActualValue, sExpectedValue, "Should return Expected Value:" + sExpectedValue);

		var sActualValue = this.oAnnotationHelper.getFilterProperty(this.oInterface, oDataField, oContextSet, oDataFieldTarget, oDataField.Target.AnnotationPath, false);
		var sExpectedValue = "Quantity";
		assert.equal(sActualValue, sExpectedValue, "Should return Expected Value:" + sExpectedValue);
	});
});
