...
sap.ui.define(
		["sap/ui/comp/smarttable/SmartTable", "sap/m/Table", "sap/ui/table/AnalyticalTable", "sap/m/MessageBox", "sap/ui/base/Event", "sap/ui/model/json/JSONModel",
		 "testUtils/sinonEnhanced", "sap/suite/ui/generic/template/genericUtilities/controlHelper", "sap/suite/ui/generic/template/lib/CommonEventHandlers", "sap/ui/model/Context",
		 "sap/ui/model/odata/v2/ODataModel", "sap/suite/ui/generic/template/genericUtilities/testableHelper", "sap/suite/ui/generic/template/js/AnnotationHelper", "sap/ui/generic/app/navigation/service/SelectionVariant", "sap/base/util/extend", "sap/ui/core/Fragment"],
		 function(SmartTable, MTable, ATable, MessageBox, Event, JSONModel, sinon, controlHelper, CommonEventHandlers, Context, ODataModel, testableHelper, AnnotationHelper, SelectionVariant, extend, Fragment) {
			"use strict";
...
//					oMessageBoxStub.restore();
			...
			QUnit.test("onListNavigate (Intent) (ListReport specific)", function(assert) {
				...
				oCommonEventHandlers.onListNavigate(oEventSource, oState);
				// check
				assert.equal(oNavigationHandlerMixAttributesArguments.length, 2, "MixAttributes called with 3 parameters");
			...
			module("lib.CommonEventHandlers.getDataForDeleteDialog", {
				setup: function() {
					oStubForPrivate = testableHelper.startTest();
					sandbox = sinon.sandbox.create();
					oCommonEventHandlers = new CommonEventHandlers(oController, oTemplateUtils.oComponentUtils,
