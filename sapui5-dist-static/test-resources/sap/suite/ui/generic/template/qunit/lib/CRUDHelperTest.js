...
sap.ui.define(["testUtils/sinonEnhanced", "sap/suite/ui/generic/template/genericUtilities/testableHelper", "sap/suite/ui/generic/template/lib/CRUDHelper"], function(sinon, testableHelper, CRUDHelper) {
	"use strict";
	...
	QUnit.test("draft, no preserve change, unsaved changes, user cancels", function(assert) {
		...
		sandbox.stub(oTemplateContract.oApplicationProxy, "getDialogFragmentAsync", function(sFragmentName, oController,	sModelName) {
			sandbox.stub(oUnsavedChangesDialog, "open", function() {
				oController.onCancel();
			});
			return Promise.resolve(oUnsavedChangesDialog);
		});
		...
		setTimeout(function() {
			oEditPromise.then(function() {
				...
				assert.ok(oTemplateContract.oApplicationProxy.getDialogFragmentAsync.calledOnce, "DialogFragment read");
				assert.equal(oTemplateContract.oApplicationProxy.getDialogFragmentAsync.getCall(0).args[0], "sap.suite.ui.generic.template.ObjectPage.view.fragments.UnsavedChangesDialog", "for UnsavedChanges Dialog");
