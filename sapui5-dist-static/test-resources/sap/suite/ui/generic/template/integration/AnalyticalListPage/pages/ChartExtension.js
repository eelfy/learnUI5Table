		...
		onTheChart: {
			...
			assertions: {
				...
				*/
				...
				iCheckInitialNoData: function(sId) {
					return this.waitFor({
						controlType: "sap.m.Text",
						success: function(aText) {
							for(var i in aText){
								if(aText[i].getId().indexOf(sId) > -1){
									var initialNoData = aText[i].getText();
									if(initialNoData == "To start, set the relevant filters"){
										QUnit.ok(true, "Chart Initial No Data Text is correct");
									}
								}
							}
						},
						errorMessage: "Initial No Data Text is incorrect"
					});
				},
				iCheckNoData: function(sId) {
					return this.waitFor({
						controlType: "sap.chart.Chart",
						success: function(aChart) {
							for(var i in aChart){
								if(aChart[i].getId().indexOf(sId) > -1){
									var noData = aChart[i].getCustomMessages().NO_DATA;
									if(noData == "No data found. Try adjusting the search or filter parameters."){
										QUnit.ok(true, "Chart No Data Text is correct");
									}
								}
							}
						},
						errorMessage: " No Data Text is incorrect"
					});
				},
