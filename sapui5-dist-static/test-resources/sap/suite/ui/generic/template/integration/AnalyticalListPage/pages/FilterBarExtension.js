	...
	Opa5.createPageObjects({
		onTheFilterBar: {
			...
			actions: {
				...
				iSetDropDownInDateRangePopoverAndCheckItems: function(selectionKey, count) {
					var dropDownList, list, dropdownListCount;
					return this.waitFor({
						...
						check: function(aPopover) {
							...
							if (aToolbar[0]) {
								...
								if(list[1] && list[1].getItems()){
									dropdownListCount = list[1].getItems().length;
								}
								list[1].setSelectedKey(selectionKey);
								if (list[1].getSelectedKey() === selectionKey && dropdownListCount === count) {
									bFlag = true;
								}
							};
							return bFlag;
						...
						success: function() {
							QUnit.ok(true,"Dropdown list Clicked, value selected");
 						},
						errorMessage: "Dropdown list value not selected"
					});
				},
				iAddDateValueInDateRangePopover: function(value) {
					return this.waitFor({
						controlType: "sap.m.ResponsivePopover",
						check: function(aPopover) {
							var aToolbar = aPopover[0].getContent();
							aToolbar[0].getItems()[2].setDateValue(new Date(value));
							return true;
						},
						errorMessage: "Date Value not added"
					});
				},
			...
			assertions: {
				iCheckVisualFilterBarInvisibleText: function(iPosition, sId, sMandatoryProperty, sOverlayMsg) {
					return this.waitFor({
						...
						check: function (aHeaderContainer) {
							...
							return bSuccess && (aItems[3].isA("sap.ui.core.InvisibleText") && aItems[3].getId().indexOf(sId) > -1 && aItems[3].getText() === sMsg);
						},
						success: function() {
				...
				checkVisualFilterBarTitle: function () {
					return this.waitFor({
						...
						success: function(oLabel) {
							QUnit.ok(oLabel[0].hasStyleClass("sapSmartTemplatesAnalyticalListPageRequired"), "The Mandatory Parameter is applied in title");
						},
									...
									if (oChart.getParent().getParentProperty() === parentProperty) {
										...
										aPoints.forEach(function(oPoint) {
											...
											if (sLabel === "Dec 10, 2016" || sLabel === "Dec 11, 2016" || sLabel === "Oct 24, 2018" &&  sLabel === sToolTip.substr(0, 12)) {
												...
												nCount++;
											} else if (sLabel === "Q1 2022" || sLabel === "Q2 2021" || sLabel === "Q3 2021" || sLabel === "Q4 2021" && sLabel === sToolTip.substr(0, 7)) {
												nCount++;
											} else if (sLabel === "009/2021" && sLabel === sToolTip.substr(0, 8)) {
												nCount++;
											} else if (sLabel === "week 32 of 2021" || sLabel === "week 42 of 2021" || sLabel === "week 44 of 2021" || sLabel === "week 45 of 2021" || sLabel === "week 22 of 2022" && sLabel === sToolTip.substr(0, 15)) {
												nCount++;
											}
										});
				...
				iCheckPinButton: function (bVisible) {
					return this.waitFor({
						controlType: "sap.f.DynamicPageHeader",
						success: function (aDynamicHeader) {
							var oDynamicContainer = aDynamicHeader[0];
							if (oDynamicContainer.getPinnable() == bVisible) {
								QUnit.ok(true, "Pin Button is ok");
							}
						},
						errorMessage: "Pin Button not ok"
					});
				},
