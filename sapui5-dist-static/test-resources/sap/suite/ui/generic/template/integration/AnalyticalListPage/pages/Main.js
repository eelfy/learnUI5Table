...
], function (Opa5, Common, PropertyStrictEquals, OpaManifest, OpaResourceBundle, EnterText) {
	...
	Opa5.createPageObjects({
		onTheMainPage: {
			...
			actions: {
					...
					return this.waitFor({
						...
						success: function (aPopover) {
							aPopover.forEach(function (oPopover) {
								if (oPopover.aCustomStyleClasses.indexOf('sapMActionSheetPopover') > -1) {
				...
				iEnterValueInField: function(sValue, sId) {
					return this.waitFor({
						...
						success: Opa5.assert.ok(true, "The value '" + sValue + "' entered in the field with id '" + sId + "'"),
						errorMessage: "Field with id: '" + sId + "' is not found"
					});
				}
			},
			...
			assertions: {
				...
				checkVisualFilterBarTitle: function () {
					return this.waitFor({
						...
						success: function (oLabel) {
							QUnit.ok(oLabel[0].hasStyleClass("sapSmartTemplatesAnalyticalListPageRequired"), "The Mandatory Parameter is applied");
						},
				...
				checkSelectionColumn: function () {
					return this.waitFor({
						...
						success: function (aTable) {
							assert.equal(aTable[0].getMode(), "MultiSelect", "Selection column is visble with MultiSelect");
						},
						errorMessage: "Selection column is not visible in this page"
					});
				},
				...
				iCheckChartDataLabel: function (bIsKPICard) {
					var sControlType = bIsKPICard ? "sap.viz.ui5.controls.VizFrame" : "sap.ui.comp.smartchart.SmartChart";
					var bVisible = false;
					return this.waitFor({
						controlType: sControlType,
						searchOpenDialogs: bIsKPICard,
						check: function (charts) {
							if (!bIsKPICard) {
								var vizFrame = charts[0].getChart()._getVizFrame();
								var oVizProperties = vizFrame.getVizProperties();
								bVisible = oVizProperties.plotArea.dataLabel.visible;
							} else {
								var oVizProperties = charts[0].getVizProperties();
								bVisible = oVizProperties.plotArea.dataLabel.visible;
							}
							return bVisible;
						},
						success: function () {
							QUnit.ok(bVisible, "DataLabel visible="+bVisible);
						},
				...
				iShouldSeeTheNavigatedRowHighlightedAnalytical: function (iRowIndex, bExpectedHighlight, sId) {
					sId = sId ? sId : "--analyticalTable";
					return this.waitFor({
						id: new RegExp(sId + "$"),
						success: function (oTable) {
							var bHighlighted = oTable[0].getRows()[iRowIndex].getAggregation('_settings').getNavigated();
							Opa5.assert.equal(bHighlighted, bExpectedHighlight, "The Row Item Highlight for the item at index: " + iRowIndex + " is " + bHighlighted + " which is correct");
						},
						errorMessage: "The table with id '" + sId + "' not found on the screen"
					});
				},
				thePageVariantShouldBeMarkedDirty: function(bExpected) {
					return this.waitFor({
