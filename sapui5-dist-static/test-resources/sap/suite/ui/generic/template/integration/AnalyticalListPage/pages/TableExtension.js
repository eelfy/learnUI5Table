			...
			assertions: {
				...
				iCheckInitialNoData: function(sId){
					return this.waitFor({
						...
						success: function(aTable){
							for (var i in aTable){
								if(aTable[i].getId().indexOf(sId) > -1){
									var initialNoData = aTable[i].getInitialNoDataText();
									if(initialNoData == "To start, set the relevant filters"){
										QUnit.ok(true, "Initial No Data Text is correct");
									}
