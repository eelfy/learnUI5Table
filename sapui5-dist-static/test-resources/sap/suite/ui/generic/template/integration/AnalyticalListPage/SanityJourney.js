...
sap.ui.define(["sap/ui/test/opaQunit"], function() {
	...
	opaTest("Check initial no data text",function(Given,When,Then){
		...
		// Assertions
		Then.onTheChart.iCheckInitialNoData("__text48");
		Then.onTheTable.iCheckInitialNoData("analytics2::sap.suite.ui.generic.template.AnalyticalListPage.view.AnalyticalListPage::ZCOSTCENTERCOSTSQUERY0020--table");
	 });
	opaTest("Check error up is shown or not", function(Given, When, Then) {
	 ...
	 opaTest("Check no data text",function(Given,When,Then){
		...
		When.onTheFilterBar.iClickGoButton();
		// Assertions
        Then.onTheChart.iCheckNoData("__chart0");
		Then.onTheTable.iCheckNoData("analytics2::sap.suite.ui.generic.template.AnalyticalListPage.view.AnalyticalListPage::ZCOSTCENTERCOSTSQUERY0020--table");
	 });
     opaTest("Close Application",function(Given,When,Then){
		Given.iTeardownMyApp();
		expect(0);
	...
	opaTest("Start the application with a different manifest", function (Given, When, Then) {
		//manifestForAllFiltersAsInParameters is the name of the manifest we intend to use.
		//analytics2 is the application we want to start.
		Given.iStartMyAppInDemokit("analytics2", "manifestForAllFiltersAsInParameters");
		When.onTheVisualFilterDialog.iAddFilterValueInCompactDialog("Cost Element", "400020");
		When.onTheVisualFilterDialog.iClickDialogButton("OK", "SmartFilterBar-adapt-filters-dialog-confirmBtn", "analytics2");
		When.onTheGenericAnalyticalListPage.iSelectVFChart("bar", "100-1100", false, "CostCenter");
	});
