					...
					success: function(oCombobox) {
						if (!sLabel) {
							...
							for (var i = 0; i < oCombobox.length; i++) {
								if (oCombobox[i].getLabels()[0].getText() === sLabel) {
			            ...
			            this.waitFor({
							...
							actions: new Press(),
							success: function () {
								Opa5.assert.ok("Item '" + sItem + "' selected from the Combo Box");
							},
			        ...
			        success: function(oSelect) {
						if (!sLabel) {
							for (var j = oSelect.length - 1; j >= 0; j--) {
								...
								if (oSelect[j].getMetadata().getElementName() === "sap.m.Select") {
							...
							for (var i = 0; i < oSelect.length; i++) {
								//oSelect contains the object of control type "sap.uxap.HierarchicalSelect"
								if (oSelect[i].getMetadata().getElementName() === "sap.m.Select" && oSelect[i].getLabels()[0].getText() === sLabel) {
			...
			**/
			...
			iSeeThePopoverWithMainNavigationId: function(sId) {
				return this.waitFor({
					...
					matchers: [
						function (oPopover) {
							return (oPopover.getContent()[0].getMainNavigationId() === sId);
						}
					],
					success: function () {
				...
				return this.waitFor({
					...
					success: function (oSmartTable) {
						...
						for (var i = 0; i < aToolBarButtons.length; i++) {
							if (aToolBarButtons[i].getText && aToolBarButtons[i].getText() === "Show Details") {
								aToolBarButtons[i].firePress();
