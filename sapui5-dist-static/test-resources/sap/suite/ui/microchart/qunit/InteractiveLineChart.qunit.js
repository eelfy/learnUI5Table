...
sap.ui.define([
	...
	QUnit.module("ARIA tests", {
		beforeEach : function() {
			this.oChart = new InteractiveLineChart("ariaTestsChart", {
				points: [
					new InteractiveLineChartPoint({label: "May", secondaryLabel: "Q1", selected: true, value: 33.1}),
					new InteractiveLineChartPoint({label: "June", secondaryLabel: "Q1", value: 31.7}),
					new InteractiveLineChartPoint({label: "July", secondaryLabel: "Q2", selected: false}),
					new InteractiveLineChartPoint({label: "October", selected: false})
				]
			}).placeAt("qunit-fixture");
			sap.ui.getCore().applyChanges();
		},
	...
	QUnit.test("Area rendering", function(assert) {
		...
		var $Area4 = this.oChart.$().find(".sapSuiteILCInteractionArea").eq(3);
		//Assert
		//bar1
		assert.equal($Area1.attr("role"), "option", "The area1 role is <option>");
		assert.equal($Area1.attr("aria-label"), "May Q1 33.1", "The area1 labelledby is correct");
		...
		assert.equal($Area1.attr("aria-setsize"), "4", "The area1 setsize is correct");
		//bar2
		assert.equal($Area2.attr("role"), "option", "The area2 role is <option>");
		assert.equal($Area2.attr("aria-label"), "June Q1 31.7", "The area2 labelledby is correct");
		...
		assert.equal($Area2.attr("aria-setsize"), "4", "The area2 setsize is correct");
		//bar3
		assert.equal($Area3.attr("role"), "option", "The area3 role is <option>");
		assert.equal($Area3.attr("aria-label"), "July Q2 N/A", "The area3 labelledby is correct");
		...
		assert.equal($Area3.attr("aria-setsize"), "4", "The area3 setsize is correct");
		//bar4
		assert.equal($Area4.attr("role"), "option", "The area4 role is <option>");
		assert.equal($Area4.attr("aria-label"), "October N/A", "The area4 labelledby is correct");
		assert.equal($Area4.attr("aria-selected"), "false", "The area4 is not selected");
		assert.equal($Area4.attr("aria-posinset"), "4", "The area4 posinset is correct");
		assert.equal($Area4.attr("aria-setsize"), "4", "The area4 setsize is correct");
	});
	...
	QUnit.test("Chart rendering (interactive)", function(assert) {
		...
		assert.equal($Chart.attr("aria-roledescription"), this.oChart._oRb.getText("INTERACTIVELINECHART"), "The chart has roledescription");
	...
	QUnit.test("Chart owns", function(assert) {
		//Arrange
		var sId = this.oChart.getId();
		var sOwns = sId + "-interaction-area-0 " + sId + "-interaction-area-1 " + sId + "-interaction-area-2 " + sId + "-interaction-area-3";
		//Act
		//Assert
		var $Chart = this.oChart.$();
		assert.equal($Chart.attr("aria-owns"), sOwns, "Chart has attribute aria-owns to the associated area ids");
	});
	...
	QUnit.test("Aria label - semantic color", function(assert) {
		...
		//Assert
		//bar1
		assert.equal($Area1.attr("aria-label"), "May Q1 33.1 Neutral", "The aria label for bar1 is correct");
		//bar2
		assert.equal($Area2.attr("aria-label"), "June Q1 31.7 Good", "The aria label for bar2 is correct");
		//bar3
		assert.equal($Area3.attr("aria-label"), "July Q2 N/A Neutral", "The aria label for bar3 is correct");
		//bar4
		assert.equal($Area4.attr("aria-label"), "October N/A Neutral", "The aria label for bar4 is correct");
	});
	...
	QUnit.test("Validate Interactive Line Chart with No Data", function(assert) {
		assert.equal(this.oChart.getPoints().length, 0, "Interactive Line Chart has no Points.");
		assert.equal(this.oChart.getDomRef().querySelector(".sapSuiteILCChartCanvas"), null, "Interactive Line Chart Canvas is not created.");
	});

	QUnit.test("Validate Interactive Line Chart with Dynamic Data", function(assert) {
		assert.equal(this.oChart.getPoints().length, 0, "Interactive Line Chart has no Points.");
		assert.equal(this.oChart.getDomRef().querySelector(".sapSuiteILCChartCanvas"), null, "Interactive Line Chart Canvas is not created.");

		this.oChart.addPoint(new InteractiveLineChartPoint({label: "May", selected: true, value: 33.1}));
		this.oChart.addPoint(new InteractiveLineChartPoint({label: "June", value: 31.7}));
		this.oChart.addPoint(new InteractiveLineChartPoint({label: "July", selected: true, value: 23.1}));
		this.oChart.addPoint(new InteractiveLineChartPoint({label: "Aug", selected: false, value: 55.2}));

		sap.ui.getCore().applyChanges();

		assert.equal(this.oChart.getPoints().length, 4, "Interactive Line Chart has 4 Points.");
		assert.notEqual(this.oChart.getDomRef().querySelector(".sapSuiteILCChartCanvas"), null, "Interactive Line Chart Canvas is created.");
		assert.equal(this.oChart.getDomRef().querySelector(".sapSuiteILCSvgElement").children.length, 3, "Interactive Line Chart has 3 lines created.");
		assert.equal(this.oChart.getDomRef().querySelector(".sapSuiteILCInteraction").children.length, 4, "Interactive Line Chart has 4 points created.");
	});
