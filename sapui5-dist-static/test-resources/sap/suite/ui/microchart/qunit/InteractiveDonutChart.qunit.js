...
sap.ui.define([
	...
	QUnit.test("Method _toggleSelected adds selected class when it is selected", function(assert) {
		...
		assert.equal($InteractionArea.hasClass(InteractiveDonutChart.SEGMENT_CSSCLASS_SELECTED), false, "Interaction area doesn't have _segmentSelectedClass");
	});
	...
	QUnit.test("Chart rendering (interactive)", function(assert) {
		...
		assert.equal($Chart.attr("aria-roledescription"), this.oChart._oRb.getText("INTERACTIVEDONUTCHART"), "The chart has roledescription");
	...
	QUnit.test("Interactive", function(assert) {
		...
		var sExpectedSegmentTitle1 = "Label1:Text 1";
		var sExpectedSegmentLegendTitle1 = "Label1:Text 1";
	...
	QUnit.test("Non-Interactive", function(assert) {
		...
		var oChartSegment = this.oChart.$().find(".sapSuiteIDCChartSegment title");
		var oChartSegmentGhost = this.oChart.$().find(".sapSuiteIDCChartSegmentGhost title");
		var sExpectedChartTitle = "Label1: Text 1\nLabel2: 31.7\nLabel3: N/A";
	...
	QUnit.test("Interactive chart, semantic colors", function(assert) {
		...
		//Act
		//Assert
		assert.equal(this.oChart.$("interactionArea-0").attr("title"), "Label1:\nText 1 Critical", "The tooltip of the first legend entry has been set correctly.");
