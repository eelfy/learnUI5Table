...
sap.ui.define([
	...
	"./TestUtils"
], function (jQuery, BulletMicroChart, BulletMicroChartData, ValueColor, Size, BulletMicroChartModeType, FlexBox, GenericTile, TileContent,
	...
	QUnit.test("Min/Max values in small range", function(assert) {
		this.oChart = new BulletMicroChart({
		...
		assert.equal(aResult.actualValuePct, 49.05, "Actual value should be at 51.01%");
		assert.equal(aResult.targetValuePct, 98.00, "Target value should take 56.84%");
	});
	...
	QUnit.test("Control not rendered when no data is present and hideOnNoData is set to true", function(assert) {
		//Arrange
