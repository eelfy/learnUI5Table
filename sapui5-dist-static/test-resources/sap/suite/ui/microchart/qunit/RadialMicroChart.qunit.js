...
sap.ui.define([
	...
	QUnit.test("No data in aria-label", function(assert) {
		assert.equal(this.oRadialMicroChart.$().attr("role"), "figure", "chart aria role was rendered successfully");
	...
	QUnit.test("Control not rendered when no data is present and hideOnNoData is set to true", function(assert) {
		//Arrange
		this.oRadialMicroChart.setHideOnNoData(true);
		//Act
		sap.ui.getCore().applyChanges();

		//Assert
		assert.equal(this.oRadialMicroChart.getDomRef(), null , "chart not rendered");
	});
