...
sap.ui.define([
	...
	QUnit.test("createEditDraftEntity with Root Expand Parameter", function(assert) {
		var done = assert.async();
		var oController, oModel, oContext, bSubmit = false;
		var sRootExpand = "to_dummyContext1,to_dummyContext2";
		...
		sinon.spy(oController, "editDraft");
		oController.createEditDraftEntity(oContext, undefined, sRootExpand).then(function(oResult) {
			assert.equal("to_dummyContext1,to_dummyContext2", oController.editDraft.getCall(0).args[1].expand, "Expand params are passed");
			oController.destroy();
			done();
		}, function() {
			assert.ok(false);
