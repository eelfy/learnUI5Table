...
sap.ui.define([
	...
	QUnit.test("callAction with Expand parameters should succeed", function (assert) {
		var done = assert.async();
		var succeed = function () {
			assert.equal("to_dummyContext1,to_dummyContext2", oBaseController._callAction.getCall(0).args[2].expand, "Expand params are passed");
			done();
		};
		...
		oBaseController = new BaseController(this.oModel);
		sinon.spy(oBaseController, "_callAction");
		oBaseController._callAction("mockFn", null, { entitySet: "dummySet", expand: "to_dummyContext1,to_dummyContext2" }).then(succeed);
		oBaseController.destroy();
	});
