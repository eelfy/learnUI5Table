...
sap.ui.define([
			...
			oView = {
				...
				getControlsByFieldGroupId: function () {
					var oControl = {
						...
						getParent: function () {
							return {
								checkValuesValidity: function () {
									return bClientValidationError ? Promise.reject() : Promise.resolve();
								}
							};
						}
					};
	...
	QUnit.test("field group validation event successful (2)", function (assert) {
		...
		// The function _fnAttachValidateFieldGroup is synchronous and _onValidateFieldGroup is asynchronous.
		// To prevent the assertion getting executed immediately after the _fnAttachValidateFieldGroup , bSubmit is done after the triggerSubmitChanges is executed
		this.oController.attachEvent("beforeSideEffectExecution", function(oEvent) {
		...
		// fFunc with fieldGroupIds will trigger the _onValidateFieldGroup event to simulate field group change
		oView.fFunc(oEvent);
	});
	...
	QUnit.test("field group validation event fails because of client errors", function (assert) {
		...
		this.oController._onValidateFieldGroup(aIDs, oView).then(function (oResult) {
			assert.ok(!bSubmit);
			//assert.ok(oResult.response.mockresponse, "Promise returned from ValidateFieldGroup is the value from oModel.submitChanges");
