...
// ]
	...
	QUnit.test("_splitInboundNavigationParameters - Invalid startup parameters", function(assert) {
		...
		}, new sap.fe.navigation.NavError("NavigationHandler.INVALID_INPUT"), "Exception has been thrown successfully");
	...
	QUnit.test("validate navigate with sensitive and excluded information", function(assert) {
		...
		oAppStatePromise.done(function(oAppState) {
			...
			assert.equal(oAppData.semanticDates.Dates.length, 1, "PotentiallySensitive Property are removed from semantic dates before storing in AppState");
			assert.equal(oAppData.semanticDates.Dates[0].PropertyName, "NonSensitiveFilter1", "Only non sensitive properties from semantic dates are stored in app state");
	...
	QUnit.test("validate navigate with measure", function(assert) {
		...
		oAppStatePromise.done(function(oAppState) {
			...
			assert.equal(oAppData.semanticDates.Dates[0].PropertyName, "NonSensitiveFilter1", "Only non measure properties in semantic dates are stored in app state");
