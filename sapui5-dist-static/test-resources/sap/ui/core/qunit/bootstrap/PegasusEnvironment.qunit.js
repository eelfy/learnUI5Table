...
(function (){
	...
	function load(sUrl) {
		jQuery.ajax({
			url : sUrl,
			async : false,
			// Load the Javascript resource as text and execute it manually
			// This is to avoid that jQuery executes the script automatically which causes CSP violation
			dataType : 'text',
	...
	QUnit.test("Check that rendering worked", function(assert) {
		assert.ok(jQuery("#oTB1").length === 1, "control has been rendered");
	});
