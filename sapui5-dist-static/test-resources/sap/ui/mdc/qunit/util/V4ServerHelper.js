 ...
 */
sap.ui.define("util/V4ServerHelper", ["jquery.sap.global"], function(jQuery) { // require jQuery?
	"use strict";
	...
	var __requestAnyBaseUrl = new Promise(function(resolve) {
		jQuery.ajax({
			...
			error: function() {
				if (document.location.href.startsWith("http://localhost:8080")) { // this is a local dev scenario, so we can safely use the cloud server instead, for convenience
					jQuery.ajax({
						method: "GET",
	...
	var _serverAvailable = function() {
		return new Promise(function(resolve, reject) {
			__requestBaseUrl.then(function(serverBaseUrl) {
				if (serverBaseUrl) {
					resolve(true);
				} else {
					resolve(false);
