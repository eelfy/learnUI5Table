...
sap.ui.define([
	...
	QUnit.test("three.js ViewStateManager - activate view", function(assert) {
		...
		var testFunction = function(obj) {
			...
			for (i = 0; i < nodes.length; i++){
				...
				var te = nodes[i].matrix.elements;
			...
			viewStateManager._resetNodesMaterialAndOpacityByCurrenView(view1);
			viewStateManager._resetNodesStatusByCurrenView(view1, true, false);
	...
	QUnit.test("three.js ViewStateManager - restTransformation", function(assert) {
		...
		var testFunction = function(obj) {
			...
			for (i = 0; i < nodes.length; i++){
				viewStateManager.updateRestTransformation(nodes[i]);
			}
	...
	QUnit.test("three.js ViewStateManager - animation translation key creation with joints", function(assert) {
		...
		var testFunction = function(obj) {
			...
			var nodeInfos = [];
			var i, nodeInfo, te, transform;
			for (i = 0; i < nodes.length; i++){
				nodeInfo = {};
				nodeInfo.target = nodes[i];
				te = nodes[i].matrix.elements;
			...
			rnode1.position.z = z;
			rnode1.updateMatrix();
			te = rnode1.matrix.elements;
			transform = [ te[0], te[4], te[8], te[12], te[1], te[5], te[9], te[13], te[2], te[6], te[10], te[14] ];
			nodeInfo = [ { target: rnode1, transform: transform } ];
	...
	QUnit.test("ViewStateManager destroy all created ManagedObjects", function(assert) {
		var done = assert.async();

		var loader = new THREE.ObjectLoader();
		loader.load("test-resources/sap/ui/vk/qunit/media/nodes_boxes.json", function(obj) {
			var nativeScene = new THREE.Scene();
			var scene = new Scene(nativeScene);
			nativeScene.add(obj);
			...
			var nodeTransitionHelperAtCreation = UI5Utils.getAllUI5ObjectsByName('sap.ui.vk.threejs.NodesTransitionHelper').length;
			assert.equal(nodeTransitionHelperAtCreation, 1, "The instance of NodeTransitionHelper when ViewStateManager is created");

			viewStateManager.destroy();
			viewStateManager = null;

			var nodeTransitionHelper = UI5Utils.getAllUI5ObjectsByName('sap.ui.vk.threejs.NodesTransitionHelper').length;
			assert.equal(nodeTransitionHelper, 0, "The instance of NodeTransitionHelper after ViewStateManager is detroyed");

			done();
		});
	});

	QUnit.test("ViewStateManager destroy should destroy OutlineRenderer", function(assert) {
		var done = assert.async();
		...
		loader.load("test-resources/sap/ui/vk/qunit/media/nodes_boxes.json", function(obj) {
			...
			assert.ok(viewStateManager._outlineRenderer, "OutlineRenderer is created");

			viewStateManager.destroy();

			assert.ok(!viewStateManager._outlineRenderer, "OutlineRenderer is detroyed");
