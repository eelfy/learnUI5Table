...
sap.ui.define([
	...
	QUnit.moduleWithContentConnector("AnimationLoad", "test-resources/sap/ui/vk/qunit/media/stand_foot_rests.asm.json", "threejs.test.json", function(assert) {
		...
		te = rnode2.matrix.elements;
		transform = [ te[0], te[4], te[8], te[12], te[1], te[5], te[9], te[13], te[2], te[6], te[10], te[14] ];
		nodeInfo = [ { target: rnode2, transform: transform } ];
		view.updateNodeInfos(nodeInfo);
	...
	QUnit.test("Animation - sequence and playback", function(assert) {
		...
		allJoints = sequence1.getJoint();
		assert.ok(!allJoints.legth, "remove all joints");
	...
	QUnit.test("Animation - translation", function(assert) {
		...
		assert.deepEqual(property2.offsetToPrevious, [ 2.5, 2.5, 2.5 ], "renference node1 offset to previous at time 0.5");
	...
	QUnit.test("Animation - scale", function(assert) {
		...
		assert.deepEqual(property2.offsetToPrevious, [ 1.5, 1.5, 1.5 ], "renference node1 offset to previous at time 0.5");
	...
	QUnit.test("Animation - opacity", function(assert) {
		...
		assert.equal(property2.offsetToPrevious, 0.9, "renference node1 offset to previous at time 0.5");
	...
	*/
	...
	QUnit.test("Matrix ordering - euler rotations", function(assert) {
		...
		assert.deepEqual([ er.x, er.y, er.z ], [ rotx, roty, rotz ], "XYZ rotation");

		var rquat = new THREE.Quaternion();
		rquat.setFromRotationMatrix(r);

		assert.deepEqual([ rquat.x, rquat.y, rquat.z, rquat.w ], [ -0.12816275864801846, 0.6076211683515376, -0.22328869972317775, 0.7513407879536255 ], "Quat rotation");

		var key = [ rotx, roty, rotz, 6 /* XYZ */ ];
		var q2 = AnimationMath.neutralEulerToGlMatrixQuat(key);
		assert.deepEqual([ q2[0], q2[1], q2[2], q2[3] ], [ -0.12816275864801843, 0.6076211683515376, -0.22328869972317772, 0.7513407879536255 ], "Quat rotation");
	});
