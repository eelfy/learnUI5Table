...
sap.ui.define([
	...
	QUnit.test("AnimationMath - Cartesian <---> Polar", function(assert) {
		...
		var normalize = function(vector) {
			var lenght2 = vector.reduce(function(accumulator, value) { return accumulator + value * value; }, 0);
			var length = Math.sqrt(lenght2);

			return vector.map(function(value) { return value / length; });
		};

		var setVectorLegth = function(vector, length) {
			var normailsed = normalize(vector);
			return normailsed.map(function(value) { return value * length; });
		};
		...
		axisNormalized = setVectorLegth(axis, 2);
		polar.radius = 2.0;
		axisConverted = AnimationMath.polarToCartesian(polar);
