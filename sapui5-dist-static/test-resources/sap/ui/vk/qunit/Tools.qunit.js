...
sap.ui.define([
	...
	QUnit.test("TooltipTool", function(assert) {
		...
		viewport.rerender(); // required to immediately generate tooltipTool.domRef
	...
	QUnit.test("MoveTool", function(assert) {
		...
		assert.equal(moveTool.getPlacementMode(), GizmoPlacementMode.Default, "Default placement mode");

		var node = viewStateManager.getNodeHierarchy().findNodesByName()[4];
		...
		var worldTx = node.getWorldPosition();
		assert.equal(worldTx.x.toFixed(), "10", "Object translated X in world system");
		assert.equal(worldTx.y.toFixed(), "0", "Object not translated Y in world system");
		assert.equal(worldTx.z.toFixed(), "0", "Object not translated Z in world system");

		moveTool.setCoordinateSystem(CoordinateSystem.Local);
		moveTool.move(10, 0, 0);	// Move in local space
		var localTx = node.getWorldPosition();
		assert.equal(localTx.x.toFixed(), "10", "Object not translated X in world system");
		assert.equal(localTx.y.toFixed(), "0", "Object not translated Y in world system");
		assert.equal(localTx.z.toFixed(), "-10", "Object translated Z in local space");

		moveTool.setCoordinateSystem(CoordinateSystem.Parent);
		moveTool.move(10, 0, 0);	// Move in parent space
		var parentTx = node.getWorldPosition();
		assert.equal(parentTx.x.toFixed(), "10", "Object not translated X in parent space");
		assert.equal(parentTx.y.toFixed(), "10", "Object translated in parent space");
		assert.equal(parentTx.z.toFixed(), "-10", "Object not translated Z in parent space");

		moveTool.setCoordinateSystem(CoordinateSystem.Screen);
		moveTool.move(10, 0, 0);	// Move in screen space (right in pixels)
		var screenTx = node.getWorldPosition();
		...
		assert.equal(screenTx.y.toFixed(), "10", "Object not translated Y in screen space");
		assert.equal(screenTx.z.toFixed(), "-10", "Object not translated Z in screen space");
	});
