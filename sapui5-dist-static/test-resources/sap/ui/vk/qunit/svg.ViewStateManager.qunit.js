	...
	QUnit.test("SVG ViewStateManager", function(assert) {
		...
		function validateOpacityAttribute(node, checkOpacity) {
			node.traverse(function(child) {
				if (child.type !== "Group") {
					if (child.domRef) {
						var opacityAttrib = child.domRef.getAttribute("opacity");
						assert.strictEqual(opacityAttrib, checkOpacity, "Geometry node opacity attribute");
					}
				}
			});
		}
		...
		function testOpacity(vsm, nodes) {
			...
			validateOpacityAttribute(childNode, rootOpacity.toString());
			// assert nodeF property is unchanged
			assert.strictEqual(childNode.opacity, childOpacity, "Node opacity property");
		...
		function testHighlightColor(vsm) {
			var originalColor = abgrToColor(0xC00000FF);
		...
		function testSelection(vsm, node) {
			...
			vsm.setSelectionState(node, true, false);
			node.traverse(function(child) {
			...
			vsm.setSelectionState(node, false, false);
			node.traverse(function(child) {
			...
			vsm.setSelectionState(node, false, true);
			node.traverse(function(child) {
			...
			vsm.setSelectionState(nodes[ 3 ], false, false);
			...
			vsm.setSelectionState(nodes[ 3 ], true, false);
