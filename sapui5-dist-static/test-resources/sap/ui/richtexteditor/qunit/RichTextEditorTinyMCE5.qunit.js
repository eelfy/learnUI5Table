...
sap.ui.define([
	...
	var fnTestToolbarGroups = function(assert) {
		...
		oRichTextEditor1.attachReady(function() {
			assert.equal(jQuery(".tox-tbtn[aria-label='Justify']").length, 1, "Text justify button should be there");
			done();
			assert.equal(jQuery(".tox-tbtn[aria-label='Emoticons']").length, 0, "The smiley button should not be there");
			done();
			...
			setTimeout(function() {
				setTimeout(function() {
					oRichTextEditor1._pTinyMCEInitialized.then(function() {
						assert.equal(jQuery(".tox-tbtn[aria-label='Emoticons']").length, 1, "The smiley button should now be there");
	...
	var fnTestPluginsNonDefault = function(assert) {
		...
		oRichTextEditor1.attachReady(function() {
			...
			assert.equal(jQuery("#myRTE1").find(".tox-tbtn[aria-label='Insert/edit media']").length, 1, "Media button shown");
			done();
			assert.equal(jQuery("#myRTE1").find(".tox-tbtn[aria-label='Table']").length, 1, "Table button shown");
			done();
	...
	var fnTestTinyMCEPopupsForEditorInPopup = function(assert) {
		...
		oDialog.attachAfterOpen(function() {
			...
			setTimeout(function() {
				...
				var oButton = jQuery(".tox-tbtn[aria-label='Insert/edit media']");
				oButton.trigger("click");
				setTimeout(function() {
					var oTextField = jQuery(".tox-form .tox-textfield")[0];
					oTextField.focus();
					setTimeout(function() {
	...
	QUnit.test("Toolbar groups setter", function (assert) {
		var done = assert.async(51),
		oRichTextEditor = new RichTextEditor("myRTE5", {
			editorType: sap.ui.richtexteditor.EditorType.TinyMCE5,
			width: "1400px"
		});
		var checkForVisibility = function (sAriaLabel, bVisible, sBtnClass) {
			assert.strictEqual(document.querySelectorAll("#myRTE5 .tox-editor-container " + sBtnClass + "[aria-label='" + sAriaLabel + "']").length, bVisible ? 1 : 0, sAriaLabel + " visible");

			done();
		};

		oRichTextEditor.placeAt("content");
		var aAriaLabels = ['Fonts', 'Font sizes', //  Group Font
		...
		var aAriaSpecialLabels = ['Text color', 'Background color'];
		...
		oRichTextEditor.attachReady(function (oEvent) {
			for (var i = 0; i < aAriaLabels.length; i++) {
				checkForVisibility(aAriaLabels[i], true, ".tox-tbtn");
			}
			for (var o = 0; o < aAriaSpecialLabels.length; o++) {
				checkForVisibility(aAriaSpecialLabels[o], true, ".tox-split-button");
			}
			fnShowGroups(false);
			setTimeout(function () {
				oRichTextEditor._pTinyMCEInitialized.then(function () {
					...
					for (var o = 0; o < aAriaSpecialLabels.length; o++) {
						checkForVisibility(aAriaSpecialLabels[o], false, ".tox-split-button");
					}
					oRichTextEditor.destroy();
					done();
				});
	...
	QUnit.test("_createButtonRowsTinyMCE", function(assert) {
		var done = assert.async(),
			oRichTextEditor = new RichTextEditor({editorType: sap.ui.richtexteditor.EditorType.TinyMCE5}),oCreateButtonsSpy = sinon.spy(oRichTextEditor, "_createButtonRowsTinyMCE");
		...
		oRichTextEditor.attachReady(function() {
			...
			assert.strictEqual(oCreateButtonsSpy.returnValues[0],"bold italic underline strikethrough | cut copy paste | bullist numlist outdent indent | alignleft aligncenter alignright alignjustify | ", "should return correct values");
	...
	QUnit.test("_shouldLoadTinyMCE", function(assert) {
		...
		var done = assert.async(),
			oRichTextEditor = new RichTextEditor({editorType: sap.ui.richtexteditor.EditorType.TinyMCE5});
	...
	QUnit.test("_getLanguageTinyMCE", function(assert) {
		// arrange
		setLanguage("test_test");
		var done = assert.async(),
			oRichTextEditor = new RichTextEditor({editorType: sap.ui.richtexteditor.EditorType.TinyMCE5});
		...
		oRichTextEditor.attachReady(function() {
			// assert
			assert.strictEqual(oRichTextEditor._getLanguageTinyMCE(), "en", "_getLanguageTinyMCE should return 'en' as a default language");
			assert.strictEqual(oRichTextEditor.getNativeApi().settings.language, "en", "The TinyMCE  default language should be 'en'.");

			// act
			setLanguage("eo");

			// assert
			assert.strictEqual(oRichTextEditor._getLanguageTinyMCE(), "eo", "_getLanguageTinyMCE should return TinyMCE5 supported language 'eo'");

			// act
			setLanguage("ml_MN");

			// assert
			assert.strictEqual(oRichTextEditor._getLanguageTinyMCE(), "ml", "_getLanguageTinyMCE should return 'ml' as a language, because there doesn't have ml_MN");

			// act
			setLanguage("es_ES");

			// assert
			assert.strictEqual(oRichTextEditor._getLanguageTinyMCE(), "es_ES", "_getLanguageTinyMCE should return TinyMCE5 supported language 'es_ES'");

			// act
			setLanguage("it_IT");

			// assert
			assert.strictEqual(oRichTextEditor._getLanguageTinyMCE(), "it_IT", "_getLanguageTinyMCE should return TinyMCE5 supported language 'it_IT'");

			// act
			setLanguage("ro_RO");

			// assert
			assert.strictEqual(oRichTextEditor._getLanguageTinyMCE(), "ro_RO", "_getLanguageTinyMCE should return TinyMCE5 supported language 'ro_RO'");
	...
	QUnit.test("SetCustomToolbar(false)", function(assert) {
		...
		oRichTextEditor.attachReady(function() {
			// assert
			assert.ok(document.querySelectorAll("#myRTE5 .tox-toolbar-overlord").length, "There should be a native TinyMCE toolbar");
	...
	QUnit.test("SetCustomToolbar(true) after creating a RichTextEditor with no custom toolbar", function(assert) {
		...
		oRichTextEditor.attachReady(function(oEvent) {
			oRichTextEditor._pTinyMCEInitialized.then(function() {
				...
				assert.ok(document.querySelectorAll("#myRTE5_1 .tox-editor-header").length, "There should be a native TinyMCE toolbar");
	...
	QUnit.test("RTE + Custom toolbar- XML view", function (assert) {
		//Setup
		var done = assert.async();
		var sXMLView = '<mvc:View xmlns="sap.m" xmlns:mvc="sap.ui.core.mvc" xmlns:rte="sap.ui.richtexteditor"><rte:RichTextEditor id="myRTEXMLFragment" editorType="TinyMCE5" customToolbar="true" /></mvc:View>';
