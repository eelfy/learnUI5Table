...
sap.ui.define([
	...
	"sap/ui/Device"
], function(RichTextEditor, Button, Event, waitForThemeApplied, JSONModel, Device) {
	"use strict";

	QUnit.config.testTimeout = 6000;
	var oResourceBundle = sap.ui.getCore().getLibraryResourceBundle("sap.ui.richtexteditor");

	QUnit.test("Warmup environment before executing tests", function (assert) {
		// arrange
		var done = assert.async();

		sap.ui.require(["sap/m/Button"], function (Btn) {
			...
			oRichTextEditor.attachReady(function () {
				assert.ok(Btn, "sap.m. Library should have been loaded.");
	...
	QUnit.test("Custom Toolbar is resolved properly", function (assert) {
		// arrange
		var done = assert.async(),
	...
	// The test is skipped in Internet Explorer, because the tested functionality
	// does not work in that browser due to TinyMCE issue.
	if (!Device.browser.msie) {
		QUnit.test("RTE with custom toolbar should not steal the focus", function (assert) {
			// arrange
			var done = assert.async(),
				oButton = new Button(),
				oRichTextEditor = new RichTextEditor({
		...
		oRichTextEditor.attachReady(function() {
			...
			setTimeout(function() {
				oRichTextEditor._pTinyMCEInitialized.then(function() {
					...
					assert.equal(sap.ui.getCore().byId(sAriaLabelId2).getText(), oResourceBundle.getText("FONT_COLOR_TEXT"), "Then correct invisible text set was for font color");
					assert.equal(sap.ui.getCore().byId(sAriaLabelId3).getText(), oResourceBundle.getText("BACKGROUND_COLOR_TEXT"), "Then correct invisible text set was for background color");
