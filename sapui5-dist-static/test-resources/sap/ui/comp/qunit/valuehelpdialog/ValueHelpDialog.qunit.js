...
sap.ui.define([
	...
	QUnit.test("_initializeTable successfully sets the tooltip of the columns in case of i18n binding", function (assert) {
		// Arrange
		var oColumnModel = new JSONModel({
				cols: [{
					...
					template: "prop2",
					label: "Not translated Label",
					tooltip: "Not translated Tooltip"
				}]
			}),
			oI18NModel = new JSONModel({
				label: "Translated Label",
				tooltip: "Translated Tooltip"
			}),
			done = assert.async(),
			aColumns, oColumn1, oColumn2;
		this.oValueHelpDialog.setModel(oColumnModel, "columns");
		this.oValueHelpDialog.setModel(oI18NModel, "i18n");
		...
		this.oValueHelpDialog.getTableAsync().then(function (oTable) {
			...
			oColumn1 = aColumns[0];
			oColumn2 = aColumns[1];
			assert.equal(oColumn1.getLabel().getText(), "Translated Label", "Label should be translated");
			assert.equal(oColumn1.getTooltip(), "Translated Tooltip", "Tooltip should be translated");
			assert.equal(oColumn2.getLabel().getText(), "Not translated Label", "Label should be set");
			assert.equal(oColumn2.getTooltip(), "Not translated Tooltip", "Tooltip should be set");
