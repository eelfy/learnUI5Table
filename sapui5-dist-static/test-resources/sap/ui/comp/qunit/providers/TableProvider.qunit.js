...
(function() {
	...
	sap.ui.define([
		...
		QUnit.test("_initialiseMetadata shall consider LineItem annotation and mark some fields as initially visible", function(assert) {
			...
			assert.strictEqual(aResult[0].importance, "High", "High importance is set correctly");
			assert.ok(this.oTableProvider.isLineItemDefaultImportance(aResult[0]), "Line item default importance");
			...
			assert.strictEqual(aResult[1].importance, "Medium", "Medium importance is set correctly");
			assert.notOk(this.oTableProvider.isLineItemDefaultImportance(aResult[1]), "Not line item default importance");
			// device
			...
			assert.strictEqual(aResult[0].isInitiallyVisible, true);
			assert.strictEqual(aResult[0].inResult, undefined, "inResult != true, since no PresentationVariant annotation exists");
			assert.strictEqual(aResult[0].width, "20rem", "correct width set");
			assert.strictEqual(aResult[1].name, "test2");
			assert.strictEqual(aResult[1].isInitiallyVisible, Device.system.desktop || Device.system.tablet); // only true on desktop or tablet
			assert.strictEqual(aResult[1].width, "150px", "correct width set");
			// device
			assert.strictEqual(aResult[2].name, "test3");
			assert.strictEqual(aResult[2].isInitiallyVisible, true);
			assert.strictEqual(aResult[2].width, "25em", "correct width set");
