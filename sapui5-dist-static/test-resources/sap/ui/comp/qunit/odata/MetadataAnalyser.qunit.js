...
sap.ui.define([
	...
	QUnit.test("_enrichValueHelpAnnotation adds correct high importance field", function(assert) {
		// Arrange
		var oMetadataAnalyser = this.oMetadataAnalyser,
			oField = {name: "SAKAN"},
			aFields = [oField],
			oAnnotation = {
				annotation: {
		...
		oMetadataAnalyser._oMetaModel = sinon.createStubInstance(ODataMetaModel);
		oMetadataAnalyser._oMetaModel.getODataEntitySet.returns(true);
		sinon.stub(oMetadataAnalyser, "getFieldsByEntitySetName").returns(aFields);

		// Act
		oMetadataAnalyser._enrichValueHelpAnnotation(oAnnotation, "Dummy Name");

		// Assert
		assert.ok(oAnnotation.aHighImportanceFields, "Annotation has high importance field");
		assert.equal(oAnnotation.aHighImportanceFields[0].name, aFields[0].name, "The high importance field is correct");
	});

	QUnit.test("_enrichValueHelpAnnotation adds correct constant param", function(assert) {
		// Arrange
		var oMetadataAnalyser = this.oMetadataAnalyser,
			oField = {name: "SAKAN"},
			aFields = [oField],
		...
		// Assert
		assert.equal(oAnnotation.constParams.CONTINENT, sConstValue, "The constant parameter is correct");
	});
	...
	QUnit.test("getDeprecationCodeFieldName shall return the name of the Deprecation property/field", function(assert) {
		var sDeprecationCodeField = null;

		var oField = {
			"name": "DeprecationCode",
			"type": "Edm.String",
			"maxLength": "1",
			"extensions": [{}],
			"sap:label": "Currency Status",
			"com.sap.vocabularies.Common.v1.Label": {
				"String": "Currency Status"
			},
			"com.sap.vocabularies.CodeList.v1.IsConfigurationDeprecationCode": {
				"Bool": "true"
			},
			"fieldLabel": "Currency Status",
			...
			"fullName": "ZEPM_C_SALESORDERITEMQUERY_CDS.Currency/DeprecationCode",
			"visible": "true",
			"hiddenFilter": "false",
			"filterable": "true",
		...
		sDeprecationCodeField = this.oMetadataAnalyser.getDeprecationCodeFieldName([oField]);
		assert.strictEqual(sDeprecationCodeField, "DeprecationCode");
	...
	QUnit.test("updateDataFieldDefault shall update the field metadata with values from DataFieldDefault annotation (Criticality - similar to LineItem)", function(assert) {
		...
		assert.ok(oFieldMetadata.width);
		assert.strictEqual(oFieldMetadata.width, "7em", "DataFieldDefault gets the correct width value from annotation");
		assert.ok(oFieldMetadata.importance);
		assert.strictEqual(oFieldMetadata.importance, "Low", "field importance set via DataFieldDefault annotation");
	});

	QUnit.test("Invalid width (%) should not be updated", function(assert) {
		var oFieldMetadata;
		...
		this.oMetadataAnalyser.updateDataFieldDefault(oFieldMetadata);
		assert.strictEqual(oFieldMetadata.label, "Label via DataFieldDefault");
		assert.notOk(oFieldMetadata.width);
		assert.strictEqual(oFieldMetadata.width, null, "% width will not be set");
	});

	QUnit.test("Invalid width (auto) should not be updated", function(assert) {
		var oFieldMetadata;
		...
		assert.strictEqual(oFieldMetadata.label, "Label via DataFieldDefault");
		assert.notOk(oFieldMetadata.width);
		assert.strictEqual(oFieldMetadata.width, null, "auto width will not be set");
	});
	...
	QUnit.test("getLineItemAnnotation shall return the matching LineItem annotation with fields and labels and Criticality", function(assert) {
		...
		assert.strictEqual(oAnnotation.width["CompanyCode"], "7rem", "Width annotation correctly set");
	...
	QUnit.test("Checking _getFilterableFieldsFromEntityDefinition with hidden fields", function(assert) {
		...
		var oFilterData,
			...
			oEntityDef = {
				property: [oPropertyA, oPropertyB]
			};
		this.oMetadataAnalyser._oMetaModel = {};

		// Act
		oFilterData = this.oMetadataAnalyser._getFilterableFieldsFromEntityDefinition(oEntityDef, 'sPropertyName', 'sEntitySetName');

		// Assert
		assert.ok(oFilterData.hiddenFields.length);
		assert.equal(oFilterData.hiddenFields[0].name, oPropertyB.name, "The hidden field is added");
	});
	...
	QUnit.test("isPotentiallySensitive", function (assert) {
		// Arrange
		var oPropertyWithIsPotentiallySensitive = {
				"com.sap.vocabularies.PersonalData.v1.IsPotentiallySensitive": {
					Bool: "true"
				}
			},
			oPropertyNoIsPotentiallySensitive = {
			};

		// Assert
		assert.ok(MetadataAnalyser.isPotentiallySensitive(oPropertyWithIsPotentiallySensitive), "should return true if IsPotentiallySensitive");
		assert.ok(!MetadataAnalyser.isPotentiallySensitive(oPropertyNoIsPotentiallySensitive), "should return false if not IsPotentiallySensitive");
	});
	...
	QUnit.test("it should interpret the com.sap.vocabularies.UI.v1.Importance OData v4 annotation", function(assert) {
		...
		// assert
		assert.ok(MetadataAnalyser.hasImportance(oProperty));
	});
