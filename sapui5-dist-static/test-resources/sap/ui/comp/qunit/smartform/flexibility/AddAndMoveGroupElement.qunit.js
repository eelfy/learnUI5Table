...
sap.ui.define([
	...
	QUnit.module("AddField (legacy) and AddFields edge case tests", {
		...
		QUnit.test("applyChange - positive test (legacy)", function(assert) {
			...
			delete this.oChangeAddField.getDefinition().content.field.selector;
			this.oChangeAddField.getDefinition().content.field.id = "addedFieldId";

			AddFieldChangeHandler.applyChange(this.oChangeAddField, this.oGroup, {modifier: JsControlTreeModifier, view: this.oView, appComponent: this.oAppComponent});
		...
		QUnit.test("applyChange - positive test with field selector (legacy)", function(assert) {
			var oAddGroupElementSpy = sandbox.spy(this.oGroup, "insertGroupElement");
			var oChangeDefinition = this.oChangeAddField.getDefinition();
			assert.equal(oChangeDefinition.texts.fieldLabel.value, "the field label");
			assert.equal(oChangeDefinition.content.field.jsType, "sap.ui.commons.TextView");
			assert.equal(oChangeDefinition.content.field.value, "BindingPath1");
			assert.equal(oChangeDefinition.content.field.valueProperty, "text");
			assert.equal(oChangeDefinition.content.field.selector.id, "addedFieldId");
			assert.equal(oChangeDefinition.content.field.index, 0);
			assert.equal(oChangeDefinition.content.field.entitySet,"testEntitySet1");

			AddFieldChangeHandler.applyChange(this.oChangeAddField, this.oGroup, {modifier: JsControlTreeModifier, appComponent: this.oAppComponent, view: this.oView});
		...
		QUnit.test("applyChange- addFields - add smart field to js control tree with duplicated id", function(assert) {
			...
			AddFieldsChangeHandler.applyChange(this.oChangeAddFields, this.oGroup, {modifier: JsControlTreeModifier, appComponent: this.oAppComponent, view: this.oView});
		...
		QUnit.test("applyChange - add smart field with duplicated id to xml tree", function(assert) {
			...
			AddFieldsChangeHandler.applyChange(this.oChangeAddFields, oXmlSmartFormGroup, {modifier: XmlTreeModifier, view: oXmlDocument, appComponent: this.oAppComponent});
