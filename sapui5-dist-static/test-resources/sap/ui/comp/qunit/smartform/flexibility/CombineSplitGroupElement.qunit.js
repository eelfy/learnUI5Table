...
sap.ui.define([
	...
	function confirmGroupElementsAreCombined(sExpectedLabel, oUiComponent, oViewAfterAction, assert) {
	...
	function confirmCombinedGroupElementsAreSplited(aExpectedLabel, oUiComponent, oViewAfterAction, assert) {
	...
	elementActionTest("Checking the combine action for group elements with special labels", {
		...
		afterAction: confirmGroupElementsAreCombined.bind(undefined, "Product ID/field2/Product Type"),
		afterUndo: confirmCombinedGroupElementsAreSplited.bind(undefined, ["Product ID", "field2", "Product Type"]),
		afterRedo: confirmGroupElementsAreCombined.bind(undefined, "Product ID/field2/Product Type")
	});
	...
	elementActionTest("Checking the combine action for group elements using horizontalLayout", {
		...
		afterAction: confirmGroupElementsAreCombined.bind(undefined, "field1/field2/field3"),
		afterUndo: confirmCombinedGroupElementsAreSplited.bind(undefined, ["field1", "field2", "field3"]),
		afterRedo: confirmGroupElementsAreCombined.bind(undefined, "field1/field2/field3")
	});

	//Split GroupElement
	function confirmGroupElementsAreSplited(oUiComponent,oViewAfterAction, assert) {
	...
	function confirmSplitedGroupElementsAreCombined(oUiComponent, oViewAfterAction, assert) {
	...
	elementActionTest("Checking the split action for group elements", {
		...
		afterAction: confirmGroupElementsAreSplited,
		afterUndo: confirmSplitedGroupElementsAreCombined,
		afterRedo: confirmGroupElementsAreSplited
	});
	...
	QUnit.module("Given a Smartform with a Group containing 4 GroupElements", {
		beforeEach: function() {
			this.oSmartField1 = new SmartField("smartField1");
			this.oSmartField2 = new SmartField("smartField2");
			this.oSmartField3 = new SmartField("smartField3");
			this.oSmartField4 = new SmartField("smartField4");
			this.oSmartField5 = new SmartField("smartField5");
			this.oSmartField6 = new SmartField("smartField6");
			this.oGroupElement1 = new GroupElement("groupElement1", {
				elements: [this.oSmartField1]
			});
			this.oGroupElement2 = new GroupElement("groupElement2", {
				elements: [this.oSmartField2]
			});
			this.oGroupElement3 = new GroupElement("groupElement3", {
				elements: [this.oSmartField3]
			});
			this.oGroupElement4 = new GroupElement("groupElement4", {
				elements: [this.oSmartField4]
			});
			this.oGroupElement5 = new GroupElement("groupElement5", {
				elements: [this.oSmartField5, this.oSmartField6]
			});
			this.oGroup = new Group("group", {
	...
	QUnit.module("CombineFields - given RTL mode for CombineFields", {
		beforeEach: function() {
			...
			return ChangesWriteAPI.create({
				changeSpecificData: {
					...
					combineElementIds: ["FirstName", "LastName"],
					sourceControlId: "LastName"
				},
		...
		QUnit.test("when creating the change", function(assert) {
			...
			assert.equal(
				this.oChange.getDependentControl("sourceControl", {modifier: JsControlTreeModifier, appComponent: this.oAppComponent}).getId(),
				this.oLastName.getId(),
				"source control is part of dependentSelector"
			);

			assert.deepEqual(
				this.oChange.getDependentControl("combinedFields", {modifier: JsControlTreeModifier, appComponent: this.oAppComponent}).map(function(oField) {
					return oField.getId();
				}),
		...
		QUnit.test("when field labels are combined in LTR mode", function(assert) {
			CombineFieldsChangeHandler.applyChange(this.oChange, this.oGroup, {modifier: JsControlTreeModifier, appComponent: this.oAppComponent});
			assert.strictEqual(
