...
sap.ui.define([
	...
	QUnit.test("internal Form", function(assert) {
		...
		oStub.withArgs("sap/ui/layout/form/ResponsiveGridLayout").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("checkButton", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/m/OverflowToolbar").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("editTogglable", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/m/Button").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("Title with customToolbar", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/m/Title").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("addGroup should call _getSmartFieldsByGroup", function(assert) {
		// Arrange
		var oGroup = new Group();
		sinon.spy(oSmartForm, "_getSmartFieldsByGroup");

		// Act
		oSmartForm.addGroup(oGroup);

		// Assert
		assert.equal(oSmartForm._getSmartFieldsByGroup.calledOnce, true, "_getSmartFieldsByGroupName is called");

	});

	QUnit.test("addGroup should override setImportance method of the SmartField", function(assert) {
		// Arrange
		var oField1 = new SmartField(),
			oField2 = new Input(),
			oField3 = new SmartField(),
			oGroupElement1 = new GroupElement({label: "Label", elements: [oField1]}),
			oGroupElement2 = new GroupElement({label: "Label", elements: [oField2, oField3]}),
			oGroup = new Group({groupElements: [oGroupElement1, oGroupElement2]}),
			fnSpyField1SetImportance = sinon.spy(oField1, "_setVisibilityBasedOnImportance"),
			fnSpyField3SetImportance = sinon.spy(oField3, "_setVisibilityBasedOnImportance");
			oSmartForm.addGroup(oGroup);

			// Act
			oField1.setImportance("High");
			oField3.setImportance("Low");

			// Assert
			assert.equal(fnSpyField1SetImportance.calledOnce, true, "_setVisibilityBasedOnImportance is called on first SmartField");
			assert.equal(fnSpyField3SetImportance.calledOnce, true, "_setVisibilityBasedOnImportance is called on second SmartField");
	});
	...
	QUnit.test("UseHorizontalLayout without Layout", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/ui/layout/form/ResponsiveLayout").returns(undefined); // to force async. loading
		oStub.withArgs("sap/ui/layout/ResponsiveFlowLayoutData").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("ColumnLayout", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/ui/layout/form/ColumnLayout").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("expandable Form", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/m/Panel").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.test("_getSmartFieldsByGroup", function(assert) {
		// Arrange
		var aSmartFields,
			oField1 = new SmartField(),
			oField2 = new SmartField(),
			oGroupElement1 = new GroupElement({label: "Label", elements: [oField1]}),
			oGroupElement2 = new GroupElement({label: "Label", elements: [oField2]}),
			oGroup = new Group({groupElements: [oGroupElement1, oGroupElement2]});
			oSmartForm.addGroup(oGroup);

			// Act
			aSmartFields = oSmartForm._getSmartFieldsByGroup(oGroup);

			// Assert
			assert.equal(aSmartFields.length, 2, "2 SmartFields are found");
	});
	...
	QUnit.test("no editToggled when error", function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/m/MessageBox").returns(undefined); // to force async. loading
		oStub.callThrough();
	...
	QUnit.module("Async validation mode", {
		beforeEach: function () {
			...
			this.oSmartForm = new SmartForm({
				validationMode: SmartFormValidationMode.Async,
				editable: true,
				groups: [
					new Group({
						groupElements: [
	...
	QUnit.test("check method is async in this mode", function (assert) {
		// Arrange
		var fnDone = assert.async();
		assert.expect(3);

		// Act
		this.oSmartForm.check().then(function (aErrorFields) {
			// Assert
			assert.ok(true, "Check method returns a promise");
			assert.ok(Array.isArray(aErrorFields), "Error fields array returned");
			assert.strictEqual(aErrorFields.length, 0, "There are no fields with errors currently");
			fnDone();
		});
	});

	QUnit.test("check method returns invalid error fields", function (assert) {
		// Arrange
		var fnDone = assert.async();
		assert.expect(2);

		sinon.stub(this.oSmartField, "checkValuesValidity").returns(new Promise(function (fnResolve, fnReject) {
			fnReject(); // We simulate value error by returning a rejected promise
		}));

		// Act
		this.oSmartForm.check().then(function (aErrorFields) {
			// Assert
			assert.strictEqual(aErrorFields.length, 1, "There is one field in error state");
			assert.strictEqual(aErrorFields[0], this.oSmartField.getId(), "The returned error field ID matches");
			fnDone();
		}.bind(this));
	});

	QUnit.test("_checkClientErrorAsync returns a promise", function (assert) {
		// Arrange
		var fnDone = assert.async();
		assert.expect(1);

		// Act
		this.oSmartForm._checkClientErrorAsync({}).then(function (aErrorFields) {
			// Assert
			assert.ok(true, "Promise returned from method");
			fnDone();
		});
	});

	QUnit.test("_checkClientErrorAsync({considerOnlyVisible: true|false})", function (assert) {
		// Arrange
		var fnDone = assert.async(),
			oNonVisibleSmartField = new SmartField({visible: false});

		assert.expect(3);

		// Mock the _checkValuesValidity method
		sinon.stub(oNonVisibleSmartField, "checkValuesValidity").returns(new Promise(function (fnResolve, fnReject) {
			fnReject(); // We simulate field in error state with a rejected promise
		}));

		// Add a non-visible SmartField to the form
		this.oSmartForm.getGroups()[0].addGroupElement(new GroupElement({
			elements: [
				oNonVisibleSmartField
			]
		}));

		// Act
		this.oSmartForm._checkClientErrorAsync({considerOnlyVisible: true}).then(function (aErrorFields) {
			// Assert
			assert.strictEqual(aErrorFields.length, 0, "No visible fields are in error state");

			// Act
			this.oSmartForm._checkClientErrorAsync({considerOnlyVisible: false}).then(function (aErrorFields) {
				// Assert
				assert.strictEqual(aErrorFields.length, 1, "There is one field in error state");
				assert.strictEqual(aErrorFields[0], oNonVisibleSmartField.getId(),
					"The returned ID matches the non visible field");
				fnDone();
			});
		}.bind(this));
	...
	QUnit.test("_checkClientErrorAsync({handleSuccess: true})", function (assert) {
		// Arrange
		var fnDone = assert.async(),
			oSpy = this.spy(this.oSmartField, "checkValuesValidity");

		assert.expect(2);

		// Act
		this.oSmartForm._checkClientErrorAsync({handleSuccess: true}).then(function () {
			// Assert
			assert.strictEqual(oSpy.callCount, 1, "Method called once");
			assert.ok(oSpy.calledWithExactly({handleSuccess: true}), "Method called with the correct parameters");
			fnDone();
		});
	});

	QUnit.test("_checkClientErrorAsync setBusy", function (assert) {
		// Arrange
		var fnDone = assert.async(),
			oSpy = this.spy(this.oSmartForm, "setBusy");

		assert.expect(3);

		// Act
		this.oSmartForm._checkClientErrorAsync({}).then(function () {
			// Assert
			assert.strictEqual(oSpy.callCount, 2, "Method called twice");
			assert.ok(oSpy.getCall(0).calledWithExactly(true), "First call set the form to busy");
			assert.ok(oSpy.getCall(1).calledWithExactly(false), "Second call removed the busy state");
			fnDone();
		});
	});

	QUnit.test("setEditable async success", function (assert) {
		// Arrange
		var fnDone = assert.async();
		assert.expect(2);

		this.oSmartForm.attachEditToggled(function () {
			assert.ok(true, "Event fired");
			assert.strictEqual(this.oSmartForm.getEditable(), false, "Property is still falsy after the validation");
			fnDone();
		}.bind(this));
		...
		sinon.stub(this.oSmartField, "checkValuesValidity").returns(new Promise(function (fnResolve, fnReject) {
			setTimeout(function () {
				fnResolve();
			}, 100 /* We give 100 milliseconds artificial validation delay */);
		}));
	...
	QUnit.test("setEditable async fail", function (assert) {
		// Arrange
		var fnDone = assert.async();
		assert.expect(2);

		this.oSmartForm.attachEditToggled(function () {
			// If the event fires we will also violate the assert.expect number of assertions
			assert.ok(false, "Event should not be fired");
		});

		sinon.stub(this.oSmartField, "checkValuesValidity").returns(new Promise(function (fnResolve, fnReject) {
			fnReject(); // We simulate failed validation check
		}));

		this.oSmartForm._displayError = function () {
			SmartForm.prototype._displayError.apply(this.oSmartForm, arguments);

			// Assert
			assert.ok(true, "The _displayError method is called");

			// We give one tick for the property to be set
			setTimeout(function () {
				InstanceManager.closeAllDialogs(); // Close the MessageBox error dialog
				assert.strictEqual(this.oSmartForm.getEditable(), true, "Property did not change");
				fnDone();
			}.bind(this), 0);
		}.bind(this);
		...
		this.oSmartForm.attachEditToggled(function () {
			assert.ok(false, "Test should fail if the event is fired.");
		});
