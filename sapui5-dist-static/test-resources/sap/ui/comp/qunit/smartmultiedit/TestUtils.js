sap.ui.define([
	...
	TestUtils.FIELDS_POOL = {
		"Float":{
			...
			nullable: false,
			valueHelp: true,
			singleContextSelectKeys: "12.32139",
			multiContextSelectKeys: ["13.91234", "12.32139"],
			newItemIndex: "3",
			newItemKey: "12.32139",
		...
		"Int64":{
			...
			nullable: true,
			valueHelp: true,
			singleContextSelectKeys: "11",
			multiContextSelectKeys: ["11"],
			newItemIndex: "3",
			newItemKey: "11",
			useApplyToEmptyOnly: false
		},
		...
		"Decimal": {
			...
			nullable: true,
			valueHelp: true,
			multiContextSelectKeys: ["3456789.87", "1230.01"],
	...
	TestUtils.mServers = Object.create(null);

	TestUtils.createMockServer = function (sMetadataPath, sServiceURL) {
		sMetadataPath = sMetadataPath || "metadata.xml";
		sServiceURL = sServiceURL || "smartmultiedit.Employees/";
		var oUriParameters = jQuery.sap.getUriParameters();

		var oServer = this.mServers[sMetadataPath];
		if (!oServer) {
			oServer = this.mServers[sMetadataPath] = new MockServer({
				rootUri: sServiceURL
			});
	...
	TestUtils.mModels = Object.create(null);

	TestUtils.createDataModel = function (sServiceURL, sEntitySet) {
		sServiceURL = sServiceURL || "smartmultiedit.Employees";
		var sReadPath = sEntitySet ? sEntitySet : "Employees",
			 oModel = this.mModels[sServiceURL];
		if (!oModel) {
			oModel = this.mModels[sServiceURL] = new ODataModel(sServiceURL);
			oModel.setUseBatch(false); // So far parallelism doesn't work well enough for batches in qunit
	...
	TestUtils.createContainer = function (aContexts, oDataModel, i18nModel, oForm, sEntitySet) {
		// Two different contexts
		oForm = oForm || TestUtils.createSmartForm();
		var oContainer = new Container({
				id: "multiEditContainer",
				entitySet: sEntitySet ? sEntitySet : "Employees",
				layout: oForm,
				contexts: aContexts
			});
