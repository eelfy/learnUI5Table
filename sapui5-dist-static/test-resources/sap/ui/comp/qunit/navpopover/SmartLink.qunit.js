...
sap.ui.define([
			...
			FakeFlpConnector.enableFakeConnector({
				TestObject: {
					links: [
						{
							action: "displayFactSheet",
							intent: window.location.href + "#/dummyLink1",
							text: "Fact Sheet"
						}, {
							action: "anyAction",
							intent: window.location.href + "#/dummyLink2",
							text: "List"
						}
					]
				},
	...
	QUnit.test("beforeNavigationCallback of SemanticObjectController" , function(assert) {
		var done = assert.async();
		var fnBeforeNavigationCallback = function() {
		...
		var oSemanticObjectController = this.oSmartLink.getSemanticObjectController(),
			oNavigationPopoverHandler = this.oSmartLink.getNavigationPopoverHandler();

		var fnFireInnerNavigateSpySmartLink = sinon.spy(this.oSmartLink, "fireInnerNavigate"),
			fnFireNavigateSpySemanticObjectController = sinon.spy(oSemanticObjectController, "fireNavigate"),
			fnFireInnerNavigateSpyNavigationPopoverHandler = sinon.spy(oNavigationPopoverHandler, "fireInnerNavigate");

		// check if SmartLink, SemanticObjectController and NavigationPopoverHandler have the same beforeNavigationCallback function
		assert.equal(this.oSmartLink.getBeforeNavigationCallback(), oSemanticObjectController.getBeforeNavigationCallback());
		assert.equal(this.oSmartLink.getBeforeNavigationCallback(), oNavigationPopoverHandler.getBeforeNavigationCallback());

		this.oSmartLink._createNavigationPopoverHandler().openPopover().then(function() {
			assert.ok(oNavigationPopoverHandler._oPopover);
			oNavigationPopoverHandler._oPopover._getContentContainer().fireNavigate({
				text: "Fact Sheet",
				href: window.location.href + "#/dummyLink1"
			});
			// check before promise is resolved
			assert.ok(fnFireInnerNavigateSpySmartLink.notCalled);
			assert.ok(fnFireNavigateSpySemanticObjectController.notCalled);
			assert.ok(fnFireInnerNavigateSpyNavigationPopoverHandler.notCalled);
			setTimeout(function() {
				assert.ok(fnFireInnerNavigateSpySmartLink.calledOnce);
				assert.ok(fnFireNavigateSpySemanticObjectController.calledOnce);
				assert.ok(fnFireInnerNavigateSpyNavigationPopoverHandler.calledOnce);
				assert.ok(window.location.href.includes("#/dummyLink1"));
				done();
			}, 2000);
		});
	});

	QUnit.test("prevent binding propagation from FLP", function(assert) {
		var done = assert.async();
		this.oSmartLink = new SmartLink({
			semanticObject: "TestObject2"
		});
		this.oSmartLink.placeAt("qunit-fixture");
		...
		var oNavigationPopoverHandler = this.oSmartLink.getNavigationPopoverHandler();
		this.oSmartLink._createNavigationPopoverHandler().openPopover().then(function() {
			var oPopover = oNavigationPopoverHandler._oPopover;
			assert.ok(oPopover);
			var aAvailableActions = oPopover._getContentContainer().getAvailableActions();
