...
sap.ui.define([
	...
	QUnit.test("applyChange: all invisible -> apply corrupt change", function(assert) {
		return ChangesWriteAPI.create({
			...
			delete oChange.getDefinition().content;
			AddLink.applyChange(oChange, this.oNavigationContainerInvisible, {
	...
	QUnit.test("applyChange: all visible -> set first visible", function(assert) {
		return ChangesWriteAPI.create({
			...
			AddLink.applyChange(oChange, this.oNavigationContainerVisible, {
				modifier: JsControlTreeModifier
			});
	...
	QUnit.test("completeChangeContent: negative cases", function(assert) {
		...
		.catch(function(oError) {
			assert.equal(oError.message, "In oSpecificChangeInfo.content.select attribute should be 'true'", "an error was thrown");
	...
	QUnit.test("applyChange: all visible -> apply corrupt change", function(assert) {
		return ChangesWriteAPI.create({
			...
			delete oChange.getDefinition().content;
			RemoveLink.applyChange(oChange, this.oNavigationContainerVisible, {
				modifier: JsControlTreeModifier
			});
	...
	QUnit.test("applyChange: all invisible -> set all invisible", function(assert) {
		return ChangesWriteAPI.create({
			...
			RemoveLink.applyChange(oChange, this.oNavigationContainerInvisible, {
				modifier: JsControlTreeModifier
			});
	...
	QUnit.test("completeChangeContent: negative cases", function(assert) {
		...
		.catch(function(oError) {
			assert.equal(oError.message, "In oSpecificChangeInfo.content.select attribute should be 'false'", "an error was thrown");
	...
	QUnit.test("applyChange: Â, E, Û, O -> A, Ê, U, Ô", function(assert) {
		...
		return Promise.all([
			...
			AddLink.applyChange(aChanges[0], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
			RemoveLink.applyChange(aChanges[2], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
			AddLink.applyChange(aChanges[1], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
			RemoveLink.applyChange(aChanges[3], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
	...
	QUnit.test("applyChange: Â, Ê, Û, Ô -> Â, E, Û, O -> A, Ê, U, Ô", function(assert) {
		...
		return Promise.all([
			...
			AddLink.applyChange(aChanges[2], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
			RemoveLink.applyChange(aChanges[3], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
			AddLink.applyChange(aChanges[4], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
			RemoveLink.applyChange(aChanges[5], oNavigationContainer, {
				modifier: JsControlTreeModifier
			});
