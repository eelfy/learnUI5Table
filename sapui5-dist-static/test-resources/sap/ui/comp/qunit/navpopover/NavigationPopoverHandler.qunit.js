...
sap.ui.define([
	...
	QUnit.module("sap.ui.comp.navpopover.NavigationPopoverHandler - beforeNavigationCallback", {
		beforeEach: function() {
			this.aIntents = [
	...
	QUnit.test("open messageToast before direct navigation", function(assert) {
		FakeFlpConnector.enableFakeConnector({
			TestObjectDirect: {
				links: [
					{
						action: "anyAction#1",
						intent: this.aIntents[0],
						text: "Link#1"
					}
				]
			}
		...
		var fnFireInnerNavigateSpy = sinon.spy(this.oNavigationPopoverHandler, "fireInnerNavigate");

		assert.ok(fnMessageToastSpy.notCalled);
		assert.ok(fnFireInnerNavigateSpy.notCalled);

		// act
		this.oNavigationPopoverHandler.openPopover().then(function() {
			// assert
			assert.ok(!this.oNavigationPopoverHandler._oPopover);
	...
	QUnit.test("wait 1000ms before popover navigation", function(assert) {
		FakeFlpConnector.enableFakeConnector({
			TestObjectDirect: {
				links: [
					{
		...
		this.oNavigationPopoverHandler.openPopover().then(function() {
			// assert
			assert.ok(this.oNavigationPopoverHandler._oPopover);
			this.oNavigationPopoverHandler._oPopover._getContentContainer().fireNavigate({
				text: "Link#1",
				href: this.aIntents[0]
			});
	...
	QUnit.test("wait 1000ms before no popover navigation", function(assert) {
		FakeFlpConnector.enableFakeConnector({
			TestObjectDirect: {
				links: [
					{
