...
sap.ui.define([
	"sap/ui/core/Control", "sap/ui/comp/util/DateTimeUtil", "sap/ui/comp/smartfilterbar/FilterProvider", "sap/ui/comp/filterbar/VariantConverterTo", "sap/ui/comp/filterbar/VariantConverterFrom", "sap/m/DatePicker", "sap/m/MultiComboBox", "sap/m/MultiInput", "sap/m/DateRangeSelection", "sap/ui/model/json/JSONModel"
], function(Control, DateTimeUtil, FilterProvider, VariantConverterTo, VariantConverterFrom, DatePicker, MultiComboBox, MultiInput, DateRangeSelection, JSONModel) {
	"use strict";
	...
	QUnit.test("Checking the _addRangesAccordingMetaData method with 'Edm.Time'", function(assert) {

		var oContent = {};
		var sFieldName = "TimeType";
		var oFilterMetaData = {
			fieldName: sFieldName,
			type: "Edm.Time"
		};
		var aRanges = [
			{ Sign: "I", Option: "EQ", Low: "PT09H04M33S", High: null }
		];

		this.oVariantConverter._addRangesAccordingMetaData(oContent, null, oFilterMetaData, aRanges, {}, "");
		assert.ok(oContent[sFieldName].ranges[0].value1 instanceof Date);
	});
	...
	QUnit.test("Checking single DateTime field", function(assert) {
					...
					"Ranges": [
						{
							"Sign": "I",
							"Option": "EQ",
							"Low": "2016-07-08T00:00:00.000+01:00",
							"High": null
						}
					]
				}
		...
		var oSuiteFormatWithMinusHours = {
			"SelectionVariantID": "",
			"SelectOptions": [
				{
					"PropertyName": "BLDAT",
		...
		sSuiteVariant = JSON.stringify(oSuiteYearMonthDateFormat);
		oResultResult = this.oVariantConverter.convert(sSuiteVariant, oFilterBar);
		assert.ok(oResultResult);
		assert.ok(oResultResult.payload);
		oResult = JSON.parse(oResultResult.payload);
		assert.equal(oResult.BLDAT, "2016-07-08T00:00:00.000Z");

		sSuiteVariant = JSON.stringify(oSuiteFormatWithPlusHours);
		oResultResult = this.oVariantConverter.convert(sSuiteVariant, oFilterBar);
		assert.ok(oResultResult);
		assert.ok(oResultResult.payload);
		oResult = JSON.parse(oResultResult.payload);
		...
		sSuiteVariant = JSON.stringify(oSuiteFormatWithMinusHours);
		oResultResult = this.oVariantConverter.convert(sSuiteVariant, oFilterBar);
		assert.ok(oResultResult);
		assert.ok(oResultResult.payload);
		oResult = JSON.parse(oResultResult.payload);
	...
	QUnit.test("Checking Edm.String filters DateRangeSelection", function(assert) {
		var oControl = new DateRangeSelection();
		var bUTC = false;
		var oMetaData = [
			{
		...
		var oExpectedResult = {
			...
			"BLDAT2": {
				"low": "2020-02-10T00:00:00.000Z",
				"high": "2020-03-10T00:00:00.000Z"
			}
		};
	...
	QUnit.test("Checking simple filter in strict/non strict mode", function(assert) {
		...
		assert.ok(oResult);
		assert.ok(oResult.payload);
		assert.deepEqual({ "NAME": "11"}, JSON.parse(oResult.payload));
	...
	QUnit.test("_createDateObject", function (assert) {
		// Arrange
		var oVCF = new VariantConverterFrom(),
			oDate;

		function fnGetDateObject (oDate) {
			return {
		...
		// Act
		oDate = oVCF._createDateObject("0001-01-01T00:00:00.000");

		// Assert
		assert.deepEqual(
			fnGetDateObject(oDate),
			{
				year: 1,
				month: 0,
				day: 1,
			...
			"Resulting date object should match for 0001-01-01T00:00:00.000"
		);

		// Act
		oDate = oVCF._createDateObject("0100-01-01T00:00:00.000");
		...
		assert.deepEqual(
			...
			"Resulting date object should match for 0100-01-01T00:00:00.000"
		);

		// Act
		oDate = oVCF._createDateObject("2015-02-28T23:00:00.000");
		...
		assert.deepEqual(
			...
			"Resulting date object should match for 2015-02-28T23:00:00.000"
		);
	});
