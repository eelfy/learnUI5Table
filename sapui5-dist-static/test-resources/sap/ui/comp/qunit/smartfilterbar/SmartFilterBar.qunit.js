...
sap.ui.define([
	...
	"sap/m/CheckBox", "sap/m/TimePicker", "sap/m/DateTimePicker", "sap/ui/core/ValueState", "sap/ui/model/json/JSONModel", "sap/ui/model/json/JSONPropertyBinding", "sap/m/DatePicker", "sap/m/SearchField", "sap/m/MessageBox", "sap/m/DateRangeSelection", "sap/m/MultiInput", "sap/m/Select", 'sap/ui/core/Item', 'sap/ui/comp/smartfilterbar/FilterProvider', 'sap/m/MultiComboBox', "sap/ui/comp/providers/ValueListProvider",
	"sap/ui/comp/odata/ODataModelUtil"
            ...
            CheckBox, TimePicker, DateTimePicker, ValueState, JSONModel, JSONPropertyBinding, DatePicker, SearchField, MessageBox, DateRangeSelection, MultiInput, Select, Item, FilterProvider, MultiComboBox, ValueListProvider,
			ODataModelUtil
) {
	"use strict";
	...
	QUnit.test("valueListAnnotationLoaded event is fired when SmartFilterBar field with VHD annotations loaded", function(assert) {
		// Arrange
		var done = assert.async(),
		...
		// Assert
		this.oSmartFilter.attachEvent("valueListAnnotationLoaded", function(oControlElement){
	...
	QUnit.test("check _createFieldInAdvancedArea overwrite correct label tooltip", function(assert) {
		...
		var aContent = this.oSmartFilter._mAdvancedAreaFilter[FilterBar.INTERNAL_GROUP].items[0].container.getContent();
		 ...
		 // Assert
		assert.equal(aContent[0].getTooltip(), oControl.getTooltip());

		// Cleanup
		oControl.destroy();
	});

	QUnit.test("check _createFieldInAdvancedArea overwrite correct label tooltip with undefined quickinfo", function(assert) {
		...
		// Assert
		assert.equal(aContent[0].getTooltip(), oField.label);
	...
	QUnit.test("check _onMetadataInitialised", function(assert) {
		...
		sinon.stub(ODataModelUtil, "_getFlexRuntimeInfoAPI").returns(Promise.resolve(oFlexRuntimeInfoAPI));
	...
	QUnit.test("check create ValueList with delayed association", function(assert) {
		...
		// Assert
		assert.ok(oFireAssociateValueListsSpy.calledWith(FilterProvider.ASSOCIATE_VALUE_LISTS));
	...
	QUnit.test("refreshFiltersCount should update adapt filters count", function (assert) {
		...
		var fnDone = assert.async(),
			...
			oFilterItem2 = new FilterGroupItem("filterItem2", {
				groupName: "GROUP1",
				control: new Input("DATE_INTERVAL"),
				label: "filterItem2",
				name: "DATE_INTERVAL",
				visible: true,
				visibleInAdvancedArea: true
			...
			oFilterItem3 = new FilterGroupItem("filterItem3", {
				groupName: "GROUP1",
				control: new MultiInput("STRING_INOUT"),
				label: "filterItem3",
				name: "STRING_INOUT",
				visible: true,
				visibleInAdvancedArea: true
		...
		this.oSmartFilter.setBasicSearch(new Input("BASIC"));
		...
		this.oSmartFilter.getControlByKey("STRING").setValue("One");
		this.oSmartFilter.getControlByKey("DATE_INTERVAL").setValue("Two");
		this.oSmartFilter.getControlByKey("STRING_INOUT").addValidator(oValueListProvider.fakeValidator.bind(oValueListProvider));
		this.oSmartFilter.getControlByKey("STRING_INOUT").setValue("Three");
		this.oSmartFilter.getControlByKey("STRING_INOUT")._validateCurrentText();
