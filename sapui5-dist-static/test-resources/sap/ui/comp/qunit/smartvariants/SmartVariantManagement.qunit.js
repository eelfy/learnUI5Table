...
sap.ui.define([], function() {
	...
	sap.ui.require([
		...
		function createVariantStub(sName, sText, oContent, mContexts) {
			var oObj = {
					content: null,
					executeOnSelection: false,
					getId: function() { return sName; },
					isUserDependent: function() { return true; },
					...
					isLabelReadOnly: function() { return false; },
					getFavorite: function() { return true; },
					getDefinition: function() { return null; },
					getText: function() { return sText; },
		...
		function createVariantStubs(bAppVariant) {
			var mChanges = {};
			var oVariant1 = createVariantStub("1", "ONE", undefined, { role: []});
			var oVariant2 = createVariantStub("2", "TWO", {data: "TEST", standardvariant: true}, { role: []});
			...
			if (bAppVariant) {
				...
				mChanges.standardVariant = createVariantStub("*standard*", "Standard");
		...
		var oStubIsContextSharingEnabled;
		...
		function stubIsContextSharingEnabled (bValue) {
			if (!(FlexFeaturesAPI && FlexFeaturesAPI.isContextSharingEnabled && FlexFeaturesAPI.isContextSharingEnabled.displayName === "isContextSharingEnabled")) {
				oStubIsContextSharingEnabled  = sinon.stub(FlexFeaturesAPI, "isContextSharingEnabled");
			}
			oStubIsContextSharingEnabled.returns(Promise.resolve(bValue));
		}
		...
		QUnit.test("check _dataReceived", function(assert) {
			...
			// Move the following part at the end of the async processing
			this.oVariantManagement._loadFlex().then(function() {

				this.oVariantManagement._dataReceived(mChanges, false, true, false, this.oVariantManagement._getControlWrapper(this.oVariantManagement._oPersoControl));
		...
		QUnit.test("check _dataReceived with isVariantPersonalizationEnabled=false", function(assert) {
			...
			assert.ok(this.oVariantManagement._oFlLibrary);
			// Move the following part at the end of the async processing
			this.oVariantManagement._oFlLibrary.then(function() {

				//stubGetChange(oControl, mChanges);
				this.oVariantManagement._dataReceived(mChanges, false, false, false, this.oVariantManagement._getControlWrapper(this.oVariantManagement._oPersoControl));
		...
		QUnit.test("check fireSelect with standard variant", function(assert) {
			...
			sinon.stub(this.oVariantManagement, "_getVariantById").returns({ getContent: function() { return {};}});
		...
		QUnit.test("check fireSave: _save", function(assert) {
			return this.oVariantManagement._loadFlex().then(function() {
				sinon.stub(this.oVariantManagement, "_addFavoriteToFlex");

				createControlInfo(this.oVariantManagement);
				stubSave(fakeResolved());

				assert.ok(!this.oVariantManagement._aFavorites);
				this.oVariantManagement._setFavorite("*standard*");
				assert.ok(this.oVariantManagement._aFavorites);
				assert.equal(this.oVariantManagement._aFavorites.length, 1);

				this.oVariantManagement._save();
				assert.equal(this.oVariantManagement._aFavorites.length, 0);

				assert.ok(oStubSave.calledOnce);

				this.oVariantManagement._addFavoriteToFlex.restore();
			}.bind(this));
		});
		...
		QUnit.test("check fireManage: _setExecuteOnSelections", function(assert) {
			...
			sinon.stub(this.oVariantManagement, "_setExecuteOnSelectOnStandardVariant");


			var mChanges = createVariantStubs();
			this.oVariantManagement._createVariantEntries(mChanges);

			this.oVariantManagement._getChange = function(sKey) {
				return this._getVariantById(sKey);
			};
			...
			assert.ok(this.oVariantManagement._setExecuteOnSelectOnStandardVariant.calledOnce);
			assert.ok(this.oVariantManagement._setExecuteOnSelectOnStandardVariant.withArgs(true));
		});
		...
		QUnit.test("check getCurrentVariantId not STANDARD", function(assert) {
			...
			sinon.stub(this.oVariantManagement,"_getExecuteOnSelectOnStandardVariant").returns(true);
		...
		QUnit.test("check setCurrentVariantId not STANDARD", function(assert) {
			...
			sinon.stub(this.oVariantManagement, "_getDefaultVariantKey").returns("3");
			sinon.stub(this.oVariantManagement, "_getExecuteOnSelectOnStandardVariant").returns("");
		...
		QUnit.test("check initialise with page variant: apply", function(assert) {
			...
			sinon.stub(this.oVariantManagement, "_getDefaultVariantKey").returns("");
			sinon.stub(this.oVariantManagement, "_getExecuteOnSelectOnStandardVariant").returns("");
		...
		QUnit.test("check search on standard variant", function(assert) {
			...
			sinon.stub(this.oVariantManagement, "_getCurrentVariantId").returns("VENDOR_VAR");
		...
		QUnit.test("check setEntitySet and model", function(assert) {
			...
			var oModel = {
					...
					getMetaModel: function() {
						return {
							getProperty: function(s) { return null; },
							getODataEntitySet: function(s) { return null; },
							loaded: function() {
								return oPromise;
							}
						};
			...
			oModel.bLoadMetadataAsync = true;
		...
		QUnit.test("check mixed sync/async fetchVariant for new variant", function(assert) {
			...
			this.oVariantManagement._oFlLibrary.then(function() {
				...
				oAfterSavePromise.then(function() {
					...
					var oChange = this.oVariantManagement._getVariantById(oVariantItem.getKey());
					assert.ok(oChange);
		...
		QUnit.test("check adaptationInfo default variant", function(assert) {

			this.oVariantManagement.addVariantItem(new VariantItem({key: "1", favorite: true}));
			this.oVariantManagement.addVariantItem(new VariantItem({key: "2", favorite: true}));

			assert.ok(this.oVariantManagement.getDefaultVariantKey(), "");

			var oAdaptionInfo = {
			                 defaultVariant: "2",
			                 addFavorites: {}
			};
			this.oVariantManagement.setProperty("adaptationInfo", oAdaptionInfo);

			assert.ok(this.oVariantManagement.getDefaultVariantKey(), "2");

			oAdaptionInfo = {
			             defaultVariant: this.oVariantManagement.STANDARDVARIANTKEY,
			             addFavorites: {}
			};
			this.oVariantManagement.setProperty("adaptationInfo", oAdaptionInfo);

			assert.ok(this.oVariantManagement.getDefaultVariantKey(), this.oVariantManagement.STANDARDVARIANTKEY);
		});
		...
		QUnit.test("Checking the apply automatic text for standard", function(assert) {
			...
			var oCheckBox = aCells[sap.ui.comp.variants.VariantManagement.EXEC_COLUMN];
			assert.ok(oCheckBox.isA("sap.m.CheckBox"));
			assert.ok(oCheckBox.getEnabled());
			assert.ok(!oCheckBox.getSelected());

			this.oVariantManagement.oManagementDialog.close();
			//------
			...
			var oText = aCells[sap.ui.comp.variants.VariantManagement.EXEC_COLUMN];
			assert.ok(oText.isA("sap.m.Text"));
			assert.equal(oText.getText(), "TEST");
		});
		...
		QUnit.module("sap.ui.comp.smartvariants.SmartVariantManagement Ui Adaptation", {
			beforeEach: function() {
				...
				stubIsVariantSharingEnabled(true);
				stubIsVariantPersonalizationEnabled(true);
				stubIsContextSharingEnabled(true);
				sinon.stub(this.oVariantManagement, "_isFlexSupported").returns(true);
			...
			afterEach: function() {
				...
				FlexFeaturesAPI.isContextSharingEnabled.restore();
			}
		});
				...
				this.oVariantManagement._loadFlex().then(function() {
					...
					oInitializedPromise.then(function() {
						var fDataCallBack = function(oData) {
							...
							assert.equal(oData.executeOnSelect, false);
						...
						var fOpened = function(oEvent) {
							var oTarget = this.oVariantManagement.oSaveSave.getFocusDomRef();
							assert.ok(oTarget);
						...
						this.oVariantManagement.oSaveDialog.attachAfterOpen(fOpened);

						this.oVariantManagement.openSaveAsDialogForKeyUser("STYLECLASS", fDataCallBack);
						this.oVariantManagement.oInputName.setValue("New Variant");
			...
			return new Promise(function(fResolve) {
				...
				this.oVariantManagement._loadFlex().then(function() {
					...
					oInitializedPromise.then(function() {
						...
						var fOpened = function(oEvent) {
							var oButton = sap.ui.getCore().byId(this.oVariantManagement.getId() + "-variantcancel");
							assert.ok(oButton);
				...
				this.oVariantManagement._loadFlex().then(function() {
					...
					oInitializedPromise.then(function() {
						var fDataCallBack = function(oData) {
							...
							assert.deepEqual(oData["1"], { favorite: false, name: "New Title 1", "deleted": true});
							assert.deepEqual(oData["2"], { executeOnSelect: false, contexts: { role: ["OME", "TWO", "THREE"]}});
							assert.deepEqual(oData["3"], { favorite: true, name: "New Title 3", executeOnSelect: true, "deleted": true});
						...
						var fOpened = function(oEvent) {
							...
							assert.ok(!aColumns[sap.ui.comp.variants.VariantManagement.RESTRICT_COLUMN].getVisible());

							this.oVariantManagement._sDefaultChanges = "3";
							this.oVariantManagement._mFavoriteChanges = { "1" : false, "3" : true};
							this.oVariantManagement._mTitleChanges = { "1": "New Title 1", "3": "New Title 3"};
							this.oVariantManagement._mExecuteOnSelectChanges = {"2": false, "3": true};
							this.oVariantManagement._mDeletedChanges = {"1": true, "3": true};
							this.oVariantManagement._mContextInfoChanges = {"2" : { role: ["OME", "TWO", "THREE"]}};


							var oTarget = this.oVariantManagement.oManagementSave.getFocusDomRef();
							assert.ok(oTarget);
							QUnitUtils.triggerTouchEvent("tap", oTarget, {
								srcControl: null
							});
						...
						this.oVariantManagement.oManagementDialog.attachAfterOpen(fOpened);

						this.oVariantManagement.openManageViewsDialogForKeyUser("STYLECLASS", fDataCallBack);
				...
				this.oVariantManagement._loadFlex().then(function() {
					...
					oInitializedPromise.then(function() {
						...
						var fOpened = function(oEvent) {
							...
							var oButton = sap.ui.getCore().byId(this.oVariantManagement.getId() + "-managementcancel");
							assert.ok(oButton);
							var oTarget = oButton.getFocusDomRef();
							QUnitUtils.triggerTouchEvent("tap", oTarget, {
								srcControl: null
				...
				this.oVariantManagement._loadFlex().then(function() {
					...
					oInitializedPromise.then(function() {
						var fDataCallBack = function(oData) {
							...
							assert.deepEqual(oData["1"], { favorite: false, name: "New Title 1", "deleted": true, contexts: { role: []}});
							assert.deepEqual(oData["2"], { executeOnSelect: false, contexts: { role:["OME", "TWO", "THREE"]}});
							assert.deepEqual(oData["3"], { favorite: true, name: "New Title 3", executeOnSelect: true, "deleted": true});
						...
						var fOpenedRolesDialog = function(oEvent) {

							var oButton = sap.ui.getCore().byId(this.oVariantManagement.getId() + "-rolesave");
							assert.ok(oButton);
							var oTarget = oButton.getFocusDomRef();
							QUnitUtils.triggerTouchEvent("tap", oTarget, {
								srcControl: null
							...
							oTarget = this.oVariantManagement.oManagementSave.getFocusDomRef();
							assert.ok(oTarget);
							QUnitUtils.triggerTouchEvent("tap", oTarget, {
								srcControl: null
							});
						...
						var fOpened = function(oEvent) {
							...
							assert.ok(!this.oVariantManagement._oRolesDialog);
							this.oVariantManagement._createRolesDialog();
							assert.ok(this.oVariantManagement._oRolesDialog);
							this.oVariantManagement._oRolesDialog.attachAfterOpen(fOpenedRolesDialog);
							...
							this.oVariantManagement._openRolesDialog(oItemStub);
						}.bind(this);
						...
						this.oVariantManagement.openManageViewsDialogForKeyUser("STYLECLASS", fDataCallBack, oContextSharingComponentContainer);
