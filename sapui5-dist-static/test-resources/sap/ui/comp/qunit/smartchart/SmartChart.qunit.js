...
sap.ui.define([
	...
	sap.ui.require([
		...
		QUnit.test("Check that patch for DimeasureController#setVisibleMeasures is up to date", function(assert){

			var fnOriginal = Object.assign(Chart.prototype.setVisibleMeasures);

			var fnPatchBase = function(aMeasureNames, bSuppressInvalidate) {
			...
			var sFnDisabled = fnPatchBase.toString().replace(/\s/g, '');
			var sFnEnabled = fnOriginal.toString().replace(/\s/g, '');

			assert.deepEqual(sFnDisabled, sFnEnabled, "The inner chart 'setVisibleMeasures' is not patched");
		});

		QUnit.test("PersoController event update execution - never keep patched 'setVisibleMeasures'", function(assert) {
			this.oSmartChart._oChart = new Chart({
			...
			var oInnerChart = this.oSmartChart._oChart;
			var fnOriginal = Object.assign(Chart.prototype.setVisibleMeasures);

			//1) Inner chart should not be patched initially
			assert.equal(oInnerChart.setVisibleMeasures.toString(), fnOriginal.toString(), "The inner chart 'setVisibleMeasures' is not patched");

			//Simulate perso controller updates
			this.oSmartChart._oPersController.fireBeforePotentialTableChange();

			//2) Inner chart should be patched during p13n Controller execution
			assert.notEqual(oInnerChart.setVisibleMeasures.toString(), fnOriginal.toString(), "Method is sequentially patched");

			//Simulate perso controller finish updates
			this.oSmartChart._oPersController.fireAfterPotentialTableChange();

			//3) Controller processing done --> inner Chart#setVisibleMeasures is original again
			assert.equal(oInnerChart.setVisibleMeasures.toString(), fnOriginal.toString(), "The inner chart 'setVisibleMeasures' is not patched");
		});
		...
		QUnit.test("test flex container height fix for Chrome and Safari browsers", function(assert) {

			//Arrange
			this.oSmartChart._oChart = new Chart();
			sap.ui.Device.browser.safari = true;
			...
			//Assert
			assert.equal(this.oSmartChart.hasStyleClass("sapUiCompSmartChartInnerChartHeight"), false, "StyleClass correctly not set when browser is not Safari or Chrome");
			...
			this.oSmartChart._oChart = new Chart();
			sap.ui.Device.browser.safari = false;
			sap.ui.Device.browser.chrome = true;
		...
		QUnit.test("_drillDown", function(assert) {
			...
			assert.ok(oSpyAddContentMiddle.calledOnce, "AddContentMiddle of bar correctly called");
			...
			Bar.prototype.addContentMiddle.restore();
			ResponsivePopover.prototype.addContent.restore();
		...
		QUnit.test("test noData property", function(assert) {
			...
			assert.equal(this.oSmartChart.getNoData(), oRb.getText("CHART_NO_RESULTS"), "The initial text for the noData property is correct in case there is a filterbar");
		...
		QUnit.test("UiState before binding with variant change", function(assert) {
			...
			//Act
			this.oSmartChart.setUiStateAsVariant(oTempUiState);
			assert.ok(this.oSmartChart._oUiStateRequestAtLeast == oTempUiState.getPresentationVariant().RequestAtLeast, "RequestAtLeast fields were cached");
