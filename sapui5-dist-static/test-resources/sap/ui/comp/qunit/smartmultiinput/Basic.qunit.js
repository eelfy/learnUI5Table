...
sap.ui.define("test.sap.ui.comp.qunit.smartmultiinput.Basic", [
...
], function (SmartMultiInput, TestUtils, ODataControlFactory, Context, ValueState, BindingMode, Token, Text, DisplayBehaviour,
			 createAndAppendDiv) {
	"use strict";
	...
	QUnit.test("_checkFieldGroups", function (assert) {
		var fnDone = assert.async();
		...
		TestUtils.createDataModel().then(function (oModel) {
			...
			this.oSmartMultiInput.attachEventOnce("innerControlsCreated", function () {
				...
				this.oSmartMultiInput._checkFieldGroups();
	...
	QUnit.test("getTextSeparator", function (assert) {
		this.oSmartMultiInput.placeAt("content");
		sap.ui.getCore().applyChanges();
	...
	QUnit.test("_bindMultiComboBoxTwoWay", function (assert) {
		...
		TestUtils.createDataModel().then(function (oModel) {
			...
			function asserts() {
				...
				assert.ok(that.oSmartMultiInput.oLocalContext.length, "Local Context array is populated after creation of token");
				that.oSmartMultiInput._oMultiComboBox.fireSelectionChange({
					selected: false,
					changedItem: that.oSmartMultiInput._oMultiComboBox.getItems()[2]
				});
				assert.notOk(that.oSmartMultiInput.oLocalContext.length, "Local Context array is empty after deletion of tokens");
