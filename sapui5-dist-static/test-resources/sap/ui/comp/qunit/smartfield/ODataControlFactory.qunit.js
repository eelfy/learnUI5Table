...
sap.ui.define([
	...
	var fCreateSmartFieldStub = function(oModel) {
		var oSmartField = {
			...
			attachInitialise: function(fSucc) {
	...
	QUnit.test("updateControl", function(assert) {
		...
		oParent.getFirstInnerControl = function() {
			return {};
		};
	...
	QUnit.test("_createEdmString with default textInEditModeSource", function (assert) {
		...
		var done = assert.async();
		var oParent = fCreateSmartFieldStub(this.oModel);
		this.stub(oParent, "data").returns("ValueList");
		this.stub(oParent, "isPropertyInitial").returns(true);
		this.stub(oParent, "getMode").returns("edit");
		var oFactory = new ODataControlFactory(this.oModel, oParent, {
			entitySet: "Project",
			path: "Name"
		});
		this.spy(oFactory.oTextArrangementDelegate, "checkRequiredMetadata");
		this.spy(oParent, "_getComputedTextInEditModeSource");
		...
		oFactory.bind().finally(function() {
			...
			assert.ok(oParent._getComputedTextInEditModeSource.calledOnce);
	...
	QUnit.test("_createEdmString returning a comp combo box", function(assert) {
		...
		oFactory.bind().finally(function() {
			...
			assert.ok(oControl.control.getBindingInfo("enteredValue"));
	...
	QUnit.test("_createEdmString returning a combo box if fixedValueListValidationEnabled is true", function(assert) {
		...
		oParent.getFixedValueListValidationEnabled = function(){
			return true;
		};
	...
	QUnit.test("check if additional handler that fetches once the id and description on model context change and working properly", function(assert) {
        ...
        oParent.getMode = function () {
            return "edit"; // This test is relevant for edit mode
		};
        ...
        oFactory.bind().finally(function() {
			...
			oFactory._handleModelContextChangeEvent();
	...
	QUnit.test("TextArrangementDelegate instance should be prevented from fetching ID/Description if there is no default data model set on the SmartField instance", function(assert) {
		// arrange
		var oModel,
		oSmartField = fCreateSmartFieldStub(oModel),
		oFactory = new ODataControlFactory(oModel, oSmartField, {
			entitySet: "Project",
			path: "Description"
		}),
		oTextArrangementDelegateSpy = this.stub(oFactory.oTextArrangementDelegate, "fetchIDAndDescriptionCollectionIfRequired").returns(undefined);

		// act
		oFactory._handleModelContextChangeEvent();

		// assert
		assert.equal(oTextArrangementDelegateSpy.callCount, 0);

		// clean
		oTextArrangementDelegateSpy.reset();

		// act
		oSmartField.setModel({});
		oFactory._handleModelContextChangeEvent();

		// assert
		assert.equal(oTextArrangementDelegateSpy.callCount, 1);
	});
	...
	QUnit.test("_createObjectStatus shall return sap.m.ObjectStatus with criticality bound to formatter", function(assert) {
		...
		assert.equal(oInfo.formatter(), "sap-icon://message-warning");
	...
	QUnit.test("_createObjectStatus shall return sap.m.ObjectStatus with criticality bound to path", function(assert) {
		...
		oFactory.bind().finally(function() {
			...
			oInfo = oControl.control.getBindingInfo("icon");
			assert.equal(oInfo.formatter(2), "sap-icon://message-warning");
	...
	QUnit.test("_checkComboBox returning a combo box for config", function(assert) {
		...
		oFactory.bind().finally(function() {
			...
			assert.ok(oControl.control.getBindingInfo("enteredValue"));
			assert.equal(oControl.control.getId(), oParent.getId() + "-comboBoxEdit");
			assert.equal(oControl.params.getValue, "getEnteredValue");
	...
	QUnit.test("_createEdmNumeric it should create a combo box control", function(assert) {
		...
		oSmartFieldStub.getMode = function(){
			return "edit";
		};
	...
	QUnit.test("_createEdmNumeric it should create a combo box control if fixedValueListValidationEnabled is true", function(assert) {
		...
		oSmartFieldStub.getFixedValueListValidationEnabled = function(){
			return true;
		};
	...
	QUnit.test("_createEdmUOM", function(assert) {
		...
		oFactory.bind().finally(function() {
			...
			assert.ok( includes(oMeasureField.getAriaLabelledBy(), oText.getId() + "-text"), "The measure field " +
				"should be labelled by the currency field if the currency field is in display mode");
	...
	QUnit.test("_getCreator", function(assert) {
		...
		oParent.getMode = SmartField.prototype.getMode.bind(oParent);
	...
	QUnit.test("return false in case of no bound field control property", function(assert) {
		...
		this.stub(oParent, "getBinding").returns(undefined);
		// act
	...
	QUnit.test("return true in case of any bound field control property", function(assert) {
		...
		this.stub(oParent, "getBinding").returns({});
		// act
		var result = oFactory._hasBoundFieldControl();
	...
	QUnit.test("fireInitialise only once when triggerCreationOfControls call twice with _bReBindOnVL = true", function(assert) {
		...
		oFactory._bReBindOnVL = true;
		this.stub(oParent, "getBinding")
			.onCall(0).returns(undefined)
			.onCall(1).returns(undefined)
			.onCall(2).returns(undefined)
			.onCall(3).returns({});
		this.stub(oFactory, "_addLabelAndQuickInfo");
		var oParentSpy = this.spy(oParent, "fireInitialise");
		...
		// assert
		assert.equal(oParentSpy.callCount, 1);
	});

	QUnit.test("fireInitialise not fire when triggerCreationOfControls call twice with _bReBindOnVL = true and FieldControl is bound", function(assert) {
		...
		// assert
		assert.equal(oParentSpy.callCount, 0);
	});

	QUnit.test("fireInitialise fire twice when triggerCreationOfControls call twice with _bReBindOnVL = false and FieldControl is bound", function(assert) {
		...
		// assert
		assert.equal(oParentSpy.callCount, 2);
	});

	QUnit.test("fireInitialise fire twice when triggerCreationOfControls call twice with _bReBindOnVL = false and FieldControl is not bound", function(assert) {
