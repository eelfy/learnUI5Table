...
sap.ui.define([
	...
	QUnit.test("_getEditable - 'updatable-path' navigation property", function(assert) {
		var oSet = this.oHelper.oMeta.getODataEntitySet("Project"),
			oType,
			oResult,
			oMeta;
		...
		this.oHelper.getProperty(oMeta);
		oMeta.navigationPath = "to_Amount"; // Mock navigation path
		oMeta.toRoleAssociationEndMultiplicity = "1"; // Mock to association multiplicity

		// Act
		oResult = this.oMetaData._getEditable(oMeta);

		// Assert
		assert.strictEqual(oResult.path()[0], "to_Amount/EntityUpdatable_FC",
			"Binding path points to the navigation entity type");

		// Arrange
		oMeta.toRoleAssociationEndMultiplicity = "*"; // Mock to association multiplicity

		// Act
		oResult = this.oMetaData._getEditable(oMeta);

		// Assert -> this is left for legacy reasons. It should be incorrect but already application are
		// counting on that behavior.
		assert.strictEqual(oResult.path()[0], "EntityUpdatable_FC",
			"Binding path points to the main entity type");
	});
