...
sap.ui.define([
	...
	QUnit.test("set FieldGroupIds", function(assert) {
		...
		this.oSmartField._oFactory = sinon.createStubInstance(ODataControlFactory);
		this.oSmartField._oFactory.bPending = false;
		this.oSmartField._oFactory.createControl.returns({control: oEditControl});
		this.oSmartField._oFactory.getMetaData.returns({
			property: {
				property: true
			}
		});
	...
	QUnit.test("textDirection set to RTL and after that LTR", function (assert) {
	...
	QUnit.test("textDirection set to LTR and after that RTL", function (assert) {
	...
	QUnit.test("Set the FieldGroups when BindingContext gets available", function (assert) {
		...
		// Act
		// We assume that _toggleInnerControlIfRequired will aways be invoked on bindingContext change.
		// Otherwise the SF won't work properly.
		this.oSmartField._toggleInnerControlIfRequired();
		this.oSmartField._checkFieldGroups();
