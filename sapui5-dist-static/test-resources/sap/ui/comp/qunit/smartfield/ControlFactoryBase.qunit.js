...
sap.ui.define([
	...
	QUnit.test("_getPreventInitialDataFetchInValueHelpDialog should return correct value based on ControlConfiguration and ValueList annotation", function (assert) {
		...
		var bResult;
		var oFactory, oParent;
		var oGetConfigurationStub = this.stub();
		oParent = {
			getBindingContext: function() {
				return {
					sPath: "/Project(id1='71' id2='abcd')"
			...
			getConfiguration: oGetConfigurationStub
		};
		oFactory = new ControlFactoryBase(this.oModel, oParent, {
			entitySet: "Project",
			path: "Description"
		...
		// Act
		oGetConfigurationStub.returns(undefined);
		bResult = oFactory._getPreventInitialDataFetchInVHDialog({});

		// Assert
		assert.notOk(bResult, "_getPreventInitialDataFetchInVHDialog should return 'false' if no Configuration and FetchValues annotation exists");
		...
		// Assert
		assert.notOk(bResult, "_getPreventInitialDataFetchInVHDialog should return 'false' if annotation value is '1' and there is no Configuration");
		...
		// Assert
		assert.ok(bResult, "_getPreventInitialDataFetchInVHDialog should return 'true' if Annotation value is '2' and there is no Configuration");

		// Act
		oGetConfigurationStub.returns({
			getPreventInitialDataFetchInValueHelpDialog: this.stub().returns(true),
			isPropertyInitial: this.stub().returns(false)
		});
		bResult = oFactory._getPreventInitialDataFetchInVHDialog({
			"com.sap.vocabularies.Common.v1.ValueList": {
				"FetchValues": {
		...
		// Assert
		assert.ok(bResult, "_getPreventInitialDataFetchInVHDialog should return 'true' if it is set through Configuration and ignore the annotation");

		// Cleanup
		oFactory.destroy();
	});

	QUnit.test("addValueHelp - historyEnabled true", function(assert) {
		var oMeta, oMetaData, oSet, oType, oFactory, fOnValueListChange, oAnnotation, oControl = {
			setModel: function () {
		...
		assert.equal(oFactory._aProviders[1]._fieldHistoryEnabled, true);
	...
	QUnit.test("addValueHelp - historyEnabled true and property is not initial", function(assert) {
		var oMeta, oMetaData, oSet, oType, oFactory, fOnValueListChange, oAnnotation, oControl = {
			setModel: function () {
	...
	QUnit.test("addValueHelp - historyEnabled false", function(assert) {
		var oMeta, oMetaData, oSet, oType, oFactory, fOnValueListChange, oAnnotation, oControl = {
			setModel: function () {
		...
		assert.equal(oFactory._aProviders[1]._fieldHistoryEnabled, false);
