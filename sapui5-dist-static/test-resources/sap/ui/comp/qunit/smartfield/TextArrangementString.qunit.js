...
sap.ui.define([
	...
	QUnit.test("it should call the OData model read method", function(assert) {
		...
		// arrange
		var sDisplayBehaviour = "idAndDescription";
		...
		var oFormatOptions = {
			textArrangement: sDisplayBehaviour
		};
	...
	QUnit.test("TextArrangement.prototype.parseDescriptionOnly should always resolve", function(assert) {
		...
		var done = assert.async(),
			...
			oType = new TextArrangementString(oFormatOptions, null, oSettings);

		// act
		oType.parseDescriptionOnly("", "").then(function(oData){
			// assert
			assert.notEqual(oData, undefined);
			done();
		});
	...
	QUnit.test("TextArrangement.prototype.parseDescriptionOnly should not be invoked if Id is empty string", function(assert) {
		...
		this.spy(oType, "parseDescriptionOnly");

		// act
		oType.parseValue("");

		// assert
		assert.equal(oType.parseDescriptionOnly.called, false);
	...
	QUnit.test("it should return the value when TextArrangement called with invalid arguments", function (assert) {
		// Arrange
		var oType = new TextArrangementString(
			{
				textArrangement: "idAndDescription"
			},
			null,
		...
		// Assert
		assert.strictEqual(oType.formatValue("string", ""), "string", "Called with string");
		assert.strictEqual(oType.formatValue(null, ""), null, "Called with null");
		assert.strictEqual(oType.formatValue(["ID", "DESCRIPTION"], "string"),
			"ID (DESCRIPTION)", "Called with valid arguments"
		);
	});
