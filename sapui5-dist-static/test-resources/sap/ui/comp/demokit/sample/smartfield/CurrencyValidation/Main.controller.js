		...
		return Controller.extend(
			"sap.ui.comp.sample.smartfield.CurrencyValidation.Main",
			{
				onInit: function () {
					...
					var oModelProcessFlow = new JSONModel();
					...
					var oModelCurrencyFormat = new JSONModel([]);
				...
				onRequestSent: function (oEvent) {
					if (Utils.isPostOrMerge(oEvent.getParameter("method"))){
						...
						var oModelPendingChanges = oEvent.getSource().getPendingChanges();
						var sValue = oModelPendingChanges[oEvent.getParameter("url")];
				...
				onRequestFailed: function (oEvent) {
					if (Utils.isPostOrMerge(oEvent.getParameter("method"))) {
						...
						var aListItems = this.oList.getItems();
						if (aListItems.length) {
							var oLastItem = aListItems[0];
				...
				onRequestCompleted: function (oEvent) {
					if (Utils.isPostOrMerge(oEvent.getParameter("method"))) {
						...
						if (!oEvent.getParameter("success")){
							var sErrorMessage = oResponse.statusText;
							var oResponseBody = JSON.parse(oResponse.responseText);
				...
				_traceChangeModelValue: function(oEvent) {
					...
					if (oEvent.getParameter("valueLastValueState") === coreLibrary.ValueState.Error ||
						oEvent.getParameter("unitLastValueState") === coreLibrary.ValueState.Error ) {
				...
				_updateCurrencyFormatModel: function (oSmartField) {
					if (oSmartField.getMode() === "edit") {
						...
						var data = this._getCurrencyScaleAndPrecision(oSmartField);

						var oDataType = oSmartField
							.getFirstInnerControl()
							.getBinding("value")
							.getType();

						var cDecimalSeparator = oDataType.oOutputFormat.oLocaleData.mData["symbols-latn-decimal"];

						var sTemplate = this._createCurrencyValueTemplate(
							data.Precision,
							data.Scale,
							cDecimalSeparator
						);

						var currencyFormatData = {
				...
				_createCurrencyValueTemplate: function (
					...
					var aResult = Array(nPrecision).fill("X");
				...
				_getCurrencyScaleAndPrecision: function(oSmartField){
					...
					var iMetadataPrecision = oDataType.oConstraints.precision;
					var iMetadataScale = oDataType.oConstraints.scale;
					var iCurrencyScale = oDataType.oOutputFormat.oLocaleData.getCurrencyDigits(sCurrencyCode);

					var iPrecision = typeof iMetadataPrecision === "number" ? iMetadataPrecision : Infinity;
					var iScale = 0;
