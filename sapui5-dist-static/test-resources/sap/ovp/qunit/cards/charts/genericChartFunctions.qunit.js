    ...
    "sap/base/Log"
           ...
           test("Test Generic Chart Functions",function(){
               ...
               ok(chartTestUtils.isValidString(
                       ...
                       "Date Formatter test");
               ok(chartUtils.formDimensionPath.bind(entityTypeDataContext)("ProductID") === "{Product}",
                       "Test if DimensionDefinition names consider sap:text");
               ok(chartUtils.formDimensionPath.bind(entityTypeDataContext)("Region") === "{Region}",
                       "Test if DimensionDefinition names fall back to dimension name when no sap:text available");
               ok(chartUtils.formDimensionPath.bind(entityTypeDataContext)("Date") === "{path:'Date', formatter: 'sap.ovp.cards.charts.VizAnnotationManager.returnDateFormat'}",
           ...
           test("Axis Scaling new Vocabulary - ZeroAlwaysVisible",function(){
               ...
               oModel.getMetaModel().loaded().then(function () {
                   ...
                   oView.loaded().then(function (oView) {
                       ...
                       var oController = {getView : function() {return null}, getOwnerComponent: function() { return null}};
                       sap.ovp.cards.charts.VizAnnotationManager.buildVizAttributes(cardViz, handler, chartTitle, oController);
        	   ...
        	   var cardTestData = {
                   ...
                   oModel.getMetaModel().loaded().then(function () {
                       ...
                       oView.loaded().then(function (oView) {
                           ...
                           oController = {getView : function() {return {getModel: function(y){return {getProperty: function(x){return {showDataLabel: true};}};}}}, getOwnerComponent: function() { return null}};
                           sap.ovp.cards.charts.VizAnnotationManager.buildVizAttributes(cardViz, handler, chartTitle, oController);
                           ok(vizProperties.plotArea.dataLabel.visible == true && vizProperties.plotArea.dataLabel.formatString, "Test analytical chart label is visible");
                           oController = {getView : function() {return {getModel: function(y){return {getProperty: function(x){return {showDataLabel: false};}};}}}, getOwnerComponent: function() { return null}};
                           sap.ovp.cards.charts.VizAnnotationManager.buildVizAttributes(cardViz, handler, chartTitle, oController);
                           ok(vizProperties.plotArea.dataLabel.visible == false, "Test analytical chart label is not visible");
                       });
                   });
               });
      ...
      test("Analytical Chart - Chart Axes formatting - Date (QQQ)",function(){

        VizAnnotationManager.formatChartAxes();
        var expectedResult = "Q1";
        var oDate = new Date("January 15, 2010 05:30:00");
        var vizFormatter = sap.viz.api.env.Format.numericFormatter();
        var actualResult = vizFormatter.format(oDate, "QQQ");
        ok((expectedResult === actualResult), "Check for date formatting");

      });

      test("Analytical Chart - Chart Axes formatting - Date (www)",function(){

        VizAnnotationManager.formatChartAxes();
        var expectedResult = "CW 03";
        var oDate = new Date("January 15, 2010 05:30:00");
        var vizFormatter = sap.viz.api.env.Format.numericFormatter();
        var actualResult = vizFormatter.format(oDate, "www");
        ok((expectedResult === actualResult), "Check for date formatting");
