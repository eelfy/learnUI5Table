...
sap.ui.define([
...
], function (utils, mockservers, AnnotationHelper, OVPCardAsAPIUtils, CommonUtils, jUtils) {
    "use strict";
        ...
        oModel.getMetaModel().loaded().then(function () {
            ...
            oView.loaded().then(function (oView) {
                ...
                oView.onAfterRendering = function () {
                    ...
                    var cardListFooter = testContainer.querySelector(".sapOvpCardFooter ");

                    //start the async test
                    start();

                    //Check list
                    ok(cardListContent.getAttribute("aria-label") == sap.ui.getCore().getLibraryResourceBundle("sap.ovp").getText("listCard"), "List Card type is accessble");
        ...
        oModel.getMetaModel().loaded().then(function () {
            ...
            oView.loaded().then(function (oView) {
                ...
                oView.onAfterRendering = function () {
                    ...
                    ok(cardListContent.getAttribute("aria-label") == sap.ui.getCore().getLibraryResourceBundle("sap.ovp").getText("barChartCard"), "Bar Chart Card type is accessble");
                    oView.destroy();
                };
            });
        });
            ...
            expectedResult: {
                Header: {
                    KPI: {
                        ...
                        numberNumericContentValue: /\{parts:\s*\[\s*{path:\s*'\w*'},\s*{\s*value:\s*{\s*"NumberOfFractionalDigits":\s*[0-2],\s*"percentageAvailable"\s*:(true|false)\s*},\s*model:\s*'\w*'}],\s*formatter:\s*'sap.ovp.cards.AnnotationHelper.KpiValueFormatter'\s*}/,
                        numberUOM: /\{path: *'CurrencyCode'.*\}/,
                        sortBy: true,
                        sortByContent: "By Lifecycle Descript., Delivery Description",
                        filterBy: false
    ...
    test("Card Test - onAfterRendering, when OVP used as API and layout type is resizable", function () {
        ...
        document.body.insertAdjacentHTML('beforeend', '<div id="mainView--ovpLayout--card056" class="sapOvpWrapper1" style="height:320px; width:1500px"><div class="sapOvpWrapper"></div></div>');
