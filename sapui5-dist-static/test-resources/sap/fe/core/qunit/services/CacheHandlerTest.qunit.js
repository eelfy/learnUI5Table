...
sap.ui.define(["sap/fe/core/services/CacheHandlerServiceFactory", "sap/ui/core/cache/CacheManager", "sap/ui/Device"], function(
	...
	QUnit.test("cannot be initialized with a failing metamodel", function(assert) {
		try {
			...
			var oOriginFn = oFakeMetaModel.fetchEntityContainer;
			oFakeMetaModel.fetchEntityContainer = function() {
			...
			oInstance
				...
				.catch(function() {
					...
					oFakeMetaModel.fetchEntityContainer = oOriginFn;
					fDone();
				});
		} catch (e) {
	...
	QUnit.test("is initialized as a global instance", function(assert) {
		...
		var sFakeMetaModelId = oFakeMetaModel.getId();
		var oFetchEntityContainerSpy = sinon.spy(oFakeMetaModel, "fetchEntityContainer");
		try {
			assert.ok(oFactory._oInstanceRegistry[sFakeMetaModelId] == null, "No global instance exists");
			var oInstance = oFactory.createInstance({ settings: { metaModel: oFakeMetaModel } });
			...
			oInstance
				.then(function(oInstanceLoaded) {
					...
					assert.ok(oFactory._oInstanceRegistry[sFakeMetaModelId] != null, "Global instance exists");
				...
				.then(function(oGlobalInstance) {
					...
					assert.ok(oFactory._oInstanceRegistry[sFakeMetaModelId] == null, "Global instance is removed");
