/*global QUnit */
sap.ui.define(["sap/fe/navigation/SelectionVariant", "sap/fe/navigation/NavError"], function(SelectionVariant, NavError) {
	"use strict";

	/**
	 * tests for the sap.fe.navigation.SelectionVariant class
	 * */
	...
	QUnit.test("get/setText: identity when reading/writing text", function(assert) {
		...
		assert.equal(
			oSelVar.getText(),
			"myTextWhichShallBeUsedForQunitTesting",
			"reading back text needs to provide the same value as when set"
		);
	});
	...
	QUnit.test("get/setParameterContextUrl: identity when reading/writing", function(assert) {
		...
		assert.equal(
			oSelVar.getParameterContextUrl(),
			"/sap/opu/odata/myOdataService//$metadata#AccountBalanceQueryParameters",
			"reading back Parameter Context URL needs to provide the same value as when set"
		);
	});
	...
	QUnit.test("get/setFilterContextUrl: identity when reading/writing", function(assert) {
		...
		assert.equal(
			oSelVar.getFilterContextUrl(),
			"/sap/opu/odata/myOdataService//$metadata#AccountBalanceQueryResult",
			"reading back Parameter Context URL needs to provide the same value as when set"
		);
	});
	...
	QUnit.test("add/getParameter: setting a parameter with value null returns an exception", function(assert) {
		...
		try {
			...
			assert.ok(x instanceof sap.fe.navigation.NavError, "raised error needs to be of type sap.fe.navigation.NavError");
	...
	QUnit.test("getParameterNames: Parameter added needs to be listed", function(assert) {
		...
		var expected = ["myParameterName"];
		assert.deepEqual(
			oSelVar.getParameterNames(),
			expected,
			"parameter which has been set needs to be available when asking for the ParameterNames"
		);
	});
	...
	QUnit.test("add/getSelectOption: identity when reading/writing", function(assert) {
		...
		assert.deepEqual(
			oSelVar.getSelectOption("mySelOptName"),
			expected,
			"select option range set needs to be returned when asking to retrieve it back"
		);
	});
	...
	QUnit.test("getSelectOptionsNames: SelectOptions added needs to be listed", function(assert) {
		...
		oSelVar.addSelectOption("mySelOptName", "I", "EQ", "low-value");
		var expected = ["mySelOptName"];

		assert.deepEqual(
			oSelVar.getSelectOptionsPropertyNames(),
			expected,
			"select option which has been added needs to be available when asking for the list of select option names"
		);
	});
	...
	QUnit.test("getSelectOptionsNames: multiple SelectOptions properties", function(assert) {
		...
		var expected = ["mySelOptName1", "mySelOptName2", "mySelOptName3", "mySelOptName4", "mySelOptName5", "mySelOptName6"];

		assert.deepEqual(
			oSelVar.getSelectOptionsPropertyNames().sort(),
			expected.sort(),
			"select option which has been added needs to be available when asking for the list of select option names"
		);
	});
	...
	QUnit.test("getPropertyNames: Parameter and SelectOption added need to be listed", function(assert) {
		...
		var expected = ["myParameterName", "mySelOptName"];
		assert.deepEqual(
			oSelVar.getPropertyNames().sort(),
			expected.sort(),
			"parameter and select option which have been set need to be available when asking for the PropertyNames"
		);
	});
	...
	QUnit.test("addSelectOption: failure on invalid High values", function(assert) {
		var fInvalidHigh = function(sSign, sOption) {
			...
			try {
				...
				thrown = true;
				assert.ok(
					x instanceof sap.fe.navigation.NavError,
					"raised error for " + sVariant + " needs to be of type sap.fe.navigation.NavError"
				);
	...
	QUnit.test("addSelectOption: failure on invalid sign", function(assert) {
		...
		try {
			...
			assert.ok(
				x instanceof sap.fe.navigation.NavError,
				"raised error on invalid sign needs to be of type sap.fe.navigation.NavError"
			);
	...
	QUnit.test("addSelectOption: failure on invalid option", function(assert) {
		...
		try {
			...
			assert.ok(
				x instanceof sap.fe.navigation.NavError,
				"raised error on invalid option needs to be of type sap.fe.navigation.NavError"
			);
	...
	QUnit.test("toJSONString/toJSONObject: Simple Serialization", function(assert) {
		...
		assert.deepEqual(
			effectiveObject,
			expectedObject,
			"toJSONObject should also return the same result (using object notation) as toJSONString"
		);
	});
	...
	//	var thrown = false;
	//	try {
	//		var effective = new sap.fe.navigation.SelectionVariant(sJSON);
	//	} catch (x) {
	//		thrown = true;
	//	}
	...
	//	} catch (x) {
	//		thrown = true;
	//		assert.ok(x instanceof sap.fe.navigation.NavError, "raised error on invalid SelectionVariant needs to be of type sap.fe.navigation.NavError");
	...
	QUnit.test("parseFromString: Failure on missing PropertyName", function(assert) {
		...
		try {
			...
			assert.ok(
				x instanceof sap.fe.navigation.NavError,
				"raised error on missing PropertyName needs to be of type sap.fe.navigation.NavError"
			);
	...
	QUnit.test("parseFromString: Failure on missing ParameterName", function(assert) {
		...
		try {
			...
			assert.ok(
				x instanceof sap.fe.navigation.NavError,
				"raised error on missing ParameterName needs to be of type sap.fe.navigation.NavError"
			);
	...
	QUnit.test("parseFromString: Failure on invalid Ranges format", function(assert) {
		var oJSON = {
			...
			"SelectOptions": [
				...
				{
					"PropertyName": "mySelOptName_duplicate",
					// here's the error in the format! --> needs to be an array
					"Ranges": {
						"Sign": "I",
						"Option": "CP",
						"Low": "*low-value*",
		...
		try {
			...
			assert.ok(
				x instanceof sap.fe.navigation.NavError,
				"raised error on invalid Ranges format needs to be of type sap.fe.navigation.NavError"
			);
	...
	QUnit.test("parseFromString: empty Select Options need to be ignored", function(assert) {
		...
		assert.deepEqual(
			effective.getSelectOption("mySelOptName1"),
			[
				{
					"Sign": "I",
					"Option": "EQ",
	...
	QUnit.test("removeParameter: removed parameter not available on serialization", function(assert) {
		...
		assert.equal(
			sJsonSerialization,
			'{"Version":{"Major":"1","Minor":"0","Patch":"0"},"SelectionVariantID":"test-id_1234-removeParameter","Text":"test text for removeParameter check","ODataFilterExpression":"","Parameters":[],"SelectOptions":[]}',
			"serialization is not equal to expected serialized value"
		);
	});
	...
	QUnit.test("removeParameter: removed Parameter is no longer available in getParameterNames", function(assert) {
		...
		var expectedParameterNames = ["testParameter2"];
	...
	QUnit.test("removeSelectOption: removed Select Option not available on serialization", function(assert) {
		...
		assert.equal(
			sJsonSerialization,
			'{"Version":{"Major":"1","Minor":"0","Patch":"0"},"SelectionVariantID":"test-id_1234-removeSelectOption","Text":"test text for removeSelectOption check","ODataFilterExpression":"","Parameters":[],"SelectOptions":[]}',
			"serialization is not equal to expected serialized value"
		);
	});
	...
	QUnit.test("removeSelectOption: removed Select Option is no longer available in getSelectOptionsPropertyNames", function(assert) {
		...
		var expectedPropertyNames = ["testSelOpt2"];
	...
	QUnit.test("renameSelectOption - success", function(assert) {
		...
		assert.deepEqual(
			oSelVar.getSelectOption("selOptNew"),
			[
				{
					"High": null,
					"Low": "value",
		...
		assert.deepEqual(
			oSelVar.getSelectOption("selOpt2"),
			[
				{
					"High": null,
					"Low": "value2",
