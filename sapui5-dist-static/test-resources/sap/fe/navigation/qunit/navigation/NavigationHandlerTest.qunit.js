...
sap.ui.define(
	[
		...
		"./CrossAppNavigationServiceMock",
		"sap/base/Log",
		"sap/ui/core/routing/HashChanger",
		"sap/ui/thirdparty/URI"
	],
	function(NavigationHandler, NavError, SelectionVariant, navigationLibrary, CrossAppNavigationServiceMock, Log, HashChanger, URI) {
		"use strict";

		// shortcuts for types from sap.fe.navigation.library
		var NavType = navigationLibrary.NavType;
		var ParamHandlingMode = navigationLibrary.ParamHandlingMode;
		var SuppressionBehavior = navigationLibrary.SuppressionBehavior;

		/**
		 * tests for the sap.fe.navigation.NavigationHandler class
		 */
		...
		QUnit.test("Test of _getURLParametersFromSelectionVariant", function(assert) {
			...
			for (var i = 0; i < aTestScenarios.length; i++) {
				...
				assert.deepEqual(
					oActualParameters,
					aTestScenarios[i].ExpectedParameters,
					"sSelectionVariant: Actual parameters must be equal to expected parameters"
				);
				...
				assert.deepEqual(
					oActualParameters,
					aTestScenarios[i].ExpectedParameters,
					"oSelectionVariant: Actual parameters must be equal to expected parameters"
				);
			}
		});
		...
		QUnit.test("_getInnerAppStateKey", function(assert) {
			...
			// get instance of NavigationHandler
			var oNavigationHandlerV2 = new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);

			sResult = oNavigationHandlerV2._getInnerAppStateKey("");
			assert.equal(sResult, undefined, "an empty App Hash does not provide an IAPP_STATE");

			sResult = oNavigationHandlerV2._getInnerAppStateKey(undefined);
			assert.equal(sResult, undefined, "a non-existing App Hash does not provide an IAPP_STATE");

			sResult = oNavigationHandlerV2._getInnerAppStateKey("sap-iapp-state=xyz");
			assert.equal(sResult, "xyz", "an old-approach IAPP_STATE shall still be valid");

			sResult = oNavigationHandlerV2._getInnerAppStateKey("key/route/sap-iapp-state=xyz");
			assert.equal(sResult, "xyz", "an old-approach IAPP_STATE with key shall still be valid");

			sResult = oNavigationHandlerV2._getInnerAppStateKey("?sap-iapp-state=xyz");
			assert.equal(sResult, "xyz", "an new-approach IAPP_STATE shall be valid as well");

			sResult = oNavigationHandlerV2._getInnerAppStateKey("sap-iapp-state=abc?sap-iapp-state=xyz");
			assert.equal(sResult, "xyz", "an new-approach IAPP_STATE shall superseed the old-approach one");

			sResult = oNavigationHandlerV2._getInnerAppStateKey("sap-iapp-state=abc?sap-noapp-state=xyz");
			assert.equal(sResult, "abc", "other query parameters shall not mix things up");
		...
		QUnit.test("_replaceInnerAppStateKey ODataV2", function(assert) {
			// mock controller and sap.ushell.Container.getService
			var oController = {
				getOwnerComponent: function() {
					return {
			...
			aTestScenarios.push({
				AppHash: "",
				AppStateKey: "",
				ExpectedHash: "?" + oNavigationHandlerV2.IAPP_STATE + "="
			});

			aTestScenarios.push({
				AppHash: "abcDEF123",
				AppStateKey: "",
				ExpectedHash: "abcDEF123?" + oNavigationHandlerV2.IAPP_STATE + "="
			});

			aTestScenarios.push({
				AppHash: "",
				AppStateKey: "ghiJKL456",
				ExpectedHash: "?" + oNavigationHandlerV2.IAPP_STATE + "=ghiJKL456"
			});

			aTestScenarios.push({
				AppHash: "abcDEF123",
				AppStateKey: "ghiJKL456",
				ExpectedHash: "abcDEF123?" + oNavigationHandlerV2.IAPP_STATE + "=ghiJKL456"
			});

			// Old iapp-state key shall be replaced by new approach
			aTestScenarios.push({
				AppHash: oNavigationHandlerV2.IAPP_STATE + "=oldHashKey",
				AppStateKey: "ghiJKL456",
				ExpectedHash: "?" + oNavigationHandlerV2.IAPP_STATE + "=ghiJKL456"
			});
			...
			aTestScenarios.push({
				AppHash: "route/" + oNavigationHandlerV2.IAPP_STATE + "=oldHashKey",
				AppStateKey: "ghiJKL456",
				ExpectedHash: "route?" + oNavigationHandlerV2.IAPP_STATE + "=ghiJKL456"
			});

			// New iapp-state key with route shall be replaced properly
			aTestScenarios.push({
				AppHash: "route/secondKey?" + oNavigationHandlerV2.IAPP_STATE + "=oldHashKey",
				AppStateKey: "ghiJKL456",
				ExpectedHash: "route/secondKey?" + oNavigationHandlerV2.IAPP_STATE + "=ghiJKL456"
			});
			...
			for (var i = 0; i < aTestScenarios.length; i++) {
				...
				assert.ok(
					true,
					'Running scenario: sAppHash = "' +
						aTestScenarios[i].AppHash +
						'", sAppStateKey = "' +
						aTestScenarios[i].AppStateKey +
						'"'
				);
				// call method to be tested
				var sActualHash = oNavigationHandlerV2._replaceInnerAppStateKey(aTestScenarios[i].AppHash, aTestScenarios[i].AppStateKey);
				// compare data
		...
		QUnit.test("storeInnerAppState", function(assert) {
			...
			oStoreInnerAppStatePromise.done(function(sAppStateKey) {
				...
				assert.equal(
					oResultData.replacedHash,
					"?" + oNavigationHandler.IAPP_STATE + "=" + sAppStateKey,
					"hash can be replaced correctly"
				);
			...
			oStoreInnerAppStatePromise.done(function(sAppStateKey) {
				...
				assert.deepEqual(
					oTestData.getInnerAppData(),
					oNavigationHandler._oLastSavedInnerAppData.oAppData,
		...
		QUnit.test("storeInnerAppState ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				getInnerAppData: function() {
					var oSelectionVariant = new SelectionVariant();
			...
			// get instance of the NavigationHandler
			var oNavigationHandler = new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);
		...
		QUnit.test("storeInnerAppStateWithImmediateReturn", function(assert) {
			...
			// test caching
			oStoreInnerAppStatePromise = oNavigationHandlerV2.storeInnerAppStateWithImmediateReturn(oTestData.getInnerAppData());
			assert.ok(oStoreInnerAppStatePromise);
		...
		QUnit.test("_saveAppState - negative test for JSON.parse", function(assert) {
			...
			var fnOnError = function(oError) {
				assert.equal(
					oError.getErrorCode(),
					"NavigationHandler.AppStateSave.parseError",
					"JSON.parse failed and error was raised (as expected)"
				);
			};
			...
			// call the method to be tested
			sKey = oNavigationHandlerV2._saveAppState(oAppData, fnOnAfterSave, fnOnError);
			assert.equal(sKey, undefined, "sKey must be undefined");
		...
		QUnit.test("navigate", function(assert) {
			...
			oNavigationHandler.navigate(
				oTestData.sSemanticObject,
				oTestData.sActionName,
				oTestData.getNavData(),
				oTestData.getInnerAppData(),
				fOnError
			);
			...
			oAppStatePromise.done(function(oAppState) {
				...
				assert.deepEqual(
					oAppData.selectionVariant,
					JSON.parse(oTestData.getNavData()),
					"data handed over to save in xapp-state as expected"
				);
			});
			...
			// check stored data (sap-xapp-state)
			sAppStateKey = oCrossNavMock.toExternalArgs.appStateKey;
			oAppStatePromise = oCrossNavMock.getAppState(null, sAppStateKey);
			oAppStatePromise.done(function(oAppState) {
				var oAppData = oAppState.getData();
				// assert.deepEqual(oAppData.selectionVariant, {}, "data handed over to save in xapp-state as expected");
		...
		QUnit.test("navigate with xapp data", function(assert) {
			...
			oAppStatePromise.done(function(oAppState) {
				...
				assert.deepEqual(
					oAppData.selectionVariant,
					JSON.parse(oTestData.getXAppData()),
					"data handed over to save in xapp-state as expected"
				);
			...
			// call function to be tested
			oNavigationHandlerV2.navigate(oTestData.sSemanticObject, oTestData.sActionName, oTestData.getNavData(), null, fOnError, {
				selectionVariant: oTestData.getXAppData(),
				presentationVariant: {},
				valueTexts: {},
				semanticDates: {}
			...
			oAppStatePromise.done(function(oAppState) {
				...
				assert.deepEqual(oAppData.valueTexts, {}, "valueTexts handed over to save in xapp-state as expected");
				assert.deepEqual(oAppData.semanticDates, {}, "semanticDates handed over to save in xapp-state as expected");
				assert.deepEqual(oAppData.presentationVariant, {}, "presentationVariant handed over to save in xapp-state as expected");
			});
		...
		QUnit.test("navigate with an empty innerAppData object", function(assert) {
			...
			// call function to be tested
			oNavigationHandlerV2.navigate(oTestData.sSemanticObject, oTestData.sActionName, oTestData.getNavData(), {}, fOnError);

			assert.deepEqual(
				oNavigationHandlerV2._oLastSavedInnerAppData.oAppData,
				{},
				"data handed over to save in iapp-state as expected"
			);
		...
		QUnit.test("navigate with 'sap-ushell-next-navmode' ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				sSemanticObject: "SalesOrder",
				sActionName: "create",
		...
		QUnit.test("navigate with navMode", function(assert) {
			...
			// get instance of the NavigationHandler
			var oNavigationHandlerV2 = new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);
			oNavigationHandlerV2.oComponent = oComponentData;

			sinon.spy(oNavigationHandlerV2, "storeInnerAppStateWithImmediateReturn");
			sinon.spy(oNavigationHandlerV2, "_saveAppStateWithImmediateReturn");
			...
			assert.ok(oNavigationHandlerV2.storeInnerAppStateWithImmediateReturn.calledOnce);
			assert.ok(oNavigationHandlerV2._saveAppStateWithImmediateReturn.calledOnce);

			oComponentData.startupParameters = {};
			oNavigationHandlerV2._oLastSavedInnerAppData = null;
			oNavigationHandlerV2.storeInnerAppStateWithImmediateReturn.reset();
			oNavigationHandlerV2._saveAppStateWithImmediateReturn.reset();
			...
			assert.deepEqual(oNavigationHandlerV2._oLastSavedInnerAppData, null, "data not handed over to save in iapp-state as expected");
			assert.ok(!oNavigationHandlerV2.storeInnerAppStateWithImmediateReturn.called);
			assert.ok(oNavigationHandlerV2._saveAppStateWithImmediateReturn.calledOnce);
		...
		QUnit.test("parseNavigation: sap-xapp-state only / with URL parameters", function(assert) {
			...
			oParseNavigationPromise.done(function(oAppData, oURLParameters, sNavType) {
				var oExpectedAppData = oNavigationHandler._splitInboundNavigationParameters(
					oTestData.getNavData(),
					oTestData.getURLParameters(),
					[]
				);
				assert.equal(
					oAppData.selectionVariant,
					oExpectedAppData.oNavigationSelVar.toJSONString(),
					"parsed app data from URL params and xapp-state returned as expected"
				);
			...
			// call method to be tested
			oParseNavigationPromise = oNavigationHandlerV2.parseNavigation();

			oParseNavigationPromise.done(function(oAppData, oURLParameters, sNavType) {
				var oExpectedAppData = oNavigationHandlerV2._splitInboundNavigationParameters(
					oTestData.getNavData(),
					oTestData.getURLParameters(),
					[]
				);
		...
		QUnit.test("parseNavigation via String & Object: sap-xapp-state only / no URL parameters", function(assert) {
			var fnTestRoutine = function(sSelVarMode, oStartupParameters) {
				...
				oParseNavigationPromise.done(function(oAppData, oURLParameters, sNavType) {
					var oExpectedAppData = oNavigationHandler._splitInboundNavigationParameters(
						new SelectionVariant(oTestData.getNavData()),
						{},
						[]
					);
					assert.equal(
						oAppData.selectionVariant,
						oExpectedAppData.oNavigationSelVar.toJSONString(),
						"parsed app data from xapp-state returned as expected"
					);
					assert.equal(sNavType, NavType.xAppState, "NavType must be correct");
				});
				oParseNavigationPromise.fail(function(oError) {
		...
		QUnit.test("parseNavigation via String & Object: sap-iapp-state / old Approach ODataV2", function(assert) {
			var fnTestRoutine = function(sSelVarMode) {
				// prepare test data
				var oTestData = {
					getInnerAppData: function() {
				...
				// get instance of the NavigationHandler
				oNavigationHandler = new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);
		...
		QUnit.test("parseNavigation via String & Object: sap-iapp-state / new Approach ODataV2", function(assert) {
			var fnTestRoutine = function(sSelVarMode) {
				// prepare test data
				var oTestData = {
					getInnerAppData: function() {
		...
		QUnit.test("parseNavigation: basic URL parameter navigation", function(assert) {
			...
			oParseNavigationPromise.done(function(oAppData, oURLParameters, sNavType) {
				...
				assert.deepEqual(
					oSelectionVariant.getSelectOption("Customer"),
					oExpected,
					"Selection Variant SelOption for Customer needs to contain the value"
				);
			});
			oParseNavigationPromise.fail(function(oError) {
				assert.ok(false, oError.getErrorCode() + " was raised");
		...
		QUnit.test("parseNavigation: initial startup with default parameters", function(assert) {
			...
			var oTestData = {
				defaultedParameters: ["Customer", "CompanyCode"]
			};
			...
			var oController = {
				getOwnerComponent: function() {
					return {
						oComponentData: {
							startupParameters: {
								"sap-ushell-defaultedParameterNames": [JSON.stringify(oTestData.defaultedParameters)]
							}
						},
						getComponentData: function() {
							return this.oComponentData;
		...
		QUnit.test("parseNavigation: xapp-state with default parameters", function(assert) {
			...
			var oTestData = {
				...
				getComponentData: function() {
					return {
						"startupParameters": {
							"sap-ushell-defaultedParameterNames": [JSON.stringify(["Customer", "CompanyCode"])],
							Customer: "C0001",
							CompanyCode: "0001"
						},
						"sap-xapp-state": "dummy"
		...
		QUnit.test("parseNavigation: xapp-state with invalid JSON", function(assert) {
			...
			// set startup data (sap-xapp-state)
			oCrossNavMock.setStartupAppState(function() {}); // invalid JSON --> JSON.parse(JSON.stringify(oAppData)) will lead to an error
			...
			oParseNavigationPromise.fail(function(oError) {
				...
				assert.equal(
					sErrorCode,
					"NavigationHandler.AppStateData.parseError",
					"NavigationHandler.AppStateData.parseError was raised"
				);
				bFailCalled = true;
			});
		...
		QUnit.test("processBeforeSmartLinkPopoverOpens: selectionVariant not provided", function(assert) {
			...
			oSmartLinkPromise.done(function(oTableEventParameters) {
				...
				var oMixedSelVar = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					oTableEventParameters.getSemanticAttributes(),
					oTestData.getSelectionVariant()
				);
				...
				assert.deepEqual(
					oMixedSemanticAttributesActual,
					oMixedSemanticAttributesExpected,
					"Mixed Semantic Attributes must be generated correctly"
				);

				// check AppStateKey
				assert.ok(oTableEventParameters.getAppStateKey() !== "", "AppStateKey must not be empty");
				assert.equal(
					Object.keys(oCrossNavMock.oAppStates)[0],
					oTableEventParameters.getAppStateKey(),
					"AppStateKey must be generated correctly"
				);
			});
			...
			// call method to be tested
			var oTableEventParametersV2 = fnGetTableEventParameters();
			oTableEventParametersV2.setSemanticAttributes(oTestData.getSemanticAttributes());
			oSmartLinkPromise = oNavigationHandlerV2.processBeforeSmartLinkPopoverOpens(oTableEventParametersV2);

			oSmartLinkPromise.done(function(oTableEventParametersV2) {
				// check popover has been opened
				assert.ok(oTableEventParametersV2.isOpenCalled, "oTableEventParametersV2.open() must be called");

				// check semantic attributes were mixed and set correctly
				var oMixedSelVar = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					oTableEventParametersV2.getSemanticAttributes(),
					oTestData.getSelectionVariant()
				);
				var oMixedSemanticAttributesExpected = oNavigationHandlerV2._getURLParametersFromSelectionVariant(oMixedSelVar);
				var oMixedSemanticAttributesActual = oTableEventParametersV2.getSemanticAttributes();
				assert.ok(oMixedSemanticAttributesActual, "semanticAttributes must not be empty");
				assert.deepEqual(
					oMixedSemanticAttributesActual,
					oMixedSemanticAttributesExpected,
				...
				// check AppStateKey
				assert.ok(oTableEventParametersV2.getAppStateKey() !== "", "AppStateKey must not be empty");
				assert.equal(
					Object.keys(oCrossNavMock.oAppStates)[1],
					oTableEventParametersV2.getAppStateKey(),
					"AppStateKey must be generated correctly"
				);
			});
		...
		QUnit.test("processBeforeSmartLinkPopoverOpens: selectionVariant provided", function(assert) {
			...
			var oTableEventParameters = fnGetTableEventParameters();
			oTableEventParameters.setSemanticAttributes(oTestData.getSemanticAttributes());
			var oSmartLinkPromise = oNavigationHandler.processBeforeSmartLinkPopoverOpens(
			...
			var oTableEventParametersV2 = fnGetTableEventParameters();
			oTableEventParametersV2.setSemanticAttributes(oTestData.getSemanticAttributes());
			oSmartLinkPromise = oNavigationHandlerV2.processBeforeSmartLinkPopoverOpens(
				oTableEventParametersV2,
				oTestData.getSelectionVariant()
			);
		...
		QUnit.test("processBeforeSmartLinkPopoverOpens: innerAppState provided", function(assert) {
			...
			oSmartLinkPromise.done(function(oTableEventParameters) {
				...
				oExpectedData.selectionVariant = JSON.parse(oExpectedData.selectionVariant);
				assert.deepEqual(
					oCrossNavMock.oAppStates[oController.getAppStateKey()].getData(),
					oExpectedData,
					"AppStateKey must be generated correctly"
				);
			});
		...
		QUnit.test("processBeforeSmartLinkPopoverOpens: innerAppState provided ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				getSelectionVariant: function() {
					var oSelectionVariant = new SelectionVariant();
		...
		QUnit.test("processBeforeSmartLinkPopoverOpens: selectionVariant provided and TableEventParameters not provided", function(assert) {
			// prepare test data
			var oTestData = {
				getSelectionVariant: function() {
					var oSelectionVariant = new SelectionVariant();
			...
			// call method to be tested
			var oSmartLinkPromise = oNavigationHandler.processBeforeSmartLinkPopoverOpens(undefined, oTestData.getSelectionVariant());

			oSmartLinkPromise.done(function(oSemanticAttributesParam, sAppStateKey) {
				// check semantic attributes were mixed and set correctly
				var oMixedSelVar = oNavigationHandler.mixAttributesAndSelectionVariant(
					oSemanticAttributesParam,
					oTestData.getSelectionVariant()
				);
				var oMixedSemanticAttributesExpected = oNavigationHandler._getURLParametersFromSelectionVariant(oMixedSelVar);
				var oMixedSemanticAttributesActual = oSemanticAttributesParam;
				assert.ok(oMixedSemanticAttributesActual, "semanticAttributes must not be empty");
				assert.deepEqual(
					oMixedSemanticAttributesActual,
					oMixedSemanticAttributesExpected,
				...
				// check AppStateKey
				assert.ok(sAppStateKey !== "", "AppStateKey must not be empty");
				assert.equal(Object.keys(oCrossNavMock.oAppStates)[0], sAppStateKey, "AppStateKey must be generated correctly");
			});
		...
		QUnit.test("Ensure processBeforeSmartLinkPopoverOpens is called from _getAppStateKeyAndUrlParameters", function(assert) {
			// prepare test data
			var oTestData = {
				getSelectionVariant: function() {
					var oSelectionVariant = new SelectionVariant();
			...
			// call method to be tested
			var oSmartLinkPromise = oNavigationHandler._getAppStateKeyAndUrlParameters(oTestData.getSelectionVariant());

			oSmartLinkPromise.done(function(oSemanticAttributesParam, sAppStateKey) {
				assert.ok(oSemanticAttributesParam, "semanticAttributes must not be empty");
				assert.ok(sAppStateKey !== "", "AppStateKey must not be empty");
			});
		...
		QUnit.test("constructor: negative test - invalid input", function(assert) {
			...
			try {
				...
				assert.ok(x instanceof NavError, "raised error needs to be of type sap.fe.navigation.NavError");
		...
		QUnit.test("constructor: negative test - invalid input", function(assert) {
			...
			try {
				/* eslint-disable no-new */
				new NavigationHandler({} /* Controller */, navigationLibrary.Mode.ODataV2);
				/* eslint-enable no-new */
			} catch (x) {
				assert.ok(x instanceof NavError, "raised error needs to be of type sap.fe.navigation.NavError");
				// assert.equal(x.getSeverity(), sap.ca.ui.message.Type.ERROR, "raised error needs to be of severity ERROR");
			...
			try {
				/* eslint-disable no-new */
				new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);
				/* eslint-enable no-new */
			} catch (x) {
				assert.ok(x instanceof NavError, "raised error needs to be of type sap.fe.navigation.NavError");
				// assert.equal(x.getSeverity(), sap.ca.ui.message.Type.ERROR, "raised error needs to be of severity ERROR");
		...
		function getMockedNavigationHandler(sHandlingMode, sODataVersion) {
			if (!sHandlingMode) {
			...
			var oNavigationHandler = new NavigationHandler(oController, sODataVersion, sHandlingMode);
			stubGetRouter.restore();
		...
		QUnit.test("_splitInboundNavigationParameters - Empty input", function(assert) {
			...
			var oNavigationHandlerV2 = getMockedNavigationHandler(undefined, navigationLibrary.Mode.ODataV2);
			oResult = oNavigationHandlerV2._splitInboundNavigationParameters(oAppStateSelVar, oStartupParameters, aDefaultedParameters);
		...
		QUnit.test("_splitInboundNavigationParameters - Conflicting non-default parameters", function(assert) {
			...
			var oResult = oNavigationHandlerSelVarWins._splitInboundNavigationParameters(
				oAppStateSelVar,
				oStartupParameters,
			...
			oResult = oNavigationHandlerURLParamWins._splitInboundNavigationParameters(
				oAppStateSelVar,
				oStartupParameters,
				aDefaultedParameters
			);
			...
			oResult = oNavigationHandlerInsertInSelOpt._splitInboundNavigationParameters(
				oAppStateSelVar,
				oStartupParameters,
				aDefaultedParameters
			);
			...
			var oNavigationHandlerSelVarWinsV2 = getMockedNavigationHandler(ParamHandlingMode.SelVarWins, navigationLibrary.Mode.ODataV2);
			var oNavigationHandlerURLParamWinsV2 = getMockedNavigationHandler(
				ParamHandlingMode.URLParamWins,
				navigationLibrary.Mode.ODataV2
			);
			var oNavigationHandlerInsertInSelOptV2 = getMockedNavigationHandler(
				ParamHandlingMode.InsertInSelOpt,
				navigationLibrary.Mode.ODataV2
			);

			oResult = oNavigationHandlerSelVarWinsV2._splitInboundNavigationParameters(
				oAppStateSelVar,
				oStartupParameters,
				aDefaultedParameters
			);
			...
			oResult = oNavigationHandlerURLParamWinsV2._splitInboundNavigationParameters(
				oAppStateSelVar,
				oStartupParameters,
				aDefaultedParameters
			);
			...
			oResult = oNavigationHandlerInsertInSelOptV2._splitInboundNavigationParameters(
				oAppStateSelVar,
				oStartupParameters,
				aDefaultedParameters
			);
		...
		QUnit.test("_splitInboundNavigationParameters - Complex case with multiple parametrs in output", function(assert) {
			...
			var aDefaultedParameters = ["K1", "K3", "K5"];
			var oAppStateSelVar = new SelectionVariant();
			oAppStateSelVar.addParameter("K1", "1");
		...
		QUnit.test("_splitInboundNavigationParameters - Invalid startup parameters", function(assert) {
			...
			assert.raises(
				...
				new NavError("NavigationHandler.INVALID_INPUT"),
				"Exception has been thrown successfully"
			);
			...
			assert.raises(
				function() {
					oNavigationHandlerV2._splitInboundNavigationParameters(
						oAppStateSelVar,
						oStartupParametersInvalid,
						aDefaultedParameters
					);
		...
		QUnit.test("check getTechnicalParameter method", function(assert) {
			...
			var oNavServiceV2 = new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);
			assert.ok(oNavServiceV2);

			aTechnicalParameters = oNavServiceV2.getTechnicalParameters();
			assert.ok(aTechnicalParameters);
			assert.equal(aTechnicalParameters[0], "hcpApplicationId");
		...
		QUnit.test("check setTechnicalParameter/_isTechnicalParamater methods", function(assert) {
			...
			assert.ok(oNavServiceV2._isTechnicalParameter("sap-system"));
			assert.ok(oNavServiceV2._isTechnicalParameter("sap-ushell-defaultedParameterNames"));
			assert.ok(oNavServiceV2._isTechnicalParameter("hcpApplicationId"));

			assert.ok(oNavServiceV2._isTechnicalParameter("sap-client"));
			assert.ok(oNavServiceV2._isTechnicalParameter("sap-xxx"));
			assert.ok(!oNavServiceV2._isTechnicalParameter("sapp-client"));

			assert.equal(oNavServiceV2._aTechnicalParamaters.length, 1);

			assert.ok(!oNavServiceV2._isTechnicalParameter("test"));

			aTechnicalParameters = oNavServiceV2.getTechnicalParameters();
			aTechnicalParameters.push("test");

			oNavServiceV2.setTechnicalParameters(aTechnicalParameters);
			assert.ok(oNavServiceV2._isTechnicalParameter("test"));

			assert.equal(oNavServiceV2._aTechnicalParamaters.length, 2);
			assert.equal(oNavServiceV2.getTechnicalParameters().length, 2);

			oNavServiceV2.setTechnicalParameters(null);
			assert.equal(oNavServiceV2._aTechnicalParamaters.length, 0);
			assert.equal(oNavServiceV2.getTechnicalParameters().length, 0);

			NavigationHandler.prototype._getRouter.restore();
			NavigationHandler.prototype._getAppNavigationService.restore();
		});

		QUnit.test("validate navigate with sensitive and excluded information ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				sSemanticObject: "SalesOrder",
				sActionName: "create",
					...
					return {
						...
						semanticDates: {
							"Dates": [
								...
								{
									PropertyName: "NonSensitiveFilter1",
									Data: {
										calendarType: "Georgian",
										key: "Today",
										operation: "=",
			...
			var oMetaModel = {
				...
				isA: function(s) {
					if (s == "sap.ui.model.odata.v4.ODataModel") {
						return false;
					} else {
						return true;
					}
			...
			var oCompModel = {
				isA: function(s) {
					if (s == "sap.ui.model.odata.v2.ODataModel") {
						return true;
					} else {
						return false;
					}
			...
			oAppStatePromise.done(function(oAppState) {
				...
				assert.ok(oAppData.semanticDates);
				assert.ok(oAppData.semanticDates.Dates);
				assert.equal(
					oAppData.semanticDates.Dates.length,
					1,
					"PotentiallySensitive Property are removed from semantic dates before storing in AppState"
				);
				assert.equal(
					oAppData.semanticDates.Dates[0].PropertyName,
					"NonSensitiveFilter1",
					"Only non sensitive properties from semantic dates are stored in app state"
				);
			});
		...
		QUnit.test("validate navigate with sensitive and excluded information", function(assert) {
			...
			var oMetaModel = {
				getObject: function(sPath) {
					if (sPath === "/EntitySet/") {
						return oEntityType;
					} else if (sPath.indexOf("/SensitiveFilter1") > -1) {
						return oEntityType.SensitiveFilter1["@"];
					} else if (sPath.indexOf("/SensitiveFilter2") > -1) {
						return oEntityType.SensitiveFilter2["@"];
					} else if (sPath.indexOf("/ExcludedFilter1") > -1) {
						return oEntityType.ExcludedFilter1["@"];
					} else if (sPath === "/") {
						return true;
					}
					return {};
			...
			var oCrossNavMock = new CrossAppNavigationServiceMock();
			sinon.stub(NavigationHandler.prototype, "_getAppNavigationService").returns(oCrossNavMock);
			sinon.stub(NavigationHandler.prototype, "_constructContextUrl").returns("/foo/$metadata");
			...
			stubGetRouter.restore();
			NavigationHandler.prototype._getAppNavigationService.restore();
			NavigationHandler.prototype._constructContextUrl.restore();
		});
		...
		QUnit.test("validate navigate with measure", function(assert) {
			...
			var oMetaModel = {
				getObject: function(sPath) {
					if (sPath === "/EntitySet/") {
						return oEntityType;
					} else if (sPath.indexOf("/Measure1") > -1) {
						return oEntityType.Measure1["@"];
					} else if (sPath.indexOf("/Measure2") > -1) {
						return oEntityType.Measure2["@"];
					} else if (sPath === "/") {
						return true;
					}
					return {};
		...
		QUnit.test("validate navigate with measure ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				sSemanticObject: "SalesOrder",
				sActionName: "create",
			...
			oAppStatePromise.done(function(oAppState) {
				...
				assert.ok(oAppData.semanticDates);
				assert.ok(oAppData.semanticDates.Dates);
				assert.equal(oAppData.semanticDates.Dates.length, 1, "Measures are removed from semantic dates before storing in AppState");
				assert.equal(
					oAppData.semanticDates.Dates[0].PropertyName,
					"NonSensitiveFilter1",
					"Only non measure properties in semantic dates are stored in app state"
				);
			});
		...
		QUnit.test("validate navigate with sensitive information - not loaded odata metadata ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				sSemanticObject: "SalesOrder",
				sActionName: "create",
		...
		QUnit.test("processBeforeSmartLinkPopoverOpens with sensitive information ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				getSelectionVariant: function() {
					var oSelectionVariant = new SelectionVariant();
		...
		QUnit.test("checking setModel", function(assert) {
			...
			// get instance of the NavigationHandler
			var oNavigationHandlerV2 = new NavigationHandler(oController, navigationLibrary.Mode.ODataV2);
			assert.deepEqual(oNavigationHandlerV2._getModel(), oCompModel);

			oNavigationHandlerV2.setModel(oNewModel);
			assert.deepEqual(oNavigationHandlerV2._getModel(), oNewModel);
		...
		QUnit.test("checking constructContextUrl", function(assert) {
			...
			sinon.stub(NavigationHandler.prototype, "_getRouter").returns({});
			sinon.stub(URI.prototype, "absoluteTo").returns({
				toString: function() {
					return "server";
				}
			});
			...
			var sContextUrlV2 = oNavigationHandlerV2.constructContextUrl("test", oModel);
			assert.equal(sContextUrlV2, "server/service/$metadata#test");
		...
		QUnit.test("validate navigate with sensitive information and measures", function(assert) {
			...
			var oMetaModel = {
				getObject: function(sPath) {
					if (sPath === "/EntitySet/") {
						return oEntityType;
					} else if (sPath.indexOf("/NonSensitiveFilter1") > -1) {
						return oEntityType.NonSensitiveFilter1["@"];
					} else if (sPath.indexOf("/SensitiveFilter1") > -1) {
						return oEntityType.SensitiveFilter1["@"];
					} else if (sPath.indexOf("/SensitiveFilterMeasure2") > -1) {
						return oEntityType.SensitiveFilterMeasure2["@"];
					} else if (sPath.indexOf("/Measure1") > -1) {
						return oEntityType.Measure1["@"];
					} else if (sPath.indexOf("/Measure2") > -1) {
						return oEntityType.Measure2["@"];
		...
		QUnit.test("validate navigate with sensitive information and measures ODataV2", function(assert) {
			// prepare test data
			var oTestData = {
				sSemanticObject: "SalesOrder",
				sActionName: "create",
		...
		QUnit.module("NavigationHandler Tests - mixAttributesAndSelectionVariant", {});
		QUnit.test("mixAttributesAndSelectionVariant: semantic attribute is taken over to Select Option - no overwrite", function(assert) {
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOptionsPropertyNames(),
				["attribute1"],
				"attribute1 needs to be a select option"
			);
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute1"),
				[
					{
						Sign: "I",
						Option: "EQ",
		...
		QUnit.test("mixAttributesAndSelectionVariant: parameters are converted to select option", function(assert) {
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOptionsPropertyNames().sort(),
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute2"),
				[
					{
						Sign: "I",
						Option: "EQ",
		...
		QUnit.test("mixAttributesAndSelectionVariant: semantic attribute is converted to string", function(assert) {
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute3"),
				[
					{
						Sign: "I",
						Option: "EQ",
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute4"),
				[
					{
						Sign: "I",
						Option: "EQ",
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute5"),
				[
					{
						Sign: "I",
						Option: "EQ",
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute6"),
				[
					{
						Sign: "I",
						Option: "EQ",
						Low: JSON.stringify(["a", "b"]),
						High: null
					}
				],
				"Value needs to be a string and in the range of select options"
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute7"),
				[
					{
						Sign: "I",
						Option: "EQ",
			...
			assert.deepEqual(
				oActualSelectionVariant.getSelectOption("attribute8"),
				[
					{
						Sign: "I",
						Option: "EQ",
		...
		QUnit.test("mixAttributesAndSelectionVariant: suppression behavior = ignoreEmptyString", function(assert) {
			...
			var oActualSelectionVariant = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
				mSmanticAttributes,
				oSelVar.toJSONString(),
				SuppressionBehavior.ignoreEmptyString
			);
		...
		QUnit.test("mixAttributesAndSelectionVariant: suppression behavior = raiseErrorOnNull", function(assert) {
			...
			try {
				NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					mSmanticAttributes,
					oSelVar.toJSONString(),
					SuppressionBehavior.raiseErrorOnNull
				);
			} catch (oError) {
				bError = true;
			...
			var spy = sinon.spy(Log, "warning");
			oActualSelectionVariant = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
				mSmanticAttributes,
				oSelVar.toJSONString(),
				SuppressionBehavior.raiseErrorOnNull
			);
		...
		QUnit.test("mixAttributesAndSelectionVariant: suppression behavior = raiseErrorOnUndefined", function(assert) {
			...
			try {
				oActualSelectionVariant = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					mSmanticAttributes,
					oSelVar.toJSONString(),
					SuppressionBehavior.raiseErrorOnUndefined
				);
			} catch (oError) {
				bError = true;
				assert.equal(oError.getErrorCode(), "NavigationHandler.INVALID_INPUT", "Error code must be correct");
		...
		QUnit.test("mixAttributesAndSelectionVariant: suppression behavior = raiseErrorOnNull | ignoreEmptyString", function(assert) {
			...
			try {
				oActualSelectionVariant = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					mSmanticAttributes,
					oSelVar.toJSONString(),
					SuppressionBehavior.raiseErrorOnNull | SuppressionBehavior.ignoreEmptyString
				);
			} catch (oError) {
				bError = true;
				assert.equal(oError.getErrorCode(), "NavigationHandler.INVALID_INPUT", "Error code must be correct");
		...
		QUnit.test("mixAttributesAndSelectionVariant: suppression behavior = raiseErrorOnUndefined | ignoreEmptyString", function(assert) {
			...
			try {
				NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					mSmanticAttributes,
					oSelVar.toJSONString(),
					SuppressionBehavior.raiseErrorOnUndefined | SuppressionBehavior.ignoreEmptyString
				);
			} catch (oError) {
				bError = true;
				assert.equal(oError.getErrorCode(), "NavigationHandler.INVALID_INPUT", "Error code must be correct");
		...
		QUnit.test(
			"mixAttributesAndSelectionVariant: suppression behavior = raiseErrorOnNull | raiseErrorOnUndefined | ignoreEmptyString",
			function(assert) {
				// positive test
				var oSelVar = new SelectionVariant();
				var mSmanticAttributes = {
				...
				oActualSelectionVariant = NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					mSmanticAttributes,
					oSelVar.toJSONString(),
					SuppressionBehavior.raiseErrorOnNull | SuppressionBehavior.raiseErrorOnUndefined | SuppressionBehavior.ignoreEmptyString
				);
				...
				assert.deepEqual(
					oActualSelectionVariant.getSelectOptionsPropertyNames(),
					[],
					"Only expected select options must be created"
				);
		...
		QUnit.test("mixAttributesAndSelectionVariant: suppression behavior = raiseErrorOnNull | raiseErrorOnUndefined", function(assert) {
			...
			try {
				NavigationHandler.prototype.mixAttributesAndSelectionVariant(
					mSmanticAttributes,
					oSelVar.toJSONString(),
					SuppressionBehavior.raiseErrorOnNull | SuppressionBehavior.raiseErrorOnUndefined
				);
			} catch (oError) {
				bError = true;
				assert.equal(oError.getErrorCode(), "NavigationHandler.INVALID_INPUT", "Error code must be correct");
		...
		QUnit.test("mixAttributesAndSelectionVariant: semantic attribute with invalid types", function(assert) {
			...
			fnTestFunction(function() {}, "function");
			fnTestFunction(/test/, "regexp");
			fnTestFunction(new Error(), "object");
		});

		QUnit.module("mixAttributesAndSelectionVariant: with multiple of semantic attributes", {
			beforeEach: function() {},
			afterEach: function() {}
		});
		QUnit.test("mixAttributesAndSelectionVariant - For Merging SV and Context with no same property paths", function(assert) {
			var oSelVar = new SelectionVariant();
			oSelVar.addSelectOption("SalesOrder", "I", "EQ", "6437");
			oSelVar.addSelectOption("SalesOrder", "I", "EQ", "29876");
			var oSelData = [
				{
					SalesOrder: "6437"
				},
			...
			var oExpectedResult = {
					...
					"_mSelectOptions": {
						"SalesOrder": [
							{ "Sign": "I", "Option": "EQ", "Low": "6437", "High": null },
							{ "Sign": "I", "Option": "EQ", "Low": "29876", "High": null }
						],
						"attribute1": [{ "Sign": "I", "Option": "EQ", "Low": "value2", "High": null }]
					},
					"_sId": ""
				},
				oSV = oSelVar.addSelectOption("attribute1", "I", "EQ", "value2");
			var actualValue = NavigationHandler.prototype.mixAttributesAndSelectionVariant(oSelData, oSV.toJSONString());
			assert.ok(JSON.stringify(actualValue) === JSON.stringify(oExpectedResult));
		});

		QUnit.test("mixAttributesAndSelectionVariant - For Merging SV and Context with same property paths", function(assert) {
			var oSV = new SelectionVariant({
				"_mParameters": {},
				"_mSelectOptions": {
					"SalesOrder": [{ "Sign": "I", "Option": "EQ", "Low": "29877", "High": null }]
				},
				"_sId": ""
			});
			var oSelData = [
				...
				oExpectedResult = {
					"_mParameters": {},
					"_mSelectOptions": {
						"SalesOrderItem": [{ "Sign": "I", "Option": "EQ", "Low": "1", "High": null }],
						"TotalNetAmount": [
							{ "Sign": "I", "Option": "EQ", "Low": "120", "High": null },
							{ "Sign": "I", "Option": "EQ", "Low": "10", "High": null }
						],
						"SalesOrder": [{ "Sign": "I", "Option": "EQ", "Low": "2", "High": null }]
					},
					"_sId": ""
				},
				actualValue = NavigationHandler.prototype.mixAttributesAndSelectionVariant(oSelData, oSV.toJSONString());
			assert.ok(JSON.stringify(oExpectedResult) === JSON.stringify(actualValue));
		});
	}
);
