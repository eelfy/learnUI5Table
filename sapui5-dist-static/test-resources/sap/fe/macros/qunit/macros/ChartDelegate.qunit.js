...
sap.ui.define(
	...
	function(deepEqual, TestUtils, ODataModel, ChartDelegate, coreLib) {
		"use strict";
		// *********************************************************************************************
		QUnit.module("Chart Delegate Tests for an OData V4 Service", {
			before: function() {
				...
				return ChartDelegate.retrieveAllMetadata(this.oChart).then(
					function(mMetadata) {
		...
		QUnit.test("Fetch Properties returns a parts", function(assert) {
			...
			ChartDelegate.fetchProperties(this.oChart).then(
		...
		QUnit.test("Chart Items and item count", function(assert) {
			...
			assert.equal(aItems.length, 12, "There are 11 items");
			var iCountCustom = 0;
		...
		QUnit.test("Labels of custom aggregates", function(assert) {
			...
			assert.equal(
				oItem.label,
				"Custom Aggregate (SalesAmountTotal)",
				"There is no label in metadata thus a default label is filled"
			);
			...
			assert.equal(
				oItem.label,
				"Custom Aggregate (NumberOfCustomers)",
				"There is no label in metadata thus a default label is filled"
			);
		});
		...
		QUnit.test("Labels and Names of aggregated properties", function(assert) {
			...
			assert.equal(oSalesAmount.chartItems.length, 2, "There are 2 items for the sales amount");

			var aItems = oSalesAmount.chartItems;

			//Supported aggregation methods are in order sum, min, max, average
			assert.equal(aItems[0].aggregationMethod, "min");
			assert.equal(aItems[1].aggregationMethod, "max");

			//While min/max are Aggregated properties there label counts for the others
			//the label is generated
			assert.equal(aItems[0].label, "Aggregatable property (minSalesMount)");
			//note minSalesMount has no label
			assert.equal(aItems[1].label, "Maximum Sales Amount");

			//While min/max are Aggregated properties there name counts for the others
			//the label is generated
			assert.equal(aItems[0].name, "minSalesMount");
			assert.equal(aItems[1].name, "maxSalesMount");
		});
	}
);
