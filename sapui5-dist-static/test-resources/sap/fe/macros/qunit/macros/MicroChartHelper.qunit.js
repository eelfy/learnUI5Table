...
/* global QUnit */
sap.ui.define(["sap/fe/macros/microchart/MicroChartHelper", "sap/base/Log"], function(MicroChartHelper, Log) {
	"use strict";
	...
	QUnit.test("Unit test to check getCriticalityBinding", function(assert) {
		[
			...
			assert.equal(actualValue, oProperty.bExpectedValue, "Unit test to check getCriticalityBinding " + oProperty.sMessage + " : ok");
		});
	});
		...
		[
			{
				...
				bExpectedValue:
					"/SalesOrderManage/@com.sap.vocabularies.UI.v1.Facets#MicroCharts/0/Target/$AnnotationPath/MeasureAttributes/1/DataPoint/$AnnotationPath/",
	...
	QUnit.test("Unit test to check getSelectParameters", function(assert) {
		[
			...
			assert.equal(actualValue, oProperty.bExpectedValue, "Unit test to check getSelectParameters " + oProperty.sMessage + " : ok");
		});
	});
