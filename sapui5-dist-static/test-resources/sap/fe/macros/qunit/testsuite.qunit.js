sap.ui.define(["sap/ui/Device", "sap/base/util/LoaderExtensions"], function(Device, LoaderExtensions) {
	"use strict";
	sap.ui.loader.config({ paths: { "/test/sap/fe/macros/qunit": "/test-resources/sap/fe/macros/qunit" } });
	var oTests = LoaderExtensions.loadResource("/test/sap/fe/macros/qunit/index.json");
	return {
		...
		defaults: {
			...
			ui5: {
				...
				libs: ["sap.ui.core", "sap.m", "sap.fe.macros"], // Libraries to load upfront in addition to the library which is tested (sap.ui.mdc), if null no libs are loaded
				"xx-waitForTheme": true
				// Whether the start of the test should be delayed until the theme is applied
			},
			coverage: {
				only: "[sap/fe]", // Which files to show in the coverage report, if null, no files are excluded from coverage
				branchCoverage: true
