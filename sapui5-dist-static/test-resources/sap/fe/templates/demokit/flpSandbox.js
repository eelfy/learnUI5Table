(function() {
	...
	var __bKarmaTest = false;
	if (window.parent && window.parent.__karma__) {
		__bKarmaTest = true;
	}
	...
	window.onInit = function() {
		sap.ui.require(["sap/ui/thirdparty/jquery", "local/mockServer"], function(jQuery, mockServer) {
				...
				if (proxyPrefix) {
					...
					jQuery.ajax = function(sUrl) {
						if (
							sUrl &&
							typeof sUrl === "string" &&
							(sUrl.indexOf("/sap/opu/odata4") === 0 ||
								sUrl.indexOf("/odata/v4") === 0 ||
								sUrl.indexOf("/catalog") === 0 ||
								sUrl.indexOf("/tenant") === 0)
						) {
							if (proxyPrefix.endsWith("/") && sUrl.startsWith("/")) {
								sUrl = proxyPrefix + sUrl.substr(1);
							} else {
			...
			//set flexibility connectors for current app, SessionStorageConnector is default
