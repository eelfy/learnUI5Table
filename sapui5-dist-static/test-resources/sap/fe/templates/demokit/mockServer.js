sap.ui.define(
	["sap/ui/thirdparty/sinon", "sap/fe/macros/filter/DraftEditState", "local/mockServerHelper", "sap/ui/util/XMLHelper", "sap/base/Log"],
	function(sinon, DraftEditState, mockServerHelper, XMLHelper, Log) {
		"use strict";

		function filterEditState(sEditState, aData) {
			switch (sEditState) {
				case DraftEditState.ALL:
					return aData;
				case DraftEditState.UNCHANGED:
					return aData.filter(function(oEntry) {
						return oEntry.IsActiveEntity && !oEntry.HasDraftEntity;
					});
				case DraftEditState.OWN_DRAFT:
					return aData.filter(function(oEntry) {
						return !oEntry.IsActiveEntity;
					});
				case DraftEditState.LOCKED:
					return aData.filter(function(oEntry) {
						return (
							oEntry.IsActiveEntity && oEntry.DraftAdministrativeData && oEntry.DraftAdministrativeData.InProcessByUser !== ""
						);
					});
				case DraftEditState.UNSAVED_CHANGES:
					return aData.filter(function(oEntry) {
						return (
							oEntry.IsActiveEntity && oEntry.DraftAdministrativeData && oEntry.DraftAdministrativeData.InProcessByUser === ""
						);
			...
			for (var i = 0; i < oApp.mockDataSets.length; i++) {
				if (sUrl.indexOf(oApp.mockDataSets[i]) > -1) {
					...
					if (sUrl.indexOf("$filter") > -1) {
						...
						aEditState = Object.keys(DraftEditState);
						for (var i = 0; i < aEditState.length; i++) {
							sFilter = DraftEditState.getFilterForEditState(i.toString());
							if (sFilter && sFilterString.indexOf(sFilter) > -1) {
								sEditStateKey = aEditState[i];
								break;
							}
						}
						if (sEditStateKey) {
							sEditState = DraftEditState[sEditStateKey];
						}
						sFilter = DraftEditState.getFilterForEditState(sEditState);
