...
sap.ui.define("sap.gantt2.simple.PerfSetting", [
	"sap/ui/base/ManagedObject", 'sap/ui/core/format/DateFormat',"sap/ui/table/Column"
], function (ManagedObject, DateFormat, Column) {
	"use strict";
	...
	var PerfSetting = ManagedObject.extend("sap.gantt2.simple.PerfSetting", {
		...
		metadata: {
			...
			properties: {
				 ...
				 */
				numberOfColumns: { type: 'int', defaultValue: 1 },
	...
	PerfSetting.prototype.generate = function() {
		...
		this.columnData = this._generateColumnData();
	...
	PerfSetting.prototype._generateColumnData = function() {
		var columnsArray = [];
		var iTotalColumns = this.getNumberOfColumns();
		for (var iIndex = 0; iIndex < iTotalColumns; iIndex++) {
			columnsArray.push(new Column({
				width: "100px",
				sortProperty: "ObjectID",
				filterProperty: "ObjectID",
				label: new sap.m.Label({ text: "C" + "_" + iIndex }),
