...
sap.ui.define([
...
], function(Log, Core, BaseShape, CoordinateUtils, qutils, utils) {
	"use strict";
	...
	QUnit.test("Verify the horizontalTextAlignment property", function (assert) {
		...
		oShape.setHorizontalTextAlignment("Ribbon");
		assert.strictEqual(oShape.getHorizontalTextAlignment(), "Ribbon", "Ribbon value of the horizontalTextAlignment property is Ribbon");
	});
	...
	QUnit.test("Interaction - onmouseover", function(assert) {
		var done1 = assert.async();
		utils.waitForGanttRendered(this.sut).then(function () {
			this.delayedAssert(function() {
				var oFirstShape = this.getFirstShape();
				...
				var oHoverSpy = sinon.spy(oFirstShape, "onmouseover");
				var oHandleShapeMouseEnter = sinon.spy(this.sut, "handleShapeMouseEnter");
				sinon.stub(CoordinateUtils, "getCursorElement").returns(oFirstShape.getDomRef());
				qutils.triggerEvent("mouseenter", oFirstShape.getId());
				assert.ok(oHoverSpy.calledOnce, "user has hovered on the first shape");
	...
	QUnit.test("Interaction - onmouseout", function(assert) {
		var done1 = assert.async();
		utils.waitForGanttRendered(this.sut).then(function () {
			this.delayedAssert(function() {
				var oFirstShape = this.getFirstShape();
				...
				var oHoverSpy = sinon.spy(oFirstShape, "onmouseout");
				var oHandleShapeMouseLeave = sinon.spy(this.sut, "handleShapeMouseLeave");
				qutils.triggerEvent("mouseout", oFirstShape.getId());
				assert.ok(oHoverSpy.calledOnce, "user has hovered out of the first shape");
