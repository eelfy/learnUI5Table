...
sap.ui.define([
...
], function (Core, ContainerToolbar, ContainerToolbarPlaceholder, library, SettingItem, JSONModel, ButtonType, OverflowToolbar, ControlVariantApplyAPI) {
	"use strict";
	...
	QUnit.test("Test zoom in button." , function (assert) {
		...
		oToolbar.attachEventOnce("zoomStopChange", function(oEvent){
			...
			assert.equal(oToolbar._oZoomInButton.getAriaDescribedBy()[0], oToolbar._oZoomSlider.getId(), "Aria described by for Zoom in button");
	...
	QUnit.test("Test zoom out button." , function (assert) {
		...
		oToolbar.attachEventOnce("zoomStopChange", function(oEvent){
			...
			assert.equal(oToolbar._oZoomOutButton.getAriaDescribedBy()[0], oToolbar._oZoomSlider.getId(), "Aria described by for Zoom in button");
	...
	QUnit.test("Test mSettingsConfig" , function (assert) {
		...
		setTimeout(function(){
			...
			setTimeout(function(){
				assert.equal(Object.keys(oToolbar.mSettingsConfig).length, 4);
	...
	QUnit.test("Exit destroys all available toolbar controls", function (assert) {
		//Arrange
		var oToolbar = new ContainerToolbar({
			showDisplayTypeButton: true,
			showBirdEyeButton: true,
