...
sap.ui.define([
...
], function (SvgDefs, SlashPattern, DataProducer, QUnitUtils) {
	"use strict";
	...
	function _testRowChangeEvent(assert, oEventParams, aRowCount, aAsyncDone) {
		...
		var fCheckRowChange = function(oEvent){
			...
			assert.equal(document.getElementsByClassName("sapUiTableRowSel")[0].rowIndex - 1,
				aSelectedRows[0].rowIndex, "The selected row is :" + aSelectedRows[0].rowIndex);
			fnDone();
		};
		if (!oChart.mEventRegistry.rowSelectionChange || oChart.mEventRegistry.rowSelectionChange.length < 1) {
			oChart.attachRowSelectionChange(fCheckRowChange, this);
		}

		QUnitUtils.triggerEvent("mousedown", jQuery("#" + oChart.getId() + "-svg-ctn"),
				oEventParams);
		QUnitUtils.triggerEvent("mouseup", jQuery("#" + oChart.getId() + "-svg-ctn"),
				oEventParams);
	}
	...
	function doTestRowChangeEvent() {
		QUnit.test("Row Selection Change Event: select one row", function(assert) {
			...
			oEventParams.detail = 1;
			oEventParams.offsetX = 1309;
			oEventParams.offsetY = 16;
			oEventParams.pageX = 830;
			oEventParams.pageY = 500;
			oEventParams.clientX = 830;
			oEventParams.clientY = 580;
			oEventParams.shiftKey = false;
			oEventParams.ctrlKey = false;
	...
	// }
		...
		QUnit.module("Row Selection Change");
		setTimeout(function () {
			window.scrollBy(0,100);
			doTestRowChangeEvent();
		}.bind(this), 2000);

		QUnit.module("Shapes with Text");
		QUnit.test("Validate Shape id is added to Elipsis", function(assert) {
			var fnDone = assert.async();
			setTimeout(function () {
				assert.equal(oChart.getShapes()[0].getGroupAggregation()[2].getShapeProperties().truncateWidth, 20,  "Truncate width has been set to 20.");
				if (oChart.getDomRef().querySelector(".sapGanttChartSvg").children != null && oChart.getDomRef().querySelector(".sapGanttChartSvg").children[3].children != null) {
					for (var i = 0; i < oChart.getDomRef().querySelector(".sapGanttChartSvg").children[3].children.length; i++) {
						for (var j = 0; j < oChart.getDomRef().querySelector(".sapGanttChartSvg").children[3].children[i].children.length; j++) {
							assert.equal(oChart.getDomRef().querySelector(".sapGanttChartSvg").children[3].children[i].children[j].querySelector(".sapGanttShapeSvgText").children[0].nodeName, "tspan", "Current Element is tspan");
							assert.ok(oChart.getDomRef().querySelector(".sapGanttChartSvg").children[3].children[i].children[j].querySelector(".sapGanttShapeSvgText").children[0].classList, "Class has been added to tspan");
						}
					}
				}
				fnDone();
