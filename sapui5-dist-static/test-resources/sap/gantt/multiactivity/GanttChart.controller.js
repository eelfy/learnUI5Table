sap.ui.define([
...
], function(uid, Controller, JSONModel, MockServer, ODataModel, AdhocLine, Format, BaseRectangle, ListLegend, ListLegendItem, DimensionLegend, LegendColumnConfig, LegendRowConfig, GanttPrinting) {
	"use strict";
	...
	return Controller.extend("sap.gantt.multiactivity.test.Controller", {
		...
		handleMouseEnterLeave: function(oEvent, bEnter) {
			...
			if (!this.oPopover) {
				this.oPopover = sap.ui.xmlfragment("sap.gantt.multiactivity.test.view.QuickViewPopover", this);
		...
		onShapeContextMenu: function(oEvent) {
			...
			if (oShape) {
				...
				oMenu.open(true, jQuery(document.getElementById(oShape.getId())), eDock.BeginTop, eDock.LeftTop, null, sOffset);
				...
				oMenu.open(true, jQuery(document.getElementById(oRow.getId())), eDock.BeginTop, eDock.LeftTop, null, sOffset);
			} else {
		...
		switchRowSettings: function(oEvent) {
			...
			var oRowSettings = sap.ui.xmlfragment("sap.gantt.multiactivity.test.view." + oEvent.getParameter("item").getKey(), this);
		...
		onPressAddTask: function(oEvent) {
			...
			if (!this.oAddTaskPopover) {
				this.oAddTaskPopover = sap.ui.xmlfragment("sap.gantt.multiactivity.test.view.AddTask", this);
