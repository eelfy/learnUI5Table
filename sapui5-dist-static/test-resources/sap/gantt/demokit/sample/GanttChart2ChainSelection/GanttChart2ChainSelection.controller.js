sap.ui.define([
...
], function (Controller, MockServer, ODataModel, Relationship, AdhocLine, GanttUtils) {
	"use strict";
	...
	return Controller.extend("sap.gantt.sample.GanttChart2ChainSelection.GanttChart2ChainSelection", {
		...
		onShapePress:function(oEvent){
			var oShape = oEvent.getParameter('shape');
			var bCtrlOrMeta = oEvent.getParameter('ctrlOrMeta');
			if (oShape && oShape.isA('sap.gantt.simple.BaseRectangle')){
				var oGantt = sap.ui.getCore().byId(oEvent.getSource().getId());
				GanttUtils.selectAssociatedShapes({
					shape: oShape,
					ctrlOrMeta: bCtrlOrMeta
				}, oGantt);
			}
		},
		fnChainSelection: function(name){
			if (name === 'Tooling Approval'){
				return false;
			}
			return true;
		}
